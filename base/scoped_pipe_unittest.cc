...
class ScopedPipeTest : public ::testing::Test {
 protected:
  void SetUp() override { g_freed_values.clear(); }
};

}  // namespace

TEST(ScopedPipeTest, Close) {
  { ScopedPipe<IntTraits> x(IntTraits::kInvalidValue); }
  ASSERT_TRUE(g_freed_values.empty());

  { ScopedPipe<IntTraits> x(3); }
  ASSERT_EQ(1u, g_freed_values.size());
  EXPECT_EQ(3, g_freed_values[0]);
...

  {
    ...
    ScopedPipe<IntTraits> y(IntTraits::kInvalidValue);
...

  {
    ...
    EXPECT_EQ(4, g_freed_values[0]);
  }
  ASSERT_EQ(2u, g_freed_values.size());
  EXPECT_EQ(4, g_freed_values[0]);
  EXPECT_EQ(3, g_freed_values[1]);
  g_freed_values.clear();
}
