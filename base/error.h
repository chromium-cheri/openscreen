...
class Error {
 public:
  ...
  Error();
  explicit Error(const Error& error);
  ...
  static const std::string CodeToString(Error::Code code);
...
class ErrorOr {
 public:
  explicit ErrorOr(ErrorOr&& error_or) = default;
  explicit ErrorOr(std::unique_ptr<Value> value) : value_(std::move(value)) {}
  explicit ErrorOr(Value* value) : value_(value) {}
  explicit ErrorOr(Error&& error) : error_(std::move(error)) {}
  explicit ErrorOr(const Error& error) : error_(error) {}
  explicit ErrorOr(Error::Code code) : error_(code) {}
  ErrorOr(Error::Code code, std::string&& message) : error_(code, message) {}
  ...
  bool is_value() const { return value_.get(); }
  ...
  std::unique_ptr<Value>&& MoveValue() {
