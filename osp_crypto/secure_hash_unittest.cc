...
TEST(SecureHashTest, TestUpdate) {
  ...
  uint8_t output3[SHA256_DIGEST_LENGTH];
...
TEST(SecureHashTest, TestClone) {
  ...
  uint8_t output1[SHA256_DIGEST_LENGTH];
  ...
  EXPECT_EQ(0, memcmp(output1, output2, SHA256_DIGEST_LENGTH));
  EXPECT_EQ(0,
            memcmp(output1, kExpectedHashOfInput1And2, SHA256_DIGEST_LENGTH));

  // Finish() ctx3, which should produce the hash of input1.
  ctx3->Finish(&output3, sizeof(output3));
  EXPECT_EQ(0, memcmp(output3, kExpectedHashOfInput1, SHA256_DIGEST_LENGTH));
}
