...
TEST(SecureHashTest, TestUpdate) {
  ...
  for (size_t i = 0; i < kSHA256Length; i++)
...
TEST(SecureHashTest, TestClone) {
  ...
  const uint8_t kExpectedHashOfInput1[kSHA256Length] = {
  ...
  const uint8_t kExpectedHashOfInput1And2[kSHA256Length] = {
  ...
  uint8_t output1[kSHA256Length];
  uint8_t output2[kSHA256Length];
  uint8_t output3[kSHA256Length];

  std::unique_ptr<SecureHash> ctx1(SecureHash::Create(SecureHash::SHA256));
  ctx1->Update(input1.data(), input1.size());

  std::unique_ptr<SecureHash> ctx2(ctx1->Clone());
  std::unique_ptr<SecureHash> ctx3(ctx2->Clone());
  ...
  EXPECT_EQ(0, memcmp(output1, output2, kSHA256Length));
  EXPECT_EQ(0, memcmp(output1, kExpectedHashOfInput1And2, kSHA256Length));

  // Finish() ctx3, which should produce the hash of input1.
  ctx3->Finish(&output3, sizeof(output3));
  EXPECT_EQ(0, memcmp(output3, kExpectedHashOfInput1, kSHA256Length));
}

TEST(SecureHashTest, TestLength) {
  std::unique_ptr<SecureHash> ctx(SecureHash::Create(SecureHash::SHA256));
  EXPECT_EQ(kSHA256Length, ctx->GetHashLength());
}
...
TEST(SecureHashTest, Equality) {
  ...
  std::unique_ptr<SecureHash> ctx2(SecureHash::Create(SecureHash::SHA256));
