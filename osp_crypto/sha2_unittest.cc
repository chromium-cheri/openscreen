...
namespace {
template <typename T>
void ExpectArrayEquals(const T* expected, const T* actual, size_t len) {
...
TEST(Sha256Test, Test1) {
  ...
  ExpectArrayEquals(kExpected, output, SHA256_DIGEST_LENGTH);
}
...
TEST(Sha256Test, Test1_String) {
  ...
  ExpectArrayEquals(kExpected, reinterpret_cast<const uint8_t*>(output.c_str()),
