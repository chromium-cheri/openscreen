...
TEST(Sha256Test, Test1) {
  ...
  SHA256HashString(input1, output1, sizeof(output1));
  ...
  SHA256HashString(input1, output_truncated1, sizeof(output_truncated1));
...
TEST(Sha256Test, Test1_String) {
  ...
  std::string output1 = SHA256HashString(input1);
  ASSERT_EQ(kSHA256Length, output1.size());
  for (size_t i = 0; i < kSHA256Length; i++)
...
TEST(Sha256Test, Test2) {
  ...
  SHA256HashString(input2, output2, sizeof(output2));
  for (size_t i = 0; i < kSHA256Length; i++)
    EXPECT_EQ(expected2[i], static_cast<int>(output2[i]));

  uint8_t output_truncated2[6];
  SHA256HashString(input2, output_truncated2, sizeof(output_truncated2));
...
TEST(Sha256Test, Test3) {
  ...
  SHA256HashString(input3, output3, sizeof(output3));
  for (size_t i = 0; i < kSHA256Length; i++)
    EXPECT_EQ(expected3[i], static_cast<int>(output3[i]));

  uint8_t output_truncated3[12];
  SHA256HashString(input3, output_truncated3, sizeof(output_truncated3));
