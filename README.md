...
must use the GN argument `use_coverage=true`.  You can read more about this in
[clang's documentation](http://clang.llvm.org/docs/SourceBasedCodeCoverage.html)
but the bare minimum steps are also outlined below.  You will also need to
download the pre-built clang coverage tools, which are not normally downloaded.
The easiest way to do this is to set a custom variable in your `.gclient` file.
...
`clang_coverage` hook in `//DEPS` yourself or even download the tools manually
...
third_party/llvm-build/Release+Asserts/bin/llvm-cov show out/coverage/openscreen_unittests -instr-profile=foo.profdata -format=html [filter paths] > coverage.html
```
There are a few things to note here:
 ...
 - To just get a high-level report, run `llvm-cov report
 out/coverage/openscreen_unittests -instr-profile=foo.profdata [filter paths]`.
