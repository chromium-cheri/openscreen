...
`gn`, `clang-format`, and `ninja` must be included in your `$PATH` or the build
will fail.
...
The instructions above using `git cl upload` are preferred for patch management.
However, you can interact manually with the Gerrit code review system via `git`
