...
The following are the main tools are required for development/builds:
 ...
 - Compiler/Linker: `clang` (default) or `gcc` (you're on your own)

All of these are automatically downloaded/updated for the Linux and Mac
environments via `gclient sync` as described above. The first two are installed
into `buildtools/<platform>/`.

Linux only: The LLVM toolchain (clang) is downloaded as a third-party package
on-demand by the `gn` build. For Mac, XCode must be installed on the system.
