...
Information about the Open Scren Protocol and its specification can be found [on
...
``` bash
  mkdir out/
  gn gen out/             # Creates the build directory and necessary ninja files
  ninja -C out/ osp_demo  # Builds the executable with ninja
  ./out/osp_demo          # Runs the executable
```
...
Running `ninja -C out/ gn_all` will build all non-test targets in the
repository.

`gn ls --type=executable out/` will list all of the executable targets
that can be built.

If you want to customize the build further, you can run `gn args out/` to
pull up an editor for build flags. `gn args --list out/` prints all of
the build flags available.

## Building and running unit tests

```bash
  ninja -C out/ openscreen_unittests
  ./out/openscreen_unittests
```
