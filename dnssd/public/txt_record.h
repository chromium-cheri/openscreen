...
class TxtRecord {
 public:
  ...
  virtual Error SetValue(const absl::string_view& key,
                         const absl::Span<uint8_t>& value) = 0;
  virtual Error SetFlag(const absl::string_view& key, bool value) = 0;
  ...
  virtual ErrorOr<absl::Span<uint8_t>> GetValue(
      const absl::string_view& key) = 0;
  virtual ErrorOr<bool> GetFlag(const absl::string_view& key) = 0;
};
