...
void HandleEvents(mdns::MdnsResponderAdapterImpl* mdns_adapter) {
  for (auto& ptr_event : mdns_adapter->TakePtrResponses()) {
    auto it = std::find_if(
        g_services.begin(), g_services.end(), [&ptr_event](const Service& s) {
          return s.service_instance == ptr_event.service_instance;
        });
    switch (ptr_event.header.response_type) {
      ...
      case mdns::ResponseType::kAddNoCache:
        ...
        if (it == g_services.end()) {
          g_services.emplace_back(std::move(ptr_event.service_instance));
        }
        break;
  ...
  for (auto& srv_event : mdns_adapter->TakeSrvResponses()) {
    auto it = std::find_if(
        g_services.begin(), g_services.end(), [&srv_event](const Service& s) {
          return s.service_instance == srv_event.service_instance;
        });
    if (it == g_services.end()) {
      continue;
    }
    switch (srv_event.header.response_type) {
      ...
      case mdns::ResponseType::kAddNoCache:
        ...
        it->domain_name = std::move(srv_event.domain_name);
        it->port = srv_event.port;
        break;
      case mdns::ResponseType::kRemove:
        LOG_WARN << "srv-remove: " << srv_event.service_instance;
        it->domain_name = mdns::DomainName();
        it->port = 0;
        break;
    }
  }
  for (auto& txt_event : mdns_adapter->TakeTxtResponses()) {
    auto it = std::find_if(
        g_services.begin(), g_services.end(), [&txt_event](const Service& s) {
          return s.service_instance == txt_event.service_instance;
        });
    if (it == g_services.end()) {
      continue;
    }
    switch (txt_event.header.response_type) {
      case mdns::ResponseType::kAdd:
      case mdns::ResponseType::kAddNoCache:
        it->txt = std::move(txt_event.txt_info);
        break;
      case mdns::ResponseType::kRemove:
  ...
  for (const auto& a_event : mdns_adapter->TakeAResponses()) {
    ...
    auto it = std::find_if(g_services.begin(), g_services.end(),
                           [&a_event](const Service& s) {
                             return s.domain_name == a_event.domain_name;
                           });
    if (it == g_services.end()) {
      continue;
    }
    switch (a_event.header.response_type) {
      case mdns::ResponseType::kAdd:
      case mdns::ResponseType::kAddNoCache:
        it->v4_address = a_event.address;
        break;
      case mdns::ResponseType::kRemove:
        LOG_WARN << "a-remove: " << a_event.domain_name;
        it->v4_address = IPv4Address(0, 0, 0, 0);
        break;
    }
  }
}
...
void BrowseDemo(const mdns::DomainName& service_type,
  ...
  if (service_type.GetLabels() ==
      std::vector<std::string>{"_openscreen", "_udp"}) {
    mdns_adapter->RegisterService(service_name, service_type,
                                  mdns::DomainName(), 12345,
                                  {"yurtle", "turtle"});
  }
  ...
  while (!g_done) {
    ...
    auto data = platform::OnePlatformLoopIteration(
        waiter, platform::TimeDelta::FromMilliseconds(500));
...
int main(int argc, char** argv) {
  ...
  openscreen::BrowseDemo(d, service_name);
