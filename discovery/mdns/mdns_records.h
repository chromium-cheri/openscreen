...
class NsecRecordRdata {
 public:
  ...
  NsecRecordRdata(DomainName next_domain_name, Types... types)
      : types_{types...}, next_domain_name_(next_domain_name) {
    OSP_DCHECK(types_.size());

    // Sort the types_ array for easier comparison later.
    std::sort(types_.begin(), types_.end());

    size_t max_size = next_domain_name_.MaxWireSize();

    // Calculate the bit map size as described in RFC 4034 Section 4.1.2.
    int current_block = -1;
    size_t current_bitfield_byte = 0;
    for (DnsType type : types_) {
      uint16_t as_int = static_cast<uint16_t>(type);
      int block = as_int >> 8;
      size_t bitfield_byte = (as_int & 0xFF) / 8;
      if (block > current_block) {
        current_block = block;
        current_bitfield_byte = 0;
        max_size += 3;  // 1 from window, 1 from size, 1 from bitfield.
      }
      if (bitfield_byte > current_bitfield_byte) {
        max_size += current_bitfield_byte - bitfield_byte;
        current_bitfield_byte = bitfield_byte;
      }
    }

    max_wire_size_ = max_size;
  }
