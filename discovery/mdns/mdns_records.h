...
class OptRecordRdata {
 public:
  ...
  struct Option {
    ...
    // working group and the IESG, as specified in RFC6891 section 9.1.
    uint16_t code;
...
bool CanBeProcessed(const MdnsRecord& record);

// The below functions are only valid for records with DnsType kOPT, and act as
// defined in RFC6891 section 6.1.2.
//
// The number of octets of the largest UDP payload that can be reassembled and
// delivered in the requestor's network stack. Note that path MTU, with or
// without fragmentation, could be smaller than this.
...
// Returns whether the version stored in this instance is BADVERS as defined in
// RFC6891 section 9.
