...
void ApplyQueryResults(MdnsMessage* message,
  ...
  // records may be added as negative response NSEC records in the case of their
  // non-existence.
  ...
  // section. Per RFC 6762 section 6.1, if these records are not present a
  // negative response NSEC record may be added to show their non-existence if
  // their name and class match that which is being queried for.
  ...
  // records section if present, and a negative response NSEC record be added if
  // not to show its non-existence.
  ...
  // The supported remaining records are of types TXT, NSEC, and ANY. No
  // additional records are recommended for the types TXT or ANY, and NSEC
  // records are not expected to be queried for.
}
