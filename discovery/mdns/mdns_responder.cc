...
bool AddRecord(std::function<void(MdnsRecord record)> add_func,
  ...
  auto extra_records = record_handler->GetRecords(domain, type, clazz);
...
inline bool AddAdditionalRecords(MdnsMessage* message,
  ...
  return AddRecord(add_func, record_handler, domain, type, clazz, true);
}
...
inline bool AddResponseRecords(MdnsMessage* message,
  ...
  return AddRecord(add_func, record_handler, domain, type, clazz,
...
void MdnsResponder::OnMessageReceived(const MdnsMessage& message,
  ...
  for (const auto& question : message.questions()) {
    ...
    if (is_exclusive_owner ||
      ...
      if (is_exclusive_owner) {
        SendResponse(question, send_response);
      } else {
        const auto delay = random_delay_->GetSharedRecordResponseDelay();
        std::function<void()> response = [this, question, send_response]() {
          SendResponse(question, send_response);
        };
