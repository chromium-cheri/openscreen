...
class MdnsProbe {
 public:
  ...
  const DomainName& target_name() const { return target_; }

  const IPEndpoint endpoint() const { return endpoint_; }

 private:
  // Implement the probing method as described in RFC 6762 section 8.1. In place
  // of the MdnsRecord(s) that will be published, a 'fake' mDNS record of type A
  // or AAAA will be generated from the |endpoint_| variable provided with TTL 2
  // seconds. 0 or 1 seconds are not used because these constants are used as
  // part of goodbye records, so poorly written receivers may handle these cases
  // in unexpected ways. Caching of probe queries is not supported for mDNS
  // probes (else, in a probe which failed, invalid records would be cached). If
  // for some reason this did occur, though, it should be a non-issue because
  // the probe record will expire after 2 seconds.
