...
RawRecordRdata::RawRecordRdata(std::vector<uint8_t> rdata)
  ...
  OSP_CHECK(rdata_.size() <= kMaxRawRecordSize);
}
...
MdnsMessage::MdnsMessage(uint16_t id,
  ...
  OSP_CHECK(questions_.size() < kMaxMessageFieldEntryCount);
  OSP_CHECK(answers_.size() < kMaxMessageFieldEntryCount);
  OSP_CHECK(authority_records_.size() < kMaxMessageFieldEntryCount);
  OSP_CHECK(additional_records_.size() < kMaxMessageFieldEntryCount);
