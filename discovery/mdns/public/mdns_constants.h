...
constexpr uint8_t MakeExtendedRcode(uint32_t rcode_and_flags) {
  return static_cast<uint8_t>((rcode_and_flags & kExtendedRcodeMask) >>
                              kExtendedRcodeShift);
}

constexpr uint8_t MakeVersion(uint32_t rcode_and_flags) {
  return static_cast<uint8_t>((rcode_and_flags & kVersionMask) >>
                              kVersionShift);
}

constexpr bool MakeOkBit(uint32_t rcode_and_flags) {
  return rcode_and_flags & kDnssecOkBitMask;
}
