 ...
 protected:
  ...
  void TestUniqueRecordRegistrationWorkflow(MdnsRecord record,
    ...
    EXPECT_CALL(sender_, SendMulticast(_))
        .WillOnce(Invoke([this, &record](const MdnsMessage& message) -> Error {
    ...
    EXPECT_CALL(sender_, SendMulticast(_))
        .WillOnce(Invoke([this, &record2](const MdnsMessage& message) -> Error {
...
TEST_F(MdnsPublisherTest, PTRRecordRegistrationWorkflow) {
  ...
  // Register a new record.
  EXPECT_CALL(sender_, SendMulticast(_))
      .WillOnce(Invoke([this, &record1](const MdnsMessage& message) -> Error {
