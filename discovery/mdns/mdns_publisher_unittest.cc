...
class MdnsPublisherTesting : public MdnsPublisher {
 public:
  using MdnsPublisher::EnumeratePtrRecords;
...
TEST_F(MdnsPublisherTest, PTRRecordRegistrationWorkflow) {
  ...
  ASSERT_NE(record, record2);
  ASSERT_TRUE(records.empty());
  ASSERT_EQ(publisher_.EnumeratePtrRecords(DnsClass::kANY).size(), size_t{0});
  ...
  ASSERT_EQ(publisher_.EnumeratePtrRecords(DnsClass::kANY).size(), size_t{1});
  ...
  ASSERT_EQ(publisher_.EnumeratePtrRecords(DnsClass::kANY).size(), size_t{2});
