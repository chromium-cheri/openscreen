...
class MdnsPublisherTest : public testing::Test {
 ...
 protected:
  void TestUniqueRecordRegistrationWorkflow(MdnsRecord record,
    ...
    EXPECT_TRUE(publisher_.RegisterRecord(record).ok());
    EXPECT_EQ(publisher_.RecordCount(), size_t{2});
    records = publisher_.GetNonPtrRecords(domain_, type);
    ...
    EXPECT_EQ(publisher_.RecordCount(), size_t{0});
    records = publisher_.GetNonPtrRecords(domain_, type);
...
TEST_F(MdnsPublisherTest, ARecordRegistrationWorkflow) {
  MdnsRecord record1 = GetFakeARecord(domain_);
  MdnsRecord record2 = GetFakeARecord(domain_, std::chrono::seconds(1000));
  TestUniqueRecordRegistrationWorkflow(record1, record2);
}

TEST_F(MdnsPublisherTest, AAAARecordRegistrationWorkflow) {
  MdnsRecord record1 = GetFakeAAAARecord(domain_);
  MdnsRecord record2 = GetFakeAAAARecord(domain_, std::chrono::seconds(1000));
  TestUniqueRecordRegistrationWorkflow(record1, record2);
}

TEST_F(MdnsPublisherTest, TXTRecordRegistrationWorkflow) {
  MdnsRecord record1 = GetFakeTxtRecord(domain_);
  MdnsRecord record2 = GetFakeTxtRecord(domain_, std::chrono::seconds(1000));
  TestUniqueRecordRegistrationWorkflow(record1, record2);
}

TEST_F(MdnsPublisherTest, SRVRecordRegistrationWorkflow) {
  MdnsRecord record1 = GetFakeSrvRecord(domain_);
  MdnsRecord record2 = GetFakeSrvRecord(domain_, std::chrono::seconds(1000));
  TestUniqueRecordRegistrationWorkflow(record1, record2);
}

TEST_F(MdnsPublisherTest, PTRRecordRegistrationWorkflow) {
  EXPECT_CALL(publisher_, IsOwned(domain_)).WillRepeatedly(Return(true));
  MdnsRecord record1 = GetFakePtrRecord(domain_);
  MdnsRecord record2 = GetFakePtrRecord(domain_, std::chrono::seconds(1000));
