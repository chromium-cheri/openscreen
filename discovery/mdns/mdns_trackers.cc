...
MdnsRecordTracker::MdnsRecordTracker(
  ...
  if (record_.dns_type() == DnsType::kNSEC) {
    const auto& nsec_rdata = absl::get<NsecRecordRdata>(record_.rdata());
    OSP_DCHECK(std::find_if(nsec_rdata.types().begin(),
                            nsec_rdata.types().end(), [dns_type](DnsType type) {
...
ErrorOr<MdnsRecordTracker::UpdateType> MdnsRecordTracker::Update(
  ...
  if (new_is_negative_response) {
    const auto& nsec_rdata = absl::get<NsecRecordRdata>(new_record.rdata());
    if (std::find(nsec_rdata.types().begin(), nsec_rdata.types().end(),
                  dns_type_) == nsec_rdata.types().end()) {
      // The new record has been passed to a wrong tracker.
      return Error::Code::kParameterInvalid;
    }
  }
