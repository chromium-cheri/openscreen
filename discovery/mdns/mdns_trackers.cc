...
MdnsQuestionTracker::MdnsQuestionTracker(MdnsQuestion question,
  ...
  // milliseconds.
  send_alarm_.ScheduleFromNow([this] { MdnsQuestionTracker::SendQuery(false); },
...
void MdnsQuestionTracker::SendQuery(bool is_on_demand_query) {
  if (!is_on_demand_query && !is_querying_periodically) {
    return;
  }
  ...
  // NOTE: The RFC does not specify whether the current query interval
  if (!is_on_demand_query) {
