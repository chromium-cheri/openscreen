...
mStatus mDNSPlatformSendUDP(const mDNS* m,
  ...
  const absl::optional<size_t> send_result =
      (*socket_it)->SendMessage(msg, length, dest);
  if (send_result) {
    return (*send_result == static_cast<size_t>(length)) ? mStatus_NoError
                                                         : mStatus_UnknownErr;
  }
  if (openscreen::platform::IsTryAgainError(
          openscreen::platform::GetLastError())) {
