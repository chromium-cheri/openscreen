...
class MdnsProbeTests : public testing::Test {
 ...
 protected:
  std::unique_ptr<MdnsProbeImpl> CreateProbe() {
    return std::make_unique<MdnsProbeImpl>(
        &sender_, &receiver_, &random_, &task_runner_, FakeClock::now,
        &observer_, name_, CreateAddressRecord(name_, endpoint_v4_));
  }
...
TEST_F(MdnsProbeTests, TestNoCancelationFlow) {
  ...
  clock_.Advance(MdnsProbe::kDelayBetweenProbeQueries);
  EXPECT_EQ(task_runner_.delayed_tasks_.size(), size_t{1});
