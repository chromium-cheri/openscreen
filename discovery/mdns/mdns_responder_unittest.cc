...
namespace discovery {
namespace {
constexpr platform::Clock::duration kMaximumSharedRecordResponseDelayMs(120 *
                                                                        1000);
}
...
class MdnsResponderTest : public testing::Test {
 public:
  MdnsResponderTest()
      : socket_(platform::FakeUdpSocket::CreateDefault()),
        sender_(socket_.get()),
 ...
 protected:
  ...
  std::unique_ptr<platform::FakeUdpSocket> socket_;
...
TEST_F(MdnsResponderTest, OwnedRecordsSentImmediately) {
  ...
  clock_.Advance(
      platform::Clock::duration(kMaximumSharedRecordResponseDelayMs));
}
...
TEST_F(MdnsResponderTest, AnyQueryResultsAllApplied) {
  ...
  EXPECT_CALL(sender_, SendMulticast(_))
      .WillOnce(Invoke([](const MdnsMessage& message) -> Error {
        ...
        const auto& records = message.answers();
        ...
        EXPECT_NE(std::find_if(records.begin(), records.end(),
                  ...
                  records.end());
        EXPECT_EQ(std::find_if(records.begin(), records.end(),
                               [](MdnsRecord record) {
...
TEST_F(MdnsResponderTest, PtrQueryResultsApplied) {
  ...
  EXPECT_CALL(sender_, SendMulticast(_))
      .WillOnce(Invoke([](const MdnsMessage& message) -> Error {
        ...
        EXPECT_NE(
            std::find_if(message.answers().begin(), message.answers().end(),
                         [](MdnsRecord record) {
                           return record.dns_type() == DnsType::kPTR;
                         }),
            message.answers().end());
