...
std::unique_ptr<MdnsService> MdnsService::Create(
  ...
  return std::make_unique<MdnsServiceImpl>(task_runner, Clock::now, callback,
                                           network_interface);
}
...
MdnsServiceImpl::MdnsServiceImpl(TaskRunner* task_runner,
                                 ...
                                 NetworkInterfaceIndex network_interface)
    : MdnsService(fatal_error_callback),
  ...
  socket_v6_->JoinMulticastGroup(kDefaultMulticastGroupIPv6, network_interface);
  socket_v6_->JoinMulticastGroup(kDefaultSiteLocalGroupIPv6, network_interface);

  // Initialize objects which depend on the above sockets.
  sender_ = std::make_unique<MdnsSender>(socket_v4_.get());
  ...
  probe_manager_ = std::make_unique<MdnsProbeManagerImpl>(
      sender_.get(), querier_.get(), &random_delay_, task_runner_);
  ...
  OSP_LOG << "MDNS successfully initialized\n";
}
