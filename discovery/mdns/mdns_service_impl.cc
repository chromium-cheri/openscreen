...
void MdnsServiceImpl::ReceiverCallback::SetReceiver(MdnsReceiver* receiver) {
  OSP_DCHECK(!!receiver ^ !!receiver_);
  receiver_ = receiver;
}

void MdnsServiceImpl::ReceiverCallback::OnError(UdpSocket* socket,
                                                Error error) {
  OSP_DCHECK(receiver_);
  receiver_->OnError(socket, std::move(error));
}

void MdnsServiceImpl::ReceiverCallback::OnSendError(UdpSocket* socket,
                                                    Error error) {
  OSP_DCHECK(receiver_);
  receiver_->OnSendError(socket, std::move(error));
}

void MdnsServiceImpl::ReceiverCallback::OnRead(UdpSocket* socket,
...
std::unique_ptr<MdnsService> MdnsService::Create(TaskRunner* task_runner) {
  IPEndpoint endpoint;
  endpoint.port = 5353;
  auto responder = std::make_unique<MdnsServiceImpl::ReceiverCallback>();
...
MdnsServiceImpl::MdnsServiceImpl(TaskRunner* task_runner,
                                 ...
                                 std::unique_ptr<ReceiverCallback> callback,
...
Error MdnsServiceImpl::StartProbe(MdnsDomainClaimedCallback* callback,
