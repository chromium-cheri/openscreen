...
#include "discovery/common/configuration_parameters.h"
...
MdnsPublisher::MdnsPublisher(MdnsSender* sender,
    ...
    : sender_(sender),
      ...
      target_announcement_attempts_(config.new_record_announcement_count) {
  OSP_DCHECK(ownership_manager_);
  OSP_DCHECK(sender_);
  OSP_DCHECK(task_runner_);
  OSP_DCHECK(target_announcement_attempts_ >= 0);
}
