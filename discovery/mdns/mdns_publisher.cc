...
MdnsPublisher::MdnsPublisher(MdnsQuerier* querier,
  ...
  // NOTE: querier_, sender_, and random_delay_ are intentionally not validated.
  // These are provided for future use in:
  // - Probe workflow
...
Error MdnsPublisher::UpdateRegisteredRecord(const MdnsRecord& old_record,
  ...
  Error remove_result = RemoveNonPtrRecord(old_record);
  if (!remove_result.ok()) {
    return remove_result;
  }

  // Register the new record.
  RegisterNonPtrRecord(new_record);

  return Error::None();
}

size_t MdnsPublisher::RecordCount() const {
...
std::vector<MdnsRecord::ConstRef> MdnsPublisher::GetRecords(
  ...
  if (type == DnsType::kPTR || type == DnsType::kANY) {
    auto ptr_it = ptr_records_.find(name);
...
Error MdnsPublisher::UnregisterPtrRecord(const MdnsRecord& record) {
  ...
  auto map_it = records_.find(ptr_domain);
...
Error MdnsPublisher::UnregisterNonPtrRecord(const MdnsRecord& record) {
  ...
  Error remove_result = RemoveNonPtrRecord(record);
  if (!remove_result.ok()) {
    return remove_result;
  }
...
Error MdnsPublisher::RemoveNonPtrRecord(const MdnsRecord& record) {
  ...
  auto records_it = std::find(it->second.begin(), it->second.end(), record);
