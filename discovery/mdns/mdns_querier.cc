...
class DnsTypeBitSet {
 public:
  ...
  bool Insert(DnsType type) {
    ...
    OSP_DCHECK_LE(bit, 64);

    bool was_set = elements_[bit];
    elements_.set(bit);
    return !was_set;
  }

  // Attempts to remove the given type from this data structure. Returns whether
  // the operation was successful.
  bool Remove(DnsType type) {
    if (IsEmpty()) {
      return false;
    } else if (type == DnsType::kANY) {
      elements_ = std::bitset<64>();
      return true;
    }
  ...
  std::vector<DnsType> GetTypes() const {
    ...
    for (DnsType type : kSupportedDnsTypes) {
      ...
      OSP_DCHECK_LE(cast_int, 64);
      if (elements_[cast_int]) {
...
void RemoveInvalidNsecFlags(std::vector<MdnsRecord>* records) {
  ...
  // remaining iterators.
  for (auto it = nsecs_to_delete.rbegin(); it != nsecs_to_delete.rend(); it++) {
    records->erase(*it);
  }
}
