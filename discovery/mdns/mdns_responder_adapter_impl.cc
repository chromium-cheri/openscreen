...
bool EndsWithLocalDomain(const DomainName& domain) {
  ...
  return std::equal(kLocalDomain.domain_name.begin(),
                    kLocalDomain.domain_name.end(),
...
void MdnsResponderAdapterImpl::StartAQuery(const DomainName& domain_name) {
  VLOG(1) << __func__;
  ...
  // TODO: What do these append fields mean?
  ...
  // TODO: context or pull data from question?
...
void MdnsResponderAdapterImpl::StartAaaaQuery(const DomainName& domain_name) {
  VLOG(1) << __func__;
  DCHECK(EndsWithLocalDomain(domain_name));
...
void MdnsResponderAdapterImpl::StartPtrQuery(const DomainName& service_type) {
  VLOG(1) << __func__;
  ...
  if (EndsWithLocalDomain(service_type)) {
    std::copy(service_type.domain_name.begin(), service_type.domain_name.end(),
              question.qname.c);
  } else {
    DomainName::Append(service_type, kLocalDomain, question.qname.c);
  }
...
void MdnsResponderAdapterImpl::StartSrvQuery(
...
void MdnsResponderAdapterImpl::StartTxtQuery(
    const DomainName& service_instance) {
  VLOG(1) << __func__;
  DCHECK(EndsWithLocalDomain(service_instance));
...
void MdnsResponderAdapterImpl::StopAQuery(const DomainName& domain_name) {}

void MdnsResponderAdapterImpl::StopAaaaQuery(const DomainName& domain_name) {}

void MdnsResponderAdapterImpl::StopPtrQuery(const DomainName& service_type) {}

void MdnsResponderAdapterImpl::StopSrvQuery(
    const DomainName& service_instance) {}

void MdnsResponderAdapterImpl::StopTxtQuery(
    const DomainName& service_instance) {}

bool MdnsResponderAdapterImpl::RegisterService(
  ...
  std::copy(service_type.domain_name.begin(), service_type.domain_name.end(),
            type.c);
  std::copy(kLocalDomain.domain_name.begin(), kLocalDomain.domain_name.end(),
            domain.c);
  std::copy(target_host.domain_name.begin(), target_host.domain_name.end(),
            host.c);
  port.NotAnInteger = htons(target_port);
...
void MdnsResponderAdapterImpl::PtrQueryCallback(mDNS* m,
  ...
  DomainName result;
  const auto result_length = DomainNameLength(&answer->rdata->u.name);
  result.domain_name.resize(result_length);
  std::copy(answer->rdata->u.name.c, answer->rdata->u.name.c + result_length,
...
void MdnsResponderAdapterImpl::SrvQueryCallback(mDNS* m,
  ...
  DomainName service(std::vector<uint8_t>(
      ...
      question->qname.c + DomainNameLength(&question->qname)));
  DomainName result;
  const auto result_length = DomainNameLength(&answer->rdata->u.srv.target);
  result.domain_name.resize(result_length);
  std::copy(answer->rdata->u.srv.target.c,
            answer->rdata->u.srv.target.c + result_length,
            result.domain_name.begin());
  ...
  adapter->srv_responses_.emplace_back(
      ...
      ntohs(answer->rdata->u.srv.port.NotAnInteger));
}
