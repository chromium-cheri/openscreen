...
// octet.
constexpr int32_t kMaxDnsStringLength = 255;
...
bool IsValidServiceName(const std::string& service_name) {
  // Service name requirements also come from RFC 6763:
...
std::string MakeTxtData(const std::vector<std::string>& lines) {
  ...
  for (const auto& line : lines) {
    if (line.size() > kMaxDnsStringLength ||
        (txt.size() + line.size() + 1) > kMaxDnsStringLength) {
      return {};
    }
...
bool MdnsResponderAdapterImpl::RegisterInterface(
  ...
  static_assert(sizeof(info.MAC.b) == sizeof(interface_info.hardware_address),
                "Hardware address fields must have equal size for copy");
...
MdnsResponderErrorCode MdnsResponderAdapterImpl::StartAQuery(
  ...
  DCHECK(domain_name.EndsWithLocalDomain());
...
MdnsResponderErrorCode MdnsResponderAdapterImpl::StartSrvQuery(
  ...
  DCHECK(service_instance.EndsWithLocalDomain());
...
void MdnsResponderAdapterImpl::AdvertiseInterfaces() {
  for (auto& info : v4_responder_interface_info_) {
    mDNS_SetupResourceRecord(&info.second.RR_A, nullptr, mDNSInterface_Any,
                             kDNSType_A, kHostNameTTL, kDNSRecordTypeUnique,
                             AuthRecordAny, nullptr, nullptr);
    AssignDomainName(&info.second.RR_A.namestorage, &mdns_.MulticastHostname);
    info.second.RR_A.resrec.rdata->u.ipv4 = info.second.ip.ip.v4;
    mDNS_Register(&mdns_, &info.second.RR_A);
  }
  for (auto& info : v6_responder_interface_info_) {
    mDNS_SetupResourceRecord(&info.second.RR_A, nullptr, mDNSInterface_Any,
                             kDNSType_AAAA, kHostNameTTL, kDNSRecordTypeUnique,
                             AuthRecordAny, nullptr, nullptr);
    AssignDomainName(&info.second.RR_A.namestorage, &mdns_.MulticastHostname);
    info.second.RR_A.resrec.rdata->u.ipv6 = info.second.ip.ip.v6;
    mDNS_Register(&mdns_, &info.second.RR_A);
  }
}
...
void MdnsResponderAdapterImpl::DeadvertiseInterfaces() {
  for (auto& info : v4_responder_interface_info_) {
    if (info.second.RR_A.namestorage.c[0]) {
      mDNS_Deregister(&mdns_, &info.second.RR_A);
      info.second.RR_A.namestorage.c[0] = 0;
    }
  }
  for (auto& info : v6_responder_interface_info_) {
    if (info.second.RR_A.namestorage.c[0]) {
