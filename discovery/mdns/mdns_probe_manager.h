...
class MdnsOwnershipManager {
 public:
  virtual ~MdnsOwnershipManager();
  ...
  virtual bool IsProbeQuery(const MdnsMessage& message) const = 0;
...
// name.
class MdnsProbeManager : public MdnsProbe::Observer, MdnsOwnershipManager {
 public:
  class Callback {
   public:
    virtual ~Callback();
  ...
  // MdnsOwnershipManager overrides.
  bool IsDomainClaimed(const DomainName& domain) const override;
  bool IsProbeQuery(const MdnsMessage& message) const override;
