...
class MdnsProbeManager {
 public:
  ...
  // |message| is a message received from another host. It is a considered a
  // probe query for a specific domain if it contains a query for a specific
  // domain which is answered by mDNS Records in the 'authority records' section
  // of |message|. If a probe for the provided domain name is ongoing, an
  // MdnsMessage is sent to the provided endpoint as described in RFC 6762
  // section 8.2 to allow for conflict resolution. If the requested name has
  // already been claimed, a message to specify this will be sent as described
  // in RFC 6762 section 8.1. The |src| argument is the address from which the
  // message was originally sent, so that the response message may be sent as a
  // unicast response.
...
class MdnsProbeManagerImpl : public MdnsProbe::Observer,
 ...
 private:
  ...
  // Owns an in-progress MdnsProbe. An instance of this struct is created when
  // the probe is started, and deleted when the probe completes successfully.
  // When the probe fails, the instance is updated with a new MdnsProbe object,
