...
TEST(DomainNameTest, Append) {
  ...
  EXPECT_THAT(service_type.domain_name(), ContainerEq(expected_service_type));
  ...
  uint8_t buffer[256];
  EXPECT_TRUE(DomainName::Append(service_name, service_type, buffer));
  // Reversed expectation to use ElementsAreArray with limited size.
  EXPECT_THAT(
      result.domain_name(),
      ::testing::ElementsAreArray(&buffer[0], result.domain_name().size()));
}
