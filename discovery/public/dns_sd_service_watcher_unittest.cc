...
std::vector<std::string> ConvertRefs(
    std::vector<std::reference_wrapper<const std::string>> value) {
...
class MockDnsSdService : public DnsSdService {
 ...
 private:
  ...
  StrictMock<MockQuerier> querier_;
};
 ...
 public:
  ...
  TestServiceWatcher()
      : DnsSdServiceWatcher<std::string>(
            ...
            [this](std::vector<ConstRefT> ref) { Callback(ref); }) {}

  MOCK_METHOD1(Callback, void(std::vector<ConstRefT>));

  MockDnsSdService& service() { return service_; }
...
class DnsSdServiceWatcherTests : public testing::Test {
 public:
  DnsSdServiceWatcherTests() {
    // Start service discovery, since all other tests need it
    EXPECT_FALSE(watcher_.IsRunning());
    EXPECT_CALL(watcher_.service(), StartQuery(kCastServiceId, _));
    watcher_.StartDiscovery();
    testing::Mock::VerifyAndClearExpectations(&watcher_.service());
  }

 protected:
  void CreateNewInstance(const DnsSdInstanceRecord& record) {
    std::vector<std::string> services_before = ConvertRefs(watcher_.services());
    size_t count = services_before.size();

    std::vector<TestServiceWatcher::ConstRefT> callbacked_service_refs;
  ...
  bool ContainsService(const DnsSdInstanceRecord& record) {
    ...
    std::vector<TestServiceWatcher::ConstRefT> services = watcher_.services();
...
TEST_F(DnsSdServiceWatcherTests, StartStopDiscoveryWorks) {
  ...
  EXPECT_CALL(watcher_.service(), StopQuery(kCastServiceId, _));
  watcher_.StopDiscovery();
  EXPECT_FALSE(watcher_.IsRunning());
}

TEST(DnsSdServiceWatcherTest, RefreshFailsBeforeDiscoveryStarts) {
  StrictMock<TestServiceWatcher> watcher;
...
TEST_F(DnsSdServiceWatcherTests, RefreshDiscoveryWorks) {
  ...
  // Refresh services.
  EXPECT_CALL(watcher_.service(), ReinitializeQueries(kCastServiceId));
  EXPECT_TRUE(watcher_.DiscoverNow().ok());
  EXPECT_EQ(watcher_.services().size(), size_t{1});
  testing::Mock::VerifyAndClearExpectations(&watcher_.service());

  EXPECT_CALL(watcher_.service(), ReinitializeQueries(kCastServiceId));
  EXPECT_TRUE(watcher_.ForceRefresh().ok());
  EXPECT_EQ(watcher_.services().size(), size_t{0});
  testing::Mock::VerifyAndClearExpectations(&watcher_.service());
}
