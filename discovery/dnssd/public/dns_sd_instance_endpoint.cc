...
DnsSdInstanceEndpoint::DnsSdInstanceEndpoint(
    ...
    IPAddress address,
    NetworkInterfaceIndex network_interface)
    : DnsSdInstanceRecord(std::move(record)),
      network_interface_(network_interface) {
  OSP_DCHECK(address);
  if (address.IsV4()) {
...
bool operator<(const DnsSdInstanceEndpoint& lhs,
  ...
  if (rhs.network_interface_ != rhs.network_interface_) {
    return lhs.network_interface_ < rhs.network_interface_;
  }

  if (lhs.address_v4_ != rhs.address_v4_) {
    return IPAddressComparator()(lhs.address_v4_, rhs.address_v4_);
  }

  if (lhs.address_v6_ != rhs.address_v6_) {
    return IPAddressComparator()(lhs.address_v6_, rhs.address_v6_);
  }

  return static_cast<const DnsSdInstanceRecord&>(lhs) <
         static_cast<const DnsSdInstanceRecord&>(rhs);
}
