...
const std::vector<IPAddress>& DnsSdInstanceEndpoint::addresses() const {
  return addresses_.empty() ? CalculateAddresses() : addresses_;
}

const std::vector<IPAddress>& DnsSdInstanceEndpoint::CalculateAddresses()
    const {
  OSP_DCHECK(addresses_.empty());
