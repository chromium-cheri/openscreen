...
ErrorOr<absl::Span<const uint8_t>> DnsSdTxtRecord::GetValue(
  ...
  if (boolean_txt_.find(key) != boolean_txt_.end()) {
...
ErrorOr<bool> DnsSdTxtRecord::GetFlag(const std::string& key) const {
  ...
  if (key_value_txt_.find(key) != key_value_txt_.end()) {
    return Error::Code::kOperationInvalid;
  } else {
    return false;
  }
...
Error DnsSdTxtRecord::ClearValue(const std::string& key) {
  ...
  auto it = key_value_txt_.find(key);
  if (it != key_value_txt_.end()) {
    key_value_txt_.erase(it);
  } else if (boolean_txt_.find(key) != boolean_txt_.end()) {
    return Error::Code::kOperationInvalid;
  }
...
Error DnsSdTxtRecord::ClearFlag(const std::string& key) {
  ...
  auto it = boolean_txt_.find(key);
  if (it != boolean_txt_.end()) {
    boolean_txt_.erase(it);
  } else if (key_value_txt_.find(key) != key_value_txt_.end()) {
    return Error::Code::kOperationInvalid;
  }
