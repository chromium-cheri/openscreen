...
struct DnsData {
  ...
  absl::optional<cast::mdns::ARecordRdata> address_v4;
  absl::optional<cast::mdns::AAAARecordRdata> address_v6;
};

// Hashing functions to allow for using with absl::Hash<...>.
template <typename H>
H AbslHashValue(H h, const PtrKey& key) {
  return H::combine(std::move(h), key.service_id, key.domain_id);
}
template <typename H>
H AbslHashValue(H h, const SrvKey& key) {
  return H::combine(std::move(h), key.service_id, key.domain_id);
}

// Equality operators to allow for comparison of Key types.
inline bool operator==(const PtrKey& lhs, const PtrKey& rhs) {
  return lhs.service_id == rhs.service_id && lhs.domain_id == rhs.domain_id;
}
inline bool operator!=(const PtrKey& lhs, const PtrKey& rhs) {
  return !(lhs == rhs);
}
inline bool operator==(const SrvKey& lhs, const SrvKey& rhs) {
...
// Comparison between PtrKey and SrvKey types.
inline bool IsInstanceOf(const PtrKey& ptr, const SrvKey& srv) {
