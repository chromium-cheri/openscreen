...
Error PublisherImpl::UpdateRegistration(const DnsSdInstanceRecord& record) {
  ...
  InstanceKey key(record);
  auto it = std::find_if(pending_records_.begin(), pending_records_.end(),
...
Error PublisherImpl::UpdatePublishedRegistration(
    const DnsSdInstanceRecord& record) {
  InstanceKey changed_key(record);
  auto it = std::find_if(
      published_records_.begin(), published_records_.end(),
      [&changed_key](
          const std::pair<DnsSdInstanceRecord, DnsSdInstanceRecord>& other) {
        return changed_key == InstanceKey(other.first);
      });
  ...
  if (it == published_records_.end() || it->first == record) {
  ...
  std::map<DnsType,
      ...
      changed_records;
  std::vector<MdnsRecord> old_records = GetDnsRecords(it->second);
  DnsSdInstanceRecord updated_record =
      UpdateDomain(GetDomainName(InstanceKey(it->second)), record);
  std::vector<MdnsRecord> new_records = GetDnsRecords(updated_record);
...
void PublisherImpl::OnDomainFound(const DomainName& requested_name,
  ...
  auto it = std::find_if(pending_records_.begin(), pending_records_.end(),
                         [&requested_name](const DnsSdInstanceRecord& record) {
                           return requested_name ==
                                  GetDomainName(InstanceKey(record));
                         });
  ...
  if (requested_name != confirmed_name) {
    if (!HasValidDnsRecordAddress(confirmed_name)) {
      // This means we received an invalid record from our mDNS implementation's
      // probe phase. It shouldn't happen, but check for it just in case.
      return;
    }
