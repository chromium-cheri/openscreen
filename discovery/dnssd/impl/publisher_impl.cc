...
Error PublisherImpl::UpdatePublishedRegistration(
    const DnsSdInstanceRecord& record) {
  const InstanceKey changed_key(record);
  ...
  std::map<DnsType,
      ...
      changed_records;
  const std::vector<MdnsRecord> old_records = GetDnsRecords(it->second);
  const DnsSdInstanceRecord updated_record =
...
void PublisherImpl::OnDomainFound(const DomainName& requested_name,
  ...
  const auto it = std::find_if(
      ...
      [&requested_name](const DnsSdInstanceRecord& record) {
        return requested_name == GetDomainName(InstanceKey(record));
      });
