...
TEST(DnsSdConversionLayerTest, TestApplyRecordChanges) {
  cast::mdns::MdnsRecord record = CreateFullyPopulatedRecord(port_num);
  DnsData data{};
  EXPECT_TRUE(ApplyDataRecordChange(&data, record,
                                    cast::mdns::RecordChangedEvent::kCreated)
                  .ok());
  ASSERT_TRUE(data.srv.has_value());
  EXPECT_EQ(data.srv.value().port(), port_num);

  record = CreateFullyPopulatedRecord(234);
  EXPECT_FALSE(ApplyDataRecordChange(&data, record,
                                     cast::mdns::RecordChangedEvent::kCreated)
                   .ok());
  ASSERT_TRUE(data.srv.has_value());
  ...
  EXPECT_TRUE(ApplyDataRecordChange(&data, record,
                                    cast::mdns::RecordChangedEvent::kUpdated)
                  .ok());
  ASSERT_TRUE(data.srv.has_value());
  ...
  EXPECT_TRUE(ApplyDataRecordChange(&data, record,
                                    cast::mdns::RecordChangedEvent::kDeleted)
                  .ok());
