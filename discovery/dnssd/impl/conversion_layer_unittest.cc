...
TEST(DnsSdConversionLayerTest, GetDnsRecordsPtr) {
  ...
  std::vector<cast::mdns::MdnsRecord> records = GetDnsRecords(instance_record);
  auto it = std::find_if(
      records.begin(), records.end(), [](const cast::mdns::MdnsRecord& record) {
        return record.dns_type() == cast::mdns::DnsType::kPTR;
      });
  ASSERT_NE(it, records.end());

  EXPECT_EQ(it->dns_class(), cast::mdns::DnsClass::kIN);
  EXPECT_EQ(it->record_type(), cast::mdns::RecordType::kShared);
  ...
  const auto& rdata = absl::get<cast::mdns::PtrRecordRdata>(it->rdata());
...
TEST(DnsSdConversionLayerTest, GetDnsRecordsSrv) {
  ...
  auto it = std::find_if(
      records.begin(), records.end(), [](const cast::mdns::MdnsRecord& record) {
        return record.dns_type() == cast::mdns::DnsType::kSRV;
      });
  ASSERT_NE(it, records.end());

  EXPECT_EQ(it->dns_class(), cast::mdns::DnsClass::kIN);
  EXPECT_EQ(it->record_type(), cast::mdns::RecordType::kUnique);
  ...
  const auto& rdata = absl::get<cast::mdns::SrvRecordRdata>(it->rdata());
...
TEST(DnsSdConversionLayerTest, GetDnsRecordsAPresent) {
  ...
  auto it = std::find_if(records.begin(), records.end(),
                         [](const cast::mdns::MdnsRecord& record) {
                           return record.dns_type() == cast::mdns::DnsType::kA;
                         });
  ASSERT_NE(it, records.end());
  ...
  const auto& rdata = absl::get<cast::mdns::ARecordRdata>(it->rdata());
...
TEST(DnsSdConversionLayerTest, GetDnsRecordsAAAAPresent) {
  ...
  auto it = std::find_if(
      records.begin(), records.end(), [](const cast::mdns::MdnsRecord& record) {
        return record.dns_type() == cast::mdns::DnsType::kAAAA;
      });
  ASSERT_NE(it, records.end());
  ...
  const auto& rdata = absl::get<cast::mdns::AAAARecordRdata>(it->rdata());
...
TEST(DnsSdConversionLayerTest, GetDnsRecordsTxt) {
  ...
  auto it = std::find_if(
      records.begin(), records.end(), [](const cast::mdns::MdnsRecord& record) {
        return record.dns_type() == cast::mdns::DnsType::kTXT;
      });
  ASSERT_NE(it, records.end());
  ...
  const auto& rdata = absl::get<cast::mdns::TxtRecordRdata>(it->rdata());
