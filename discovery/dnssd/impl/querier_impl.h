...
class QuerierImpl : public DnsSdQuerier, public MdnsRecordChangedCallback {
 public:
  ...
  inline bool IsQueryOngoing(const absl::string_view& service,
    ...
    return IsQueryOngoing(GetServiceKey(service, domain));
  }
 ...
 private:
  ...
  inline bool IsQueryOngoing(const ServiceKey& key) const {
  ...
  // callbacks to call when new InstanceRecords are ready.
