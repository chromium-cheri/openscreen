...
class MockClient : public DnsSdPublisher::Client {
 public:
  MOCK_METHOD2(OnInstanceClaimed,
               void(const DnsSdInstanceRecord&, const DnsSdInstanceEndpoint&));
};
...
TEST_F(PublisherImplTest, TestRegistrationAndDegrestration) {
  ...
  EXPECT_CALL(client, OnInstanceClaimed(record, _))
      .WillOnce([record2](const DnsSdInstanceRecord& requested,
                          const DnsSdInstanceEndpoint& claimed) {
        EXPECT_EQ(record2, claimed);
      });
...
TEST_F(PublisherImplTest, TestUpdate) {
  ...
  EXPECT_CALL(client, OnInstanceClaimed(record2, _))
      .WillOnce([record2](const DnsSdInstanceRecord& requested,
                          const DnsSdInstanceEndpoint& claimed) {
        EXPECT_EQ(record2, claimed);
      });
