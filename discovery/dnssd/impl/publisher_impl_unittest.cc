...
class MockClient : public DnsSdPublisher::Client {
 public:
  MOCK_METHOD2(OnInstanceIdClaimed,
...
TEST(DnsSdPublisherImplTests, TestRegistrationAndDegrestration) {
  ...
  EXPECT_CALL(client, OnInstanceIdClaimed(record, record2)).Times(1);
...
TEST(DnsSdPublisherImplTests, TestUpdate) {
  ...
  EXPECT_CALL(client, OnInstanceIdClaimed(record2, record2)).Times(1);
