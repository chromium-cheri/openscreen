...
Error QuerierImpl::HandleNonPtrRecordChange(MdnsRecord record,
  ...
  if (it == received_records_.end()) {
    if (event != RecordChangedEvent::kCreated) {
      // This can happen if records are received out of order due to UDP
      // networking weirdness.
      OSP_VLOG << "Received erroneous record change.";
      return Error::Code::kOperationInvalid;
    }
  ...
  if (!apply_result.ok()) {
    // This can happen if records are received out of order due to UDP
    // networking weirdness.
    OSP_VLOG << "Received erroneous record change. Error: " << apply_result;
    return apply_result;
  }
