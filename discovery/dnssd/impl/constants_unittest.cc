...
TEST(DnsSdConstantsTest, PtrKeyInMap) {
  PtrKey ptr{"service", "domain"};
  PtrKey ptr2{"service", "domain"};
  PtrKey ptr3{"service2", "domain"};
  PtrKey ptr4{"service", "domain2"};
  std::unordered_map<PtrKey, int32_t, absl::Hash<PtrKey>> map;
  map[ptr] = 1;
  map[ptr2] = 2;
  EXPECT_EQ(map[ptr], 2);
