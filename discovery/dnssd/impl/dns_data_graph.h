...
/*
 ...
 - The memory requirement for this graph is bounded due to a bound on the
   number of tracked records in the mDNS layer.
*/
class DnsDataGraph {
 public:
  ...
  // Creates all valid DnsSdInstanceEndpoint objects with |name| associated with
  // the provided |domain_group|.
  ErrorOr<std::vector<DnsSdInstanceEndpoint>> CreateEndpoints(
  ...
  // Returns whether the provided domain is tracked. This may be because
  // StartTracking() was manually called or due to the result of an
  // ApplyDataRecordChange() call.
  bool IsTracked(const DomainName& domain) const;
