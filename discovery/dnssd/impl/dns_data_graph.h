...
// TODO(rwkeane): Move to the .cc once this CL gets in.
class DnsDataGraphImpl : public DnsDataGraph {
 ...
 private:
  ...
  // Sets the action to take on node creation and deletion.
  std::unique_ptr<NodeLifetimeHandler> SetCreationCallback(
      DomainChangeCallback callback);
  std::unique_ptr<NodeLifetimeHandler> SetDeletionCallback(
      DomainChangeCallback callback);
