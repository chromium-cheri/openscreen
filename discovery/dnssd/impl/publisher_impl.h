...
class PublisherImpl : public DnsSdPublisher {
 ...
 private:
  PublisherImpl(
      MdnsService* publisher,
      std::function<std::vector<MdnsRecord>(const DnsSdInstanceRecord&)>
          GetDnsRecordsFunction);

  std::vector<DnsSdInstanceRecord> published_records_;

  MdnsService* const mdns_publisher_;

  const std::function<std::vector<MdnsRecord>(const DnsSdInstanceRecord&)>
