...
// Set the callback to be used for creation and deletion of a node.
...
void DnsDataGraphImpl::StartTracking(const DomainName& domain,
  ...
  SET_CREATION_CALLBACK(std::move(on_start_tracking));
...
void DnsDataGraphImpl::StopTracking(const DomainName& domain,
  ...
  SET_DELETION_CALLBACK(std::move(on_stop_tracking));
...
#undef SET_CREATION_CALLBACK
#undef SET_DELETION_CALLBACK
