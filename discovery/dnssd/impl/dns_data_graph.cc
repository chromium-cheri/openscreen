...
DnsDataGraphImpl::NodeLifetimeHandler::NodeLifetimeHandler(
  ...
  *callback_ptr = [this](const DomainName& domain) {
...
std::unique_ptr<DnsDataGraphImpl::NodeLifetimeHandler>
DnsDataGraphImpl::SetCreationCallback(DomainChangeCallback callback) {
  return std::make_unique<NodeLifetimeHandler>(&on_node_creation_,
                                               std::move(callback));
}
std::unique_ptr<DnsDataGraphImpl::NodeLifetimeHandler>
DnsDataGraphImpl::SetDeletionCallback(DomainChangeCallback callback) {
...
void DnsDataGraphImpl::StartTracking(const DomainName& domain,
  ...
  auto creation_handler = SetCreationCallback(std::move(on_start_tracking));
...
void DnsDataGraphImpl::StopTracking(const DomainName& domain,
  ...
  auto deletion_handler = SetDeletionCallback(std::move(on_stop_tracking));
