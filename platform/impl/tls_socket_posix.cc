...
ErrorOr<int> CreateNonBlockingTlsSocket(int domain) {
...
TlsSocketPosix::TlsSocketPosix(int fd, const IPEndpoint& local_endpoint)
    : fd_(fd), local_endpoint_(local_endpoint) {
  OSP_DCHECK(local_endpoint_.address.IsV4() || local_endpoint_.address.IsV6());
}

TlsSocketPosix::~TlsSocketPosix() {
  close(fd_);
}

// static
ErrorOr<TlsSocketUniquePtr> TlsSocket::Create(const IPEndpoint& endpoint) {
  int domain;
  ...
  const ErrorOr<int> fd = CreateNonBlockingTlsSocket(domain);
  if (!fd) {
    return fd.error();
  }
  return TlsSocketUniquePtr(
      static_cast<TlsSocket*>(new TlsSocketPosix(fd.value(), endpoint)));
}
...
IPEndpoint TlsSocketPosix::GetLocalEndpoint() const {
...
Error TlsSocketPosix::SetMulticastOutboundInterface(
...
Error TlsSocketPosix::JoinMulticastGroup(const IPAddress& address,
                                         NetworkInterfaceIndex ifindex) {
  switch (local_endpoint_.address.version()) {
    case TlsSocket::Version::kV4: {
...
Error ReceiveMessageInternal(int fd, TlsPacket* packet) {
  SockAddrType sa;
...
ErrorOr<TlsPacket> TlsSocketPosix::ReceiveMessage() {
  ssize_t bytes_available = recv(fd_, nullptr, 0, MSG_PEEK | MSG_TRUNC);
  if (bytes_available == -1) {
    return ChooseError(errno, Error::Code::kSocketReadFailure);
  }
  TlsPacket packet(bytes_available);
  ...
  return result.ok() ? ErrorOr<TlsPacket>(std::move(packet))
                     : ErrorOr<TlsPacket>(std::move(result));
}

// TODO(yakimakha): Consider changing the interface to accept TlsPacket as
// an input parameter.
Error TlsSocketPosix::SendMessage(const void* data,
                                  size_t length,
  ...
  // Sanity-check: TLS datagram sendmsg() is all or nothing.
  OSP_DCHECK_EQ(static_cast<size_t>(num_bytes_sent), length);
  return Error::Code::kNone;
}

Error TlsSocketPosix::SetDscp(TlsSocket::DscpMode state) {
