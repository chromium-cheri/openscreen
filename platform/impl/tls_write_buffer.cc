...
size_t TlsWriteBuffer::Write(const void* data, size_t len) {
  ...
  size_t write_len = std::min(kBufferSizeBytes - current_size - 1, len);
  ...
  size_t new_write_index = (current_write_index + write_len) % kBufferSizeBytes;
...
void TlsWriteBuffer::Consume(size_t byte_count) {
  ...
  OSP_CHECK(GetCurrentFillCount(current_write_index, current_read_index) >=
            byte_count);
  size_t new_read_index = (current_read_index + byte_count) % kBufferSizeBytes;
