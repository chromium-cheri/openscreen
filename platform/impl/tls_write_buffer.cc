...
size_t TlsWriteBuffer::Write(const void* data, size_t len) {
  ...
  const size_t current_read_index = read_index_.load(std::memory_order_acquire);
  ...
  const size_t write_len =
      std::min(kBufferSizeBytes - bytes_currently_used - 1, len);
...
absl::Span<const uint8_t> TlsWriteBuffer::GetReadableRegion() {
  ...
  const size_t current_read_index = read_index_.load(std::memory_order_relaxed);
