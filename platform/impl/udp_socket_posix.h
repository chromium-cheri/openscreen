...
class UdpSocketPosix : public UdpSocket {
 ...
 private:
  // Helpers to invoke the corresponding Client::OnXYZ() method, if a client has
  // been provided (else no-op).
  ...
  void Close();
  void CloseIfErrorIsFatal(const Error& error);
  ...
  // This must be locked while either: 1) The handle is being closed; 2) The
  // handle is in-use by a thread other than the one associated with the
  // TaskRunner.
