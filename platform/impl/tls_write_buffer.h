...
class TlsWriteBuffer {
 ...
 private:
  ...
  // Index of the current read location in the buffer. Atomics are used both to
  ...
  // NOTE: These values always increment, but in practice we never expect them
  // to wrap around to zero, so this is not accounted for.
