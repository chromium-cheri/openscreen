...
class RuntimeContextPosix : public RuntimeContext {
 public:
  RuntimeContextPosix(ClockNowFunctionPtr clock_func);
  ...
  void OnCreate(TlsConnection* connection) override {
    tls_data_router_posix_.RegisterConnection(
        static_cast<TlsConnectionPosix*>(connection));
  }

  void OnCreate(TlsConnectionFactory* factory) override {
    static_cast<TlsConnectionFactoryPosix*>(factory)->SetStreamListener(
        &tls_data_router_posix_);
  }
  ...
  void OnDestroy(TlsConnectionFactory* factory) override {}
