...
std::vector<InterfaceInfo> ProcessInterfacesList(ifaddrs* interfaces) {
  ...
  const ScopedFd ioctl_socket4(socket(AF_INET, SOCK_DGRAM, 0));
  const ScopedFd ioctl_socket6(socket(AF_INET6, SOCK_DGRAM, 0));
  ...
  for (ifaddrs* cur = interfaces; cur; cur = cur->ifa_next) {
    ...
    if (it == results.end()) {
      ...
      if (ioctl(ioctl_socket4.get(), SIOCGIFMEDIA, &ifmr) < 0 &&
          ioctl(ioctl_socket6.get(), SIOCGIFMEDIA, &ifmr) < 0) {
        continue;
      }
