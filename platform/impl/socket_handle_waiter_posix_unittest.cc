...
TEST(SocketHandleWaiterTest, WatchedSocketsReturnedToCorrectSubscribers) {
  ...
  EXPECT_CALL(subscriber, ProcessReadyHandle(std::cref(handle0_ref), 1))
      .Times(1);
  EXPECT_CALL(subscriber, ProcessReadyHandle(std::cref(handle2_ref), 2))
      .Times(1);
  EXPECT_CALL(subscriber2, ProcessReadyHandle(std::cref(handle1_ref), 1))
      .Times(1);
  EXPECT_CALL(subscriber2, ProcessReadyHandle(std::cref(handle3_ref), 3))
      .Times(1);
  EXPECT_CALL(waiter, AwaitSocketsReadable(_, _))
      .WillOnce(
          Return(ByMove(std::vector<TestingSocketHandleWaiter::ReadyHandle>{
              {std::cref(handle0_ref), 1},
              {std::cref(handle1_ref), 1},
              {std::cref(handle2_ref), 2},
              {std::cref(handle3_ref), 3}})));
  waiter.ProcessHandles(Clock::duration{0});
}
