...
void TlsDataRouterPosix::RemoveMappedSocket(StreamSocketPosix* socket) {
  std::unique_lock<std::mutex> lock(socket_mutex_);
...
bool TlsDataRouterPosix::IsSocketMapped(StreamSocketPosix* socket) const {
  std::unique_lock<std::mutex> lock(socket_mutex_);
