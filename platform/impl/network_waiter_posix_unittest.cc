...
class MockSubscriber : public NetworkWaiter::Subscriber {
 public:
  ...
  MOCK_METHOD0(GetHandles, std::vector<SocketHandleRef>());
...
TEST(NetworkWaiterTest, WatchedSocketsReturnedToCorrectSubscribers) {
  ...
  EXPECT_CALL(subscriber, GetHandles())
      .WillOnce(Return(ByMove(std::vector<NetworkWaiter::SocketHandleRef>{
          std::cref(handle0_ref), std::cref(handle2_ref)})));
  EXPECT_CALL(subscriber2, GetHandles())
      .WillOnce(Return(ByMove(std::vector<NetworkWaiter::SocketHandleRef>{
          std::cref(handle1_ref), std::cref(handle3_ref)})));
