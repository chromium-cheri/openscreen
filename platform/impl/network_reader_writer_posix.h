...
class NetworkReaderWriterPosix {
 public:
  ...
  class Provider {
   public:
    ...
    virtual void PerformNetworkingOperation() = 0;
  };
 ...
 private:
  ...
  // to do so arises.
  std::vector<std::tuple<Provider*, ProviderOperation>> providers_changes_
      GUARDED_BY(mutex_);
