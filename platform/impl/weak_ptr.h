...
// Weak pointers that are similar [but not identical] to Chromium's
// base::WeakPtr/WeakPtrFactory, in terms of behavior and usage. The
// implementation here is a thin wrapper around std::weak_ptr, for
// openscreen::WeakPtr; and a thin wrapper around std::shared_ptr, for
// openscreen::WeakPtrFactory.
//
// The following is copy-and-paste from Chromium's WeakPtr documentation, with
// small modifications made for clarity/relevance to Open Screen's concerns.
//
...
//
// WeakPtrs may be created, duplicated, and passed on any thread/sequence, but
// may only be dereferenced on the same thread/sequence that will ultimately
// execute the WeakPtrFactory destructor (or calls its InvalidateWeakPtrs()
// method). Otherwise, accessing anything through the WeakPtr would be racey,
// often with disasterous consequences (e.g., use-after-free).
//
// Difference from Chromium's base::WeakPtrFactory: In Chromium, there is an
// additional restriction where its WeakPtrFactory must be bound to one
// thread/sequence. This means that its WeakPtrs must be created on the same
// thread/sequence that will dereference them. Open Screen does not require this
// because openscreen::WeakPtr[Factory] is backed by std::shared_ptr<>, which
// provides thread-safe bookeeping.
...
class WeakPtrFactory {
 ...
 private:
  ...
  WeakPtrFactory(WeakPtrFactory&& other) noexcept = delete;
  WeakPtrFactory& operator=(WeakPtrFactory&& other) noexcept = delete;
