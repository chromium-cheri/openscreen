...
StreamSocketPosix::StreamSocketPosix(const IPEndpoint& local_endpoint)
  : address_(std::make_unique<SocketAddressPosix>(local_endpoint)) {
  int domain;
...
ErrorOr<StreamSocketPosix> StreamSocketPosix::Accept(const IPEndpoint& peer_endpoint) {
  if (!in_valid_state_) {
  ...
  const int file_descriptor = accept(file_descriptor_, peer_address.address(), &peer_address_size);
  ...
  return ErrorOr<StreamSocketPosix>(StreamSocketPosix(file_descriptor_, *address_));
}
...
StreamSocketPosix::StreamSocketPosix(int file_descriptor, SocketAddressPosix address)
  : file_descriptor_(file_descriptor) {
    address_ = std::make_unique<SocketAddressPosix>(address);
}
}  // namespace platform
}  // namespace openscreen
