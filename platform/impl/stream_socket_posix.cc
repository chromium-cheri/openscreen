...
Error StreamSocketPosix::Bind() {
  ...
  if (bind(handle.fd, local_address_.value().address(),
...
Error StreamSocketPosix::Close() {
  ...
  const int file_descriptor_to_close = handle.fd;
...
Error StreamSocketPosix::Connect(const IPEndpoint& remote_endpoint) {
  ...
  if (connect(handle.fd, address.address(), address.size()) != 0) {
  ...
  if (!is_bound_) {
    ...
    if (getsockname(handle.fd, &address, &size) != 0) {
      return CloseOnError(Error::Code::kSocketConnectFailure);
    }
...
Error StreamSocketPosix::Initialize() {
  ...
  int file_descriptor = socket(domain, SOCK_STREAM, 0);
