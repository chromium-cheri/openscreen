...
Error StreamSocketPosix::Bind() {
  ...
  // Socket should be in unbound state, ready to be be bound.
  if (state_ != SocketState::kUnbound) {
...
int64_t StreamSocketPosix::file_descriptor() const {
...
bool StreamSocketPosix::EnsureInitialized() {
  if ((state_ == SocketState::kUninitialized) &&
