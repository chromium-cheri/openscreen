...
class ReadCallbackExecutor {
 public:
  ReadCallbackExecutor(UdpPacket data, NetworkReader::Callback function)
      : function_(function) {
    data_ = std::move(data);
  }
...
Error NetworkReader::WaitAndRead(Clock::duration timeout) {
  ...
  Error error = Error::None();
  {
    ...
    for (UdpSocket* read : changed_or_error.value()) {
      ...
      ErrorOr<UdpPacket> read_packet = ReadFromSocket(mapped_socket->first);
...
ErrorOr<UdpPacket> NetworkReader::ReadFromSocket(UdpSocket* socket) {
