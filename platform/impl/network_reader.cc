...
Error NetworkReader::WaitAndRead(Clock::duration timeout) {
  ...
  Error error = Error::None();
  {
    ...
    for (UdpSocket* read : changed_or_error.value()) {
      ...
      // Capture UdpPacket by move into |arg| here to transfer the ownership.
      // This effectively move constructs a UdpPacket inside of the lambda.
      // Then UdpPacket |arg| is passed by move to the callback function |func|
