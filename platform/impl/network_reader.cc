...
Error NetworkReader::WaitAndRead(Clock::duration timeout) {
  ...
  // Process the results.
  socket_deletion_block_.notify_all();
  const std::vector<SocketHandle>& changed_fds = changed_or_error.value();
  if (changed_fds.empty()) {
    return Error::None();
  }

  {
    ...
    for (UdpSocket* socket : sockets_) {
      UdpSocketPosix* read_socket = static_cast<UdpSocketPosix*>(socket);
      if (std::find(changed_fds.begin(), changed_fds.end(),
                    SocketHandle(read_socket->GetFd())) != changed_fds.end()) {
