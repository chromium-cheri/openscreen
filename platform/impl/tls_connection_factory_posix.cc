...
void TlsConnectionFactoryPosix::OnConnectionPending(StreamSocketPosix* socket) {
  // WARNING: This method is being called on an unknown thread. See comments in
  // header file.

  // ASSUMPTION: It is safe to call Accept() because the caller of this method
  // is blocking all other threads that might call into |socket|'s methods.
  ErrorOr<std::unique_ptr<StreamSocket>> accepted = socket->Accept();
  if (accepted.is_error()) {
    DispatchError(accepted.error());
    return;
  }

  task_runner_->PostTask(
      [weak_ptr = weak_factory_.GetWeakPtr(),
       accepted_socket = std::move(accepted.value())]() mutable {
        if (auto* self = weak_ptr.get()) {
          self->OnSocketAccepted(std::move(accepted_socket));
        }
      });
}
