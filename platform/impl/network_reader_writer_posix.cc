...
constexpr auto kMinNetworkingOperationsTime = std::chrono::milliseconds(10);
...
void NetworkReaderWriterPosix::DeregisterProvider(Provider* provider) {
...
void NetworkReaderWriterPosix::Wait() {
  ...
  // Wait if necissary to ensure that this method runs for a "nontrivial" amount
  // of time, so that the Register / Deregister methods have enough time to
  // acquire the lock.
  const auto runtime = start_time - Clock::now();
  const auto remaining_time = runtime - kMinNetworkingOperationsTime;
  if (remaining_time > Clock::duration(0)) {
