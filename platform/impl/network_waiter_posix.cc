...
ErrorOr<std::vector<FileDescriptor>> NetworkWaiterPosix::AwaitSocketsReadable(
    const std::vector<FileDescriptor>& socket_fds,
    const Clock::duration& timeout) {
  int max_fd = -1;
  FD_ZERO(&read_handles_);
  for (const FileDescriptor& descriptor : socket_fds) {
  ...
  std::vector<FileDescriptor> changed_fds;
  for (const FileDescriptor& descriptor : socket_fds) {
