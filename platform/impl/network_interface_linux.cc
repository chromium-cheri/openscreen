...
std::vector<InterfaceInfo> GetLinkInfo(InterfaceType interfaces) {
  ...
  std::vector<InterfaceInfo> info_list;
  {
    ...
    while (!done) {
      ...
      for (netlink_header = reinterpret_cast<struct nlmsghdr*>(buf);
        ...
        if ((isLoopback && (interfaces & InterfaceType::kLoopback)) ||
            (!isLoopback && (interfaces & InterfaceType::kNonLoopback))) {
