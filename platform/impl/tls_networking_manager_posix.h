...
class TlsNetworkingManagerPosix : public NetworkWaiter::Subscriber {
 public:
  ...
  ~TlsNetworkingManagerPosix() override;
  ...
  void RegisterListener(StreamSocketPosix* socket, SocketObserver* client);

  void DeregisterListener(StreamSocketPosix* socket);
 ...
 protected:
  bool IsSocketMapped(StreamSocketPosix* socket) const;

 private:
  void OnSocketAccepted(StreamSocketPosix* socket);
  ...
  friend class TestingNetworkManager;
};
