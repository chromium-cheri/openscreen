...
void UdpSocketPosix::Bind() {
  std::function<std::string()> error_string = [this]() {
    std::stringstream stream;
    stream << "endpoint: " << this->local_endpoint_
  ...
  if (setsockopt(fd_, SOL_SOCKET, SO_REUSEADDR, &reuse_addr,
                 sizeof(reuse_addr)) == -1) {
    OnError(Error(Error::Code::kSocketOptionSettingFailure, error_string()));
  }
  ...
  switch (local_endpoint_.address.version()) {
    case UdpSocket::Version::kV4: {
      ...
      if (bind(fd_, reinterpret_cast<struct sockaddr*>(&address),
               sizeof(address)) == -1) {
        OnError(Error(Error::Code::kSocketBindFailure, error_string()));
      }
      return;
    }
