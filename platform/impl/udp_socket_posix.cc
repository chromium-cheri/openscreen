...
ErrorOr<UdpReadCallback::Packet> UdpSocketPosix::ReceiveMessage() {
  int available_bytes = 0;
  if (ioctl(fd_, FIONREAD, &available_bytes) == -1) {
    return ChooseError(errno, Error::Code::kSocketReadFailure);
  }

  UdpReadCallback::Packet packet(available_bytes);
  packet.socket = this;

  struct msghdr msg;
  struct iovec iov = {packet.data(), packet.size()};
  msg.msg_iov = &iov;
  msg.msg_iovlen = 1;
  alignas(alignof(struct cmsghdr)) uint8_t control_buffer[1024];
    ...
    case UdpSocket::Version::kV4: {
      ...
      if ((msg.msg_flags & MSG_CTRUNC) == 0) {
        for (struct cmsghdr* cmh = CMSG_FIRSTHDR(&msg); cmh;
          ...
          // reached.
          packet.original_destination.port = ntohs(addr.sin_port);
    ...
    case UdpSocket::Version::kV6: {
      ...
      if ((msg.msg_flags & MSG_CTRUNC) == 0) {
        for (struct cmsghdr* cmh = CMSG_FIRSTHDR(&msg); cmh;
          ...
          // |original_destination->port| will be 0 if this line isn't
          // reached.
          packet.original_destination.port = ntohs(addr.sin6_port);
