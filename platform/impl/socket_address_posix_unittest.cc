...
TEST(SocketAddressPosixTest, IPv4SocketAddressConvertsSuccessfully) {
  ...
  // sin_addr is a special case. The only guaranteed field is s_addr, which
  // is a single integer storing the address in network byte order. This address
  // is equivalent to 10.0.0.1, although there is no obvious relationship.
  EXPECT_EQ(v4_address->sin_addr.s_addr, 16777226u);
