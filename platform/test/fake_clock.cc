...
FakeClock::FakeClock(Clock::time_point start_time)
  ...
  OSP_CHECK_EQ(now_.load(std::memory_order_acquire), Clock::time_point::min())
...
FakeClock::~FakeClock() {
  ...
  now_.store(Clock::time_point::min(), std::memory_order_release);
}

Clock::time_point FakeClock::now() noexcept {
  const Clock::time_point value = now_.load(std::memory_order_acquire);
  OSP_CHECK_NE(value, Clock::time_point::min()) << "No FakeClock instance!";
  return value;
}
...
// static
std::atomic<Clock::time_point> FakeClock::now_{Clock::time_point::min()};
