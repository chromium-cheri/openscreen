...
bool SocketHandlerPosix::IsChanged(const UdpSocket& socket) const {
  const auto& posix_socket = UdpSocketPosix::From(socket);
...
// static
std::unique_ptr<EventWaiter::SocketHandler>
EventWaiter::SocketHandler::Create() {
  return std::unique_ptr<EventWaiter::SocketHandler>(new SocketHandlerPosix());
}
...
Error EventWaiterPosix::WaitForEvents(Clock::duration timeout,
  ...
  OSP_CHECK(reads) << "SocketHandler reads == nullptr";
  OSP_CHECK(writes) << "SocketHandler writes == nullptr";
