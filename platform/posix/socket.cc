...
ErrorOr<SocketMessage> Socket::ReceiveMessage() {
  auto* const socket = SocketPosix::From(this);
...
Error Socket::SendMessage(const SocketMessage& message) {
  auto* const socket = SocketPosix::From(this);

  struct iovec iov = {const_cast<void*>(message.data), message.length};
  struct msghdr msg;
  msg.msg_iov = &iov;
  msg.msg_iovlen = 1;
  ...
  switch (socket->version) {
    case Socket::Version::kV4: {
      struct sockaddr_in sa = {
          .sin_family = AF_INET,
          .sin_port = htons(message.dest.port),
      };
      message.dest.address.CopyToV4(reinterpret_cast<uint8_t*>(&sa.sin_addr.s_addr));
      msg.msg_name = &sa;
      msg.msg_namelen = sizeof(sa);
    ...
    case Socket::Version::kV6: {
      ...
      sa.sin6_flowinfo = 0;
      sa.sin6_scope_id = 0;
      sa.sin6_port = htons(message.dest.port);
      message.dest.address.CopyToV6(reinterpret_cast<uint8_t*>(&sa.sin6_addr.s6_addr));
      msg.msg_name = &sa;
      msg.msg_namelen = sizeof(sa);
      num_bytes_sent = sendmsg(socket->fd, &msg, 0);
      break;
