...
Error UdpSocket::SetDscp(UdpSocket::DscpMode state) {
  ...
  ErrorOr<uint8_t> dscp_socket_code = UdpSocketPosix::ToConstant(state);
...
// static
ErrorOr<uint8_t> UdpSocketPosix::ToConstant(UdpSocket::DscpMode state) {
  switch (state) {
    case UdpSocket::DscpMode::kUnspecified:
      return static_cast<uint8_t>(IPTOS_LOWDELAY);
    case UdpSocket::DscpMode::kAudioOnly:
      return static_cast<uint8_t>(IPTOS_LOWDELAY);
    case UdpSocket::DscpMode::kAudioVideo:
      return static_cast<uint8_t>(IPTOS_THROUGHPUT);
    case UdpSocket::DscpMode::kLowPriority:
      return static_cast<uint8_t>(IPTOS_MINCOST);
    default:
      return Error::Code::kSocketOptionSettingFailure;
  }
}
