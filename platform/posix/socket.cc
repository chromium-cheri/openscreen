...
#include "third_party/abseil/src/abseil-cpp/absl/types/optional.h"
...
absl::optional<int> ReceiveUdp(UdpSocketPtr socket,
                               void* data,
                               int64_t length,
                               IPEndpoint* src,
...
absl::optional<int> SendUdp(UdpSocketPtr socket,
                            const void* data,
                            int64_t length,
  ...
  const int return_value = sendmsg(socket->fd, &msg, 0);
  if (return_value >= 0)
    return absl::optional<int>(return_value);
