...
// static
UdpSocketUniquePtr UdpSocket::Create(UdpSocket::Version version) {
  int domain;
  switch (version) {
    case Version::kV4:
      domain = AF_INET;
  ...
  const absl::optional<int> fd = CreateNonBlockingUdpSocket(domain);
  if (!fd) {
    return nullptr;
  }
  return UdpSocketUniquePtr(
      static_cast<UdpSocket*>(new UdpSocketPosix(*fd, version)));
}
...
bool UdpSocket::SetMulticastOutboundInterface(NetworkInterfaceIndex ifindex) {
  auto* const socket = UdpSocketPosix::From(this);
  ...
  switch (socket->version) {
    case UdpSocket::Version::kV4: {
      ...
      return setsockopt(socket->fd, IPPROTO_IP, IP_MULTICAST_IF,
                        &multicast_properties,
                        sizeof(multicast_properties)) == 0;
    }
    case UdpSocket::Version::kV6: {
      const auto index = static_cast<IPv6NetworkInterfaceIndex>(ifindex);
      return setsockopt(socket->fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &index,
                        sizeof(index)) == 0;
    }
  }
...
bool UdpSocket::JoinMulticastGroup(const IPAddress& address,
...
absl::optional<size_t> UdpSocket::ReceiveMessage(
    void* data,
    size_t length,
    IPEndpoint* src,
...
absl::optional<size_t> UdpSocket::SendMessage(const void* data,
                                              size_t length,
  ...
  switch (socket->version) {
    case UdpSocket::Version::kV4: {
      ...
      msg.msg_name = &sa;
      msg.msg_namelen = sizeof(sa);
      const ssize_t return_value = sendmsg(socket->fd, &msg, 0);
      return (return_value >= 0) ? absl::optional<size_t>(return_value)
