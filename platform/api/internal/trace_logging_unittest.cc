...
class MockSyncTraceLogger : public SynchronousTraceLogger {
 public:
  MockSyncTraceLogger(TraceLoggingPlatform* test_platform,
      ...
      : SynchronousTraceLogger(name, file, line, category) {
    this->platform_ = test_platform;
  }
  MockSyncTraceLogger(TraceLoggingPlatform* test_platform,
                      const char* name,
                      const char* file,
                      ...
                      TraceCategory::Value category,
                      TraceId id)
      : SynchronousTraceLogger(name, file, line, category, id) {
    this->platform_ = test_platform;
  }
  MockSyncTraceLogger(TraceLoggingPlatform* test_platform,
                      const char* name,
  ...
  MockSyncTraceLogger(TraceLoggingPlatform* test_platform,
      ...
      : SynchronousTraceLogger(name, file, line, category, ids) {
    this->platform_ = test_platform;
  }
};
...
class MockAsyncTraceLogger : public AsynchronousTraceLogger {
 public:
  MockAsyncTraceLogger(TraceLoggingPlatform* test_platform,
                       ...
                       uint32_t line,
                       TraceCategory::Value category)
      : AsynchronousTraceLogger(name, file, line, category) {
    this->platform_ = test_platform;
  }
  MockAsyncTraceLogger(TraceLoggingPlatform* test_platform,
                       const char* name,
                       ...
                       TraceCategory::Value category,
                       TraceId id)
      : AsynchronousTraceLogger(name, file, line, category, id) {
    this->platform_ = test_platform;
  }
  MockAsyncTraceLogger(TraceLoggingPlatform* test_platform,
                       const char* name,
  ...
  MockAsyncTraceLogger(TraceLoggingPlatform* test_platform,
                       ...
                       TraceCategory::Value category,
                       TraceIdHierarchy ids)
      : AsynchronousTraceLogger(name, file, line, category, ids) {
    this->platform_ = test_platform;
  }
};
...
TEST(TraceLoggingTest, TestMacroStyleInitialization) {
...

  {
    const TraceBase& TRACE_INTERNAL_UNIQUE_VAR_NAME(trace_ref) =
        IsLoggingEnabled()
            ? static_cast<const TraceBase&>(MockSyncTraceLogger(
                  platform_ptr, "Name", __FILE__, line, category))
            : static_cast<const TraceBase&>(TraceBase());
...
TEST(TraceLoggingTest, ExpectUnsetTraceIdNotSet) {
  ...
  {
    const TraceBase& TRACE_INTERNAL_UNIQUE_VAR_NAME(trace_ref) =
        MockSyncTraceLogger(platform_ptr, "Name", __FILE__, line, category, h);
...
TEST(TraceLoggingTest, ExpectHirearchyToBeApplied) {
  ...
  {
    const TraceBase& TRACE_INTERNAL_UNIQUE_VAR_NAME(trace_ref) =
        MockSyncTraceLogger{platform_ptr, "Name",   __FILE__,
