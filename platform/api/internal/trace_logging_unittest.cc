...
TEST(TraceLoggingTest, TestUniquePtrStyleInitialization) {
  ...
  {
    TRACE_INTERNAL_IGNORE_UNUSED_VAR std::unique_ptr<TraceBase>
        TRACE_INTERNAL_UNIQUE_VAR_NAME(trace_ref) =
            IsLoggingEnabled()
                ? std::make_unique<MockSyncTraceLogger>(
                      platform_ptr, "Name", __FILE__, line, category)
                : std::make_unique<TraceBase>();
    std::this_thread::sleep_for(std::chrono::milliseconds(delay_in_ms));
  }
}

TEST(TraceLoggingTest, TestTernaryStyleInitialization) {
