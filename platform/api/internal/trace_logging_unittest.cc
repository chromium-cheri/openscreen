...
TEST(TraceLoggingTest, MacroCallScopedDoesntFail) {
  TRACE_SCOPED(TraceCategory::Value::Any, "test");
}

TEST(TraceLoggingTest, MacroCallUnscopedDoesntFail) {
...
TEST(TraceLoggingTest, TestMacroStyleInitializationTrue) {
...

  {
    ...
    auto ptr = true ? TraceInstanceHelper<MockSyncTraceLogger>::Create(
                          temp, platform_ptr, "Name", __FILE__, line, category)
...
TEST(TraceLoggingTest, ExpectErrorsPassedToResult) {
...

  {
    TRACE_INTERNAL_IGNORE_UNUSED_VAR const TraceBase&
        TRACE_INTERNAL_UNIQUE_VAR_NAME(trace_ref) =
            MockSyncTraceLogger{platform_ptr, "Name", __FILE__, line, category};
    ScopedTraceOperation::set_result(result_code);
  }
}
...
TEST(TraceLoggingTest, ExpectCreationWithIdsToWork) {
...

  {
    ...
    EXPECT_EQ(ScopedTraceOperation::current_id(), current);
    EXPECT_EQ(ScopedTraceOperation::root_id(), root);
  }
}
...
TEST(TraceLoggingTest, ExpectHirearchyToBeApplied) {
...

  {
    ...
    TRACE_INTERNAL_IGNORE_UNUSED_VAR const TraceBase&
        TRACE_INTERNAL_UNIQUE_VAR_NAME(trace_ref) =
            MockSyncTraceLogger{platform_ptr, "Name", __FILE__, line, category};
    ids = ScopedTraceOperation::hierarchy();
