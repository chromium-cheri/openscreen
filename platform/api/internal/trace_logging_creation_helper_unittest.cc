// Copyright 2019 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "platform/api/internal/trace_logging_creation_helper.h"

#include "gmock/gmock.h"
#include "gtest/gtest.h"
#include "platform/base/error.h"

namespace openscreen {
namespace platform {
namespace internal {

using ::testing::_;
using ::testing::HasSubstr;
using ::testing::Invoke;

TEST(TraceLoggingCreationHelper, CreateConstCharArg) {
  const TraceLoggingArgument& arg =
      TraceLoggingArgumentFactory::Create("name", "value");
  ASSERT_STREQ(arg.name(), "name");
  ASSERT_STREQ(arg.value(), "value");
}

TEST(TraceLoggingCreationHelper, CreateNullptrArg) {
  const TraceLoggingArgument& arg =
      TraceLoggingArgumentFactory::Create("name", nullptr);
  ASSERT_STREQ(arg.name(), "name");
  ASSERT_STREQ(arg.value(), "nullptr");
}

TEST(TraceLoggingCreationHelper, CreatePointerArg) {
  int x;
  const TraceLoggingArgument& arg =
      TraceLoggingArgumentFactory::Create("name", &x);
  ASSERT_STREQ(arg.name(), "name");
  ASSERT_THAT(arg.value(), HasSubstr("0x"));
}

TEST(TraceLoggingCreationHelper, CreateFunctionArg) {
  int value = 0;
  std::function<int()> func = [&value]() mutable { return value++; };
  const TraceLoggingArgument& arg =
      TraceLoggingArgumentFactory::Create("name", func);
  ASSERT_STREQ(arg.name(), "name");
  EXPECT_EQ(value, 0);
  ASSERT_STREQ(arg.value(), "0");
  EXPECT_EQ(value, 1);
  ASSERT_STREQ(arg.value(), "0");
  EXPECT_EQ(value, 1);

  const TraceLoggingArgument& arg2 =
      TraceLoggingArgumentFactory::Create("name2", func);
  ASSERT_STREQ(arg2.name(), "name2");
  EXPECT_EQ(value, 1);
  ASSERT_STREQ(arg2.value(), "1");
  EXPECT_EQ(value, 2);
  ASSERT_STREQ(arg2.value(), "1");
  EXPECT_EQ(value, 2);
}

TEST(TraceLoggingCreationHelper, CreateStringStreamArg) {
  const TraceLoggingArgument& arg =
      TraceLoggingArgumentFactory::Create("name", 1);
  ASSERT_STREQ(arg.name(), "name");
  ASSERT_STREQ(arg.value(), "1");

  const TraceLoggingArgument& arg2 =
      TraceLoggingArgumentFactory::Create("name", Error::Code::kNone);
  ASSERT_STREQ(arg2.name(), "name");
  ASSERT_STREQ(arg2.value(), "Success");
}

class Parent {
 public:
  Parent(int* i) : i_(i) {}
  virtual ~Parent() { *i_ *= 2; }

 protected:
  int* i_;
};

class Child : public Parent {
 public:
  Child(int* i) : Parent(i) {}
  ~Child() override { *i_ *= 2; }
};

TEST(TraceLoggingCreationHelper, UniquePtrStackWrapperRunsDestructors) {
  int value = 1;

  { UniquePtrStackWrapper<Parent>::Create<Child>(&value); }

  EXPECT_EQ(value, 4);
}

}  // namespace internal
}  // namespace platform
}  // namespace openscreen
