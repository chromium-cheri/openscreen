...
namespace internal {
namespace {
constexpr int MAX_ARGUMENT_SIZE = 128;
}  // namespace
...
class TraceLoggingArgument {
 public:
  TraceLoggingArgument(const char* name) : name_(name) {
    static_assert(sizeof(this) <= MAX_ARGUMENT_SIZE,
                  "Argument requires too much storage space");
  }
  TraceLoggingArgument(TraceLoggingArgument&& other) = default;
  virtual ~TraceLoggingArgument() {}
...
class TraceLoggingArgInternal<const char*> : public TraceLoggingArgument {
 public:
  ...
  const char* value() const override { return value_; }
...
class UniquePtrStackWrapper {
 public:
  ...
  // unique_ptr.
  // NOTE: If Child is different from parent, the destructor of Child and any
  // descendants of Parent will not be run. It will be run for Parent and its
  // ancestors
...
using UserArgument =
    UniquePtrStackWrapper<TraceLoggingArgument, MAX_ARGUMENT_SIZE>;
