...
// static
thread_local std::unique_ptr<ScopedTraceOperation>
    ScopedTraceOperation::root_node_ = std::unique_ptr<ScopedTraceOperation>(
        new TraceIdSetter(TraceIdHierarchy::Empty()));

// static
thread_local ScopedTraceOperation::TraceStack ScopedTraceOperation::traces_{
    std::vector<ScopedTraceOperation*>{ScopedTraceOperation::root_node_.get()}};
...
SynchronousTraceLogger::~SynchronousTraceLogger() {
  ...
  // negligable cost.
  auto* current_platform = this->platform_override_ != nullptr
