...
class TraceIdSetter : public ScopedTraceOperation {
 ...
 private:
  TraceIdSetter(TraceIdHierarchy ids, bool push_to_stack)
...
class TraceInstanceHelper {
 private:
  ...
  using TraceInstanceAllocator = std::unique_ptr<T, TraceBaseStackDeleter>;

 public:
  template <typename... Args>
  static TraceInstanceAllocator Create(uint8_t storage[sizeof(T)],
                                       Args... args) {
    return TraceInstanceAllocator(new (storage)
                                      T(std::forward<Args&&>(args)...));
  }

  static TraceInstanceAllocator Empty() { return TraceInstanceAllocator(); }
};
