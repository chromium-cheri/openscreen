...
class Socket {
 public:
  ...
  // Due to unfortunate API weirdness, we consistently get passed multiple
  // flavors of byte streams, with various signedness and constness, even
  // though the underlying stream type is always a mutable byte array type.
  // We attempt to unify the weirdness here, making a clean API surface that
  // attempts more typing information than the void* used by
  // POSIX system APIs.
  class Data {
   public:
    Data() : begin_(nullptr) {}
    ...
    explicit Data(uint8_t* data) : begin_(data) {}
    explicit Data(const uint8_t* data) : begin_(const_cast<uint8_t*>(data)) {}

    explicit Data(char* data) : begin_(
      reinterpret_cast<uint8_t*>(data)) {}
    explicit Data(const char* data) : begin_(
      const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(data))) {}

    explicit Data(void* data) : begin_(reinterpret_cast<uint8_t*>(data)) {}
    explicit Data(const void* data) :
      begin_(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(data))) {}

    uint8_t* operator()() { return begin_; }
    const uint8_t* operator()() const { return begin_; }
