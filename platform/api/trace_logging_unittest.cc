...
template <TraceCatagory TCatagory,
          ...
          uint32_t TLine,
          const char* TFile>
class MockTraceLogger : public TraceLogger<TCatagory, TName, TLine, TFile> {
 public:
  MockTraceLogger(TraceLoggingPlatform* test_platform, bool is_async = false)
      : TraceLogger<TCatagory, TName, TLine, TFile>(is_async) {
    this->platform = test_platform;
  }
  MockTraceLogger(TraceLoggingPlatform* test_platform, TraceId id, bool is_async = false)
      : TraceLogger<TCatagory, TName, TLine, TFile>(is_async, id) {
    this->platform = test_platform;
  }
  MockTraceLogger(TraceLoggingPlatform* test_platform,
                  TraceId trace_id,
                  TraceId parent_id,
                  TraceId root_id, bool is_async = false)
      : TraceLogger<TCatagory, TName, TLine, TFile>(is_async, trace_id,
                                                    parent_id,
                                                    root_id) {
    this->platform = test_platform;
  }
  MockTraceLogger(TraceLoggingPlatform* test_platform, TraceIdHierarchy ids, bool is_async = false)
      : TraceLogger<TCatagory, TName, TLine, TFile>(is_async, ids) {
    this->platform = test_platform;
  }
};
...
class MockLoggingPlatform : public TraceLoggingPlatform {
 public:
  MOCK_METHOD1(IsLoggingEnabled, bool(TraceCatagory));
...
constexpr TraceCatagory catagory = TraceCatagory::CastPlatformLayer;
...
TEST(TraceLoggingTest, ExpectTimestampsReflectDelay) {
  ...
  {
    const TraceBase& trace_ref =
        MockTraceLogger<catagory, name, line, file>{platform_ptr};
...
TEST(TraceLoggingTest, ExpectErrorsPassedToResult) {
...

  {
    const TraceBase& trace_ref =
        MockTraceLogger<catagory, name, line, file>{platform_ptr};
    TraceIdHierarchyHandler::SetResult({result_code});
  }
}
...
TEST(TraceLoggingTest, ExpectCreationWithIdsToWork) {
  ...
  {
    const TraceBase& trace_ref = MockTraceLogger<catagory, name, line, file>{
        platform_ptr, {current, parent, root}};
    auto ids = TraceIdHierarchyHandler::GetHierarchy();
    EXPECT_EQ(ids.current.value(), current);
    EXPECT_EQ(ids.parent.value(), parent);
    EXPECT_EQ(ids.root.value(), root);
    EXPECT_EQ(TraceIdHierarchyHandler::GetCurrentId(), current);
    EXPECT_EQ(TraceIdHierarchyHandler::GetRootId(), root);
  }
}
...
TEST(TraceLoggingTest, ExpectHirearchyToBeApplied) {
  ...
  {
    const TraceBase& MAKE_UNIQUE(trace_ref) =
        MockTraceLogger<catagory, name, line, file>{platform_ptr,
                                                    {current, parent, root}};
    auto ids = TraceIdHierarchyHandler::GetHierarchy();
    EXPECT_EQ(ids.current.value(), current);
    ...
    const TraceBase& MAKE_UNIQUE(trace_ref) =
        MockTraceLogger<catagory, name, line, file>{platform_ptr};
    ids = TraceIdHierarchyHandler::GetHierarchy();
...
TEST(TraceLoggingTest, CheckTraceAsyncStartLogsCorrectly) {
  ...
  { MockTraceLogger<catagory, name, line, file>{platform_ptr, true};}
}
