...
class TraceLogger : public TraceIdHierarchyHandler {
 public:
  ...
  explicit TraceLogger(bool is_async)
      : TraceIdHierarchyHandler(absl::nullopt, absl::nullopt, absl::nullopt),
        is_async_(is_async) {
    this->Initialize();
  }
  TraceLogger(bool is_async, TraceId id)
      : TraceIdHierarchyHandler(id, absl::nullopt, absl::nullopt),
        is_async_(is_async) {
    this->Initialize();
  }
  TraceLogger(bool is_async,
              ...
              TraceId parent_id,
              TraceId root_id)
      : TraceIdHierarchyHandler(trace_id, parent_id, root_id),
        is_async_(is_async) {
    this->Initialize();
  }
