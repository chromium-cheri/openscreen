...
class NetworkRunner : public TaskRunner {
 public:
  ...
  // Waits for |socket| to be writable and then posts |callback| as a task.
  // |callback| will only be called once and then is implicitly canceled.
  virtual Error WriteNext(UdpSocket* socket, UdpWriteCallback* callback) = 0;

  // Cancels any pending wait on writing |socket|.
  virtual void CancelWrite(UdpSocket* socket) = 0;
};
