...
class UdpSocketReadCallback {
 public:
  virtual ~UdpSocketReadCallback() = default;

  virtual void OnRead(std::unique_ptr<ReceivedData> data) = 0;
};
...
class NetworkRunner {
 public:
  ...
  // called on |callback->OnRead|, modulo syntax).
  virtual Error WaitForRead(UdpSocket* socket,
                            UdpSocketReadCallback* callback) = 0;
