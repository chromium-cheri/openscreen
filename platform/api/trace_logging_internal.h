...
class SynchronousTraceLogger
 ...
 public:
  SynchronousTraceLogger(const char name[TNameSize], const char file[TFileSize])
  ...
  virtual ~SynchronousTraceLogger() {
    ...
    auto* current_platform =
        this->platform != nullptr ? this->platform : TraceBase::default_trace_platform;
    auto end_time = Clock::now();
    current_platform->LogTrace(this->name_, TLine, this->file_name_, this->start_time_, end_time,
                               this->trace_id_, this->parent_id_, this->root_id_, this->result);
  }

  // Prevent copying this class, because that would mess with the Trace
  // Hierarchy stack.
  SynchronousTraceLogger(const SynchronousTraceLogger& other) = delete;
  SynchronousTraceLogger& operator=(const SynchronousTraceLogger& other) = delete;
};
...
class AsynchronousTraceLogger
 ...
 public:
  ...
  virtual ~AsynchronousTraceLogger() {
    ...
    auto* current_platform =
        this->platform != nullptr ? this->platform : TraceBase::default_trace_platform;
    current_platform->LogAsyncStart(this->name_, TLine, this->file_name_, this->start_time_,
  ...
  AsynchronousTraceLogger& operator=(const AsynchronousTraceLogger& other) = delete;
};
