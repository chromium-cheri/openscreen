  ...
  std::vector<InterfaceInfo> info_list;
  {
    ...
    while (!done) {
      ...
      for (nh = reinterpret_cast<struct nlmsghdr*>(buf); NLMSG_OK(nh, len);
        ...
        struct iwreq wr;
        strncpy(wr.ifr_name, info.name.c_str(), IFNAMSIZ - 1);
  ...
  std::vector<InterfaceAddresses> address_list;
  {
    ...
    while (!done) {
      ...
      for (nh = reinterpret_cast<struct nlmsghdr*>(buf); NLMSG_OK(nh, len);
        ...
        auto* ifa = static_cast<struct ifaddrmsg*>(NLMSG_DATA(nh));
        ...
        if (ifa->ifa_family == AF_INET) {
          ...
          for (struct rtattr* rta = IFA_RTA(ifa); RTA_OK(rta, attrlen);
            ...
            if (rta->rta_type == IFA_LABEL) {
              DCHECK_EQ(addr_it->info.name,
                        reinterpret_cast<const char*>(RTA_DATA(rta)));
            } else if (rta->rta_type == IFA_ADDRESS) {
              DCHECK_EQ(address.address.bytes.size(), RTA_PAYLOAD(rta));
              address.address =
          ...
          auto& address = addr_it->ipv6_addresses.back();
          address.prefix_length = ifa->ifa_prefixlen;
          ...
          for (struct rtattr* rta = IFA_RTA(ifa); RTA_OK(rta, attrlen);
            ...
            if (rta->rta_type == IFA_LABEL) {
              ...
              DCHECK_EQ(address.address.bytes.size(), RTA_PAYLOAD(rta));
              address.address =
                  IPv6Address(static_cast<uint8_t*>(RTA_DATA(rta)));
            } else if (rta->rta_type == IFA_LOCAL) {
              DCHECK_EQ(local.bytes.size(), RTA_PAYLOAD(rta));
              have_local = true;
