...
TEST(BytesViewTest, TestBasics) {
  ...
  BytesView googlePlex = BytesView(kSampleData, kSampleSize);
  ...
  BytesView lastBytes(googlePlex.last(4));
  ...
  BytesView middleBytes(googlePlex.subspan(2, 4));
