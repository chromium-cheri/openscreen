...
Error NetworkReader::ReadRepeatedly(UdpSocket* socket, Callback callback) {
  ...
  sockets_to_unwatch_.erase(socket);
...
bool NetworkReader::CancelRead(UdpSocket* socket) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (read_callbacks_.find(socket) == read_callbacks_.end()) {
    return false;
  }

  return sockets_to_unwatch_.insert(socket).second;
}

bool NetworkReader::IsRead(UdpSocket* socket) {
  std::lock_guard<std::mutex> lock(mutex_);
...
Error NetworkReader::WaitAndRead(Clock::duration timeout) {
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    for (UdpSocket* to_delete : sockets_to_unwatch_) {
      read_callbacks_.erase(to_delete);
    }
    sockets_to_unwatch_.clear();
