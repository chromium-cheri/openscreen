...
TEST(TaskRunnerTest, TaskRunnerIsStableWithLotsOfTasks) {
  ...
  for (int i = 0; i < kNumberOfTasks; ++i) {
    const auto task = std::bind([&ran_tasks] { ran_tasks += "1"; });
    runner->PostTask(task);
  }
...
TEST(TaskRunnerTest, TaskRunnerCanCancelTasks) {
  auto runner = std::unique_ptr<TaskRunnerImpl>(new TaskRunnerImpl());

  std::string ran_tasks;
  const auto task = std::bind([&ran_tasks] { ran_tasks += "Foo"; });
  const auto task1 = std::bind([&ran_tasks] { ran_tasks += "Bar"; });

  TaskId id = runner->PostTask(task);
  runner->PostTask(task1);

  runner->CancelTask(id);

  runner->RunUntilIdleForTesting();
  EXPECT_EQ(ran_tasks, "Bar");
}
}  // namespace platform
}  // namespace openscreen
