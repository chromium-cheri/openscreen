...
class TlsCredentials {
 public:
  ...
  const CRYPTO_BUFFER* certificate() const { return certificate_.get(); }

  // The original private key provided on construction.
  const bssl::UniquePtr<EVP_PKEY>& private_key() const { return private_key_; }
  ...
  // A base64 encoded version of the private key's associated public key.
  std::vector<uint8_t> public_key_base64() const { return public_key_base64_; }

  // A SHA-256 digest of the private key's associated public key.
