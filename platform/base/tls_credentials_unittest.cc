...
TEST(TlsCredentialsTest, CredentialsAreGeneratedAppropriately) {
  ...
  const auto raw_cert = credentials.raw_der_certificate().value();
  ...
  const auto private_key = credentials.private_key_base64().value();
  ...
  const auto public_key = credentials.public_key_base64().value();
  ...
  EXPECT_EQ(credentials.public_key_hash().value().size(),
            kSha256HashSizeInBytes);
}
