...
template <Error::Code TCode>
void InitializeErrorNameHandler(char names[kMaxCodeIndex][kMaxCharSize + 1]) {
  char* current = names[static_cast<int8_t>(TCode)];
  uint8_t size = WriteCodeName<TCode>(&current[1]);
  current[0] = static_cast<char>(size);
  InitializeErrorNameHandler<static_cast<Error::Code>(
      static_cast<int8_t>(TCode) - 1)>(names);
}

template <>
void InitializeErrorNameHandler<static_cast<Error::Code>(-1)>(
    char names[kMaxCodeIndex][kMaxCharSize + 1]) {}

class ErrorNameHandler {
 public:
  ErrorNameHandler() {
    InitializeErrorNameHandler<Error::Code::kLastDoNotUse>(names_);
  }
