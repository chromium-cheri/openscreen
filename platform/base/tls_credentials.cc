...
bssl::UniquePtr<X509> CreateCertificate(
  ...
  OPENSSL_CHECK(EVP_PKEY_set1_RSA(pkey.get(), &keypair));
  OPENSSL_CHECK(X509_sign(certificate.get(), pkey.get(), EVP_sha256()));
  return certificate;
}

std::vector<uint8_t> ExportBytes(RSA rsa) {
...
const std::vector<uint8_t>& TlsCredentials::private_key_base64() {
  if (!private_key_base64_) {
    bssl::UniquePtr<RSA> private_key(RSAPrivateKey_dup(keypair_.get()));
    private_key_base64_ =
        std::make_unique<std::vector<uint8_t>>(ExportBytes(*private_key));
  }
