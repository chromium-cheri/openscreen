...
ErrorOr<const std::vector<uint8_t>> Sha256Digest(
  ...
  if (!EVP_Digest(data.data(), data.size(), digest, &digest_len, EVP_sha256(),
                  nullptr)) {
    return Error::Code::kHashCalculationFailure;
  }

  return std::vector<uint8_t>(digest, digest + digest_len);
}

ErrorOr<const std::vector<uint8_t>> WriteOutKey(EVP_PKEY key, bool is_public) {
...
ErrorOr<TlsCredentials> TlsCredentials::Create(
  ...
  const std::vector<uint8_t> raw_der_certificate(buffer, buffer + len);
  // BoringSSL doesn't free the temporary buffer.
  OPENSSL_free(buffer);

  ErrorOr<const std::vector<uint8_t>> private_key_or_error =
      WriteOutKey(*key_pair, false /* is_public */);
  if (!private_key_or_error) {
    return Error::Code::kParameterInvalid;
  }
  ErrorOr<const std::vector<uint8_t>> public_key_or_error =
      WriteOutKey(*key_pair, true /* is_public */);
  if (!public_key_or_error) {
    return Error::Code::kParameterInvalid;
  }

  ErrorOr<const std::vector<uint8_t>> public_key_hash_or_error =
