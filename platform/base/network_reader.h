...
class NetworkReader {
 public:
  ...
  // NOTE: The provided TaskRunner must live for the duration of this instance's
  // life.
  static std::unique_ptr<NetworkReader> Create(TaskRunner* task_runner);
  ...
  // ReadRepeatedly is alled on a socket already being watched, the new call
 ...
 protected:
  ...
  virtual std::unique_ptr<UdpReadCallback::Packet> ReadFromSocket(
      UdpSocket* socket);
