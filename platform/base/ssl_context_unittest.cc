...
TEST(SSLContextTest, InitFailsWithInvalidCertificateOrKey) {
  TemporaryFile cert;
  TemporaryFile key;
...
TEST(SSLContextTest, InitSucceedsWithValidCertAndKey) {
  TemporaryFile cert;
  TemporaryFile key;
...
TEST(SSLContextTest, CanGetNewSSLPtr) {
  TemporaryFile cert;
  TemporaryFile key;
...
TEST(SSLContextTest, CanCreateMultipleContexts) {
  TemporaryFile cert;
  TemporaryFile key;
  ...
  auto context = SSLContext::Create(cert.get_file_name(), key.get_file_name());
  auto context2 = SSLContext::Create(cert.get_file_name(), key.get_file_name());
  auto context3 = SSLContext::Create(cert.get_file_name(), key.get_file_name());
  ASSERT_TRUE(context);
