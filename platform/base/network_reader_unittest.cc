...
class TestingNetworkWaiter final : public NetworkReader {
 public:
  ...
  bool DidDeletionGoThrough(UdpSocket* socket) {
    return read_callbacks_.find(socket) == read_callbacks_.end() &&
           sockets_to_unwatch_.find(socket) == sockets_to_unwatch_.end();
  }
...
TEST(NetworkReaderTest, UnwatchReadableSucceeds) {
  ...
  EXPECT_FALSE(network_waiter.IsRead(socket.get()));
...
TEST(NetworkReaderTest, ConfirmSocketsUnwatchedAfterWaitOnSuccess) {
  auto* mock_waiter_ptr = new MockNetworkWaiter();
  std::unique_ptr<NetworkWaiter> mock_waiter =
      std::unique_ptr<NetworkWaiter>(mock_waiter_ptr);
  std::unique_ptr<TaskRunner> task_runner =
  ...
  EXPECT_TRUE(network_waiter.CancelRead(socket.get()));
  EXPECT_FALSE(network_waiter.IsRead(socket.get()));
  EXPECT_FALSE(network_waiter.DidDeletionGoThrough(socket.get()));
  ...
  EXPECT_FALSE(network_waiter.IsRead(socket.get()));
  EXPECT_TRUE(network_waiter.DidDeletionGoThrough(socket.get()));
...
TEST(NetworkReaderTest, ConfirmSocketsUnwatchedAfterWaitOnError) {
  auto* mock_waiter_ptr = new MockNetworkWaiter();
  std::unique_ptr<NetworkWaiter> mock_waiter =
      std::unique_ptr<NetworkWaiter>(mock_waiter_ptr);
  std::unique_ptr<TaskRunner> task_runner =
  ...
  EXPECT_CALL(*mock_waiter_ptr, AwaitSocketsReadable(_, timeout))
      .WillOnce(Return(ByMove(Error::Code::kIOFailure)));
  EXPECT_EQ(network_waiter.WaitTesting(timeout), Error::Code::kIOFailure);
