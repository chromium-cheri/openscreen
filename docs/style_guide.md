...
C++17 is the maximum language level allowed in the library.  For C++11, C++14, and C++17 language and library features, we follow  [C++ features in Chromium](https://chromium.googlesource.com/chromium/src/+/main/styleguide/c++/c++-features.md) guidelines with the following exceptions noted below.

In general Open Screen follows [You Aren't Gonna Need It](https://martinfowler.com/bliki/Yagni.html) principles.  This means we avoid adding generic classes, functions, or libraries that are only used in one place for one thing.  Once there are multiple usages, the functionality is a candidate to pull out into shared code to handle those specific usages.
