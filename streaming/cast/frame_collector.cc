...
bool FrameCollector::CollectPart(const RtpPacketParser::ParseResult& part) {
  ...
  if (num_missing_packets_ == kUnknownNumberOfPackets) {
    ...
    chunks_.assign(num_missing_packets_,
                   DataChunkSpan{DataChunkSpan::kMissingOffset, 0});

    // Optimization: Assume that all packets are the same size as the first, and
    // reserve the frame's entire payload buffer up-front to prevent needless
    // re-allocations/copies when the vector capacity must be auto-increased.
    // While the Cast Streaming spec doesn't guarantee anything about how a
    // frame's payload is split-up over multiple packets, Senders will usually
    // spread it evenly across all packets (with any smaller remainder in the
    // last packet). If this assumption is wrong, no harm is being done here.
    frame_.owned_data_.reserve(part.payload.size() * frame_packet_count);
  } else {
  ...
  if (chunks_[part.packet_id].offset != DataChunkSpan::kMissingOffset) {
    return true;
  }
...
void FrameCollector::AppendMissingPackets(
  ...
  for (int packet_id = 0; packet_id < frame_packet_count; ++packet_id) {
    if (chunks_[packet_id].offset == DataChunkSpan::kMissingOffset) {
      nacks->push_back(
