...
bool FrameCollector::CollectPart(const RtpPacketParser::ParseResult& part,
  ...
  // Take ownership of the |buffer| (no copy!), and record the region of the
  // buffer containing the payload data. The payload region is usually all but
  // the first few dozen bytes of the buffer.
  DataChunk& chunk = chunks_[part.packet_id];
  ...
  // Success! Decrement the missing packet count, and update the total payload
  // size with this chunk's payload size.
  --num_missing_packets_;
  OSP_DCHECK_GE(num_missing_packets_, 0);
  payload_size_so_far_ += chunk.payload.size();
  return true;
}
...
const EncryptedFrame& FrameCollector::PeekAtAssembledFrame() {
  ...
  if (!frame_.data.data()) {
    frame_.owned_data_.reserve(payload_size_so_far_);
...
FrameCollector::DataChunk::DataChunk() = default;
FrameCollector::DataChunk::~DataChunk() = default;
