...
absl::Span<uint8_t> RtpPacketizer::GeneratePacket(const EncryptedFrame& frame,
  ...
  OSP_DCHECK_GE(static_cast<int>(buffer.size()), max_packet_size_);
  ...
  AppendField<uint8_t>(
      (is_last_packet ? kMarkerBitMask : 0) | payload_type_7bits_, &buffer);
...
int RtpPacketizer::ComputeNumberOfPackets(const EncryptedFrame& frame) const {
  ...
  int num_packets =
      (static_cast<int>(frame.data.size()) + (max_payload_size() - 1)) /
      max_payload_size();
