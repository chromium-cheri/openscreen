...
bool SenderReportParser::Parse(
  ...
  while (!buffer.empty()) {
    ...
    if (static_cast<int>(buffer.size()) < header->size) {
      return false;
    }

    // Only process Sender Reports.
    if (header->packet_type == RtcpPacketType::kSenderReport) {
      if (header->size < kRtcpSenderReportSize) {
        return false;
      }
      auto chunk = buffer.subspan(0, header->size);
      if (ConsumeField<uint32_t>(&chunk) == session_->sender_ssrc()) {
        ...
        auto optional_report_block = RtcpReportBlock::ParseOne(
            chunk, header->item_count, session_->receiver_ssrc());
        if (optional_report_block) {
          report.report_block = optional_report_block;
        }
      }
    }

    buffer.remove_prefix(header->size);
  }
