...
TEST_F(CompoundRtcpParserTest, ProcessesEmptyPacket) {
  ...
  EXPECT_TRUE(parser()->Parse(absl::Span<const uint8_t>(kEmpty, 0)));
}
...
TEST_F(CompoundRtcpParserTest, ParsesReceiverReportWithoutReportBlock) {
  ...
  // Expect NO calls to mock client.
  EXPECT_TRUE(parser()->Parse(kReceiverReportWithoutReportBlock));
}
...
TEST_F(CompoundRtcpParserTest, ParsesReceiverReportWithReportBlock) {
  ...
  EXPECT_TRUE(parser()->Parse(kReceiverReportWithReportBlock));
...
TEST_F(CompoundRtcpParserTest, ParsesPictureLossIndicatorMessage) {
  ...
  EXPECT_TRUE(parser()->Parse(kPictureLossIndicatorPacket));
}
...
TEST_F(CompoundRtcpParserTest, IgnoresStalePackets) {
  ...
  EXPECT_CALL(*(client()), OnReceiverLostPicture()).Times(0);
  EXPECT_TRUE(parser()->Parse(kNotStaleCompoundPacket));
  EXPECT_TRUE(parser()->Parse(kStaleCompoundPacketWithPli));
}
...
TEST_F(CompoundRtcpParserTest, IgnoresUnknownExtendedReports) {
  ...
  EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(expected_timestamp));
  EXPECT_TRUE(parser()->Parse(kPacketWithThreeExtendedReports));
}
...
TEST_F(CompoundRtcpParserTest, ParsesSimpleFeedback) {
  ...
  parser()->SetMaxFeedbackFrameId(FrameId::first() + 42);
  ...
  EXPECT_TRUE(parser()->Parse(kFeedbackPacket));
  Mock::VerifyAndClearExpectations(client());

  // Second scenario: Valid range of FrameIds is [299,554]. Note: 544 == 0x22a.
  parser()->SetMaxFeedbackFrameId(FrameId::first() + 0x22a);
...
TEST_F(CompoundRtcpParserTest, ParsesFeedbackWithNacks) {
  ...
  // The de-duped and sorted lists of the frame/packet NACKs expected when
  // parsing kFeedbackPacket:
  const FrameId kMissingFrames[] = {FrameId::first() + 12,
                                    FrameId::first() + 13};
  const std::pair<FrameId, FramePacketId> kMissingPackets[] = {
      {FrameId::first() + 11, 3},  {FrameId::first() + 11, 7},
      {FrameId::first() + 11, 8},  {FrameId::first() + 11, 10},
      {FrameId::first() + 11, 11}, {FrameId::first() + 11, 15},
      {FrameId::first() + 14, 0},  {FrameId::first() + 14, 2},
      {FrameId::first() + 14, 7},
  };
  ...
  EXPECT_CALL(*(client()),
              OnReceiverIsMissingFrames(absl::MakeSpan(kMissingFrames)));
  EXPECT_CALL(*(client()),
              OnReceiverIsMissingPackets(absl::MakeSpan(kMissingPackets)));
  EXPECT_TRUE(parser()->Parse(kFeedbackPacket));
}
...
TEST_F(CompoundRtcpParserTest, ParsesFeedbackWithAcks) {
  ...
  const FrameId kFrame13Only[] = {FrameId::first() + 13};
  const std::pair<FrameId, FramePacketId> kFrame11Packet3Only[] = {
      {FrameId::first() + 11, 3}};

  // From the larger packet: Many frame ACKs.
  const FrameId kManyFrames[] = {
  ...
  parser()->SetMaxFeedbackFrameId(FrameId::first() + 100);
  const auto expected_frame_id = FrameId::first() + 10;
  const auto expected_playout_delay = std::chrono::milliseconds(294);
  EXPECT_CALL(*(client()),
              OnReceiverCheckpoint(expected_frame_id, expected_playout_delay));
  EXPECT_CALL(*(client()), OnReceiverHasFrames(absl::MakeSpan(kFrame13Only)));
  EXPECT_CALL(*(client()),
              OnReceiverIsMissingPackets(absl::MakeSpan(kFrame11Packet3Only)));
  EXPECT_TRUE(parser()->Parse(kSmallerFeedbackPacket));
  Mock::VerifyAndClearExpectations(client());
  ...
  EXPECT_CALL(*(client()),
              OnReceiverCheckpoint(expected_frame_id, expected_playout_delay));
  EXPECT_CALL(*(client()),
              OnReceiverHasFrames(absl::MakeConstSpan(kManyFrames)));
  EXPECT_TRUE(parser()->Parse(kLargerFeedbackPacket));
  Mock::VerifyAndClearExpectations(client());
}
