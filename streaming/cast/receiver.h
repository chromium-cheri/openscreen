...
//    public:
//     MyPlayer(Receiver* receiver) : receiver_(receiver) {
...
//     void OnMoreFramesComplete() override {
...
class Receiver final : public ReceiverTransport::Client {
 public:
  class Consumer {
   public:
    virtual ~Consumer();
    virtual void OnMoreFramesComplete() = 0;
  };

  Receiver(Environment* environment,
           ReceiverTransport* trasport,
           Ssrc sender_ssrc,
           Ssrc receiver_ssrc,
           int rtp_timebase,
 ...
 private:
  // ReceiverTransport::Client implementation.
  ...
  ReceiverTransport* const transport_;
};
