...
class Receiver {
 public:
  ...
  // data. The Consumer calls this method after being notified via
  // OnFramesReady(), and it can also call this whenever AdvanceToNextFrame()
  // indicates another frame is ready. |frame->data| must point to a
  // sufficiently-sized buffer that will be populated with the frame's payload
  // data. Upon return |frame->data| will be set to the portion of the buffer
  // that was populated.
