...
class Receiver {
 ...
 private:
  platform::Clock::time_point now() const { return now_function_(); }
  ...
  // to request re-transmits.
  void SendRtcpImmediately();

  // Schedule an RTCP packet to be built/sent one |interval| after the last time
  // a packet was sent (or immediately). This could be pre-empted when
  // SendRtcpImmediately() is called, in reaction to certain events, before the
  // scheduled send time is reached.
  void ScheduleRtcpSend(platform::Clock::duration interval);
