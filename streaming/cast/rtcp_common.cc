...
void RtcpReportBlock::SetDelaySinceLastReport(
  ...
  constexpr Delay kMaxValidDelay{std::numeric_limits<uint32_t>::max()};
  constexpr auto kMaxValidDelayInWrongTimebase =
      std::chrono::duration_cast<platform::Clock::duration>(kMaxValidDelay);
  if (delay_in_wrong_timebase > kMaxValidDelayInWrongTimebase) {
    delay_since_last_report = kMaxValidDelay;
    return;
  }
  if (delay_in_wrong_timebase <= platform::Clock::duration::zero()) {
    delay_since_last_report = Delay::zero();
    return;
  }

  // If this point is reached, then the |delay_in_wrong_timebase| is
  // representable as a Delay within the valid range.
  delay_since_last_report =
      std::chrono::duration_cast<Delay>(delay_in_wrong_timebase);
}
