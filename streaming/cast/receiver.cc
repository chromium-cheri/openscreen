...
int Receiver::AdvanceToNextFrame() {
  ...
  for (FrameId f = immediate_next_frame; f <= latest_frame_expected_; ++f) {
    PendingFrame* const entry = GetMutableQueueEntry(f);
    if (entry->collector.is_complete()) {
      ...
      if (encrypted_frame.dependency != EncodedFrame::DEPENDENT) {
...
void Receiver::ConsumeNextFrame(EncodedFrame* frame) {
  // Assumption: The required call to AdvanceToNextFrame() should have set
  // |last_frame_consumed_| to one before the frame to be consumed here.
  const FrameId which = last_frame_consumed_ + 1;
  OSP_CHECK_LE(which, checkpoint_frame());

  // Decrypt the frame, populating the given output |frame|.
  PendingFrame* const entry = GetMutableQueueEntry(which);
  ...
  frame->reference_time =
      *entry->estimated_capture_time + ResolveTargetPlayoutDelay(which);
...
void Receiver::OnReceivedRtpPacket(Clock::time_point arrival_time,
  ...
  if (part->frame_id > latest_frame_expected_) {
    ...
    do {
      ...
      GetMutableQueueEntry(latest_frame_expected_)
  ...
  PendingFrame* const pending_frame = GetMutableQueueEntry(part->frame_id);
...
Receiver::PendingFrame* Receiver::GetMutableQueueEntry(FrameId frame_id) {
  return &pending_frames_[(frame_id - FrameId::first()) %
                          pending_frames_.size()];
}
