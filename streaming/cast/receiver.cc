...
Receiver::Receiver(Environment* environment,
    ...
    : now_function_(environment->now_function()),
      packet_router_(packet_router),
      rtcp_session_(sender_ssrc, receiver_ssrc, now()),
...
void Receiver::OnReceivedRtcpPacket(Clock::time_point arrival_time,
                                    std::vector<uint8_t> packet) {
  if (auto parsed_report = rtcp_parser_.Parse(packet)) {
    last_sender_report_ = std::move(parsed_report);
    last_sender_report_arrival_time_ = arrival_time;
  } else {
    RECEIVER_LOG(WARN) << "Parse of a [seemingly?] RTCP packet of "
                       << packet.size() << " bytes failed.";
    return;
  }
  ...
  report.SetDelaySinceLastReport(now() - last_sender_report_arrival_time_);
  rtcp_builder_.IncludeReceiverReportInNextPacket(report);

  SendRtcpImmediately();
}

void Receiver::SendRtcpImmediately() {
  ...
  // Schedule an upper-bound on when the next RTCP packet must be sent. While
  // incomplete frames exist in the queue, send RTCP packets (with ACK/NACK
  // feedback) frequently. When there are no incomplete frames, use a longer
  // "keepalive" interval.
  ScheduleRtcpSend(no_nacks ? kRtcpReportInterval : kNackFeedbackInterval);
}

void Receiver::ScheduleRtcpSend(Clock::duration interval) {
  rtcp_alarm_.Schedule(std::bind(&Receiver::SendRtcpImmediately, this),
