...
// these are used to determine an fixed origin reference point for all
// conversions. Thus, to avoid introducing sampling errors, only one
// NtpTimeConverter instance should be used for converting all the NTP
...
class NtpTimeConverter {
 ...
 private:
  // The time point on the platform clock that corresponds to approximately the
  // same time point on the NTP timeline. Note that it is acceptable for the
  // granularity of the NTP seconds value to be whole seconds here: Both a Cast
  // Streaming Sender and Receiver will assume their clocks can be off (with
  // respect to each other) by even a large amount; and all that matters is that
  // time ticks forward at a reasonable pace from some initial point.
