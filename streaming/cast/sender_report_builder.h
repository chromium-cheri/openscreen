...
class SenderReportBuilder {
 public:
  ...
  // |buffer| (which must be kBufferSize in size). Returns the subspan of
  // |buffer| that contains the result and a StatusReportId the receiver might
  // use in its own reports to reference this specific report.
  std::pair<absl::Span<uint8_t>, StatusReportId> BuildPacket(
      const RtcpSenderReport& sender_report,
      absl::Span<uint8_t> buffer) const;

  // The maximum possible size (in bytes) of the Sender Report RTCP packets
  // built by BuildPacket().
