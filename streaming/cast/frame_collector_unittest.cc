...
TEST(FrameCollectorTest, CollectsFrameWithOnlyOnePart) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    EXPECT_TRUE(collector.CollectPart(part, &buffer));
...
TEST(FrameCollectorTest, RejectsInvalidParts) {
  ...
  part.payload = absl::Span<uint8_t>(buffer);
  EXPECT_FALSE(collector.CollectPart(part, &buffer));
  // Note: When CollectPart() returns false, it does not take ownership of the
  // buffer memory.
