...
void DemoMain() {
  ...
  platform::LogInit(nullptr /* stdout */);
  ...
  const DummyPlayer audio_player(now_function, &task_runner, &audio_receiver);
  const DummyPlayer video_player(now_function, &task_runner, &video_receiver);
