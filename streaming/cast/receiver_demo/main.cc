...
int main(int argc, const char* argv[]) {
  ...
  auto bind_error = socket->Bind();
  if (!bind_error.ok()) {
    OSP_LOG_ERROR << "Failed to bind receive UDP socket at " << receive_endpoint
                  << ": " << bind_error.message();
    return 1;
  }
  ...
  // Create the common transport layer shared by both the Audio Receiver and
  // Video Receiver.
  openscreen::cast_streaming::ReceiverTransport transport(&env);

  // Create the two Receivers.
  openscreen::cast_streaming::Receiver audio_receiver(
      &env, &transport, kDemoAudioSenderSsrc, kDemoAudioReceiverSsrc,
  ...
  openscreen::cast_streaming::Receiver video_receiver(
      &env, &transport, kDemoVideoSenderSsrc, kDemoVideoReceiverSsrc,
