...
Clock::time_point SDLAudioPlayer::RenderNextFrame(
  ...
  if (frame.channels != 1 && frame.channels != 2 && frame.channels != 4 &&
      frame.channels != 6) {
    std::ostringstream error;
    error << "SDL does not support anything but 1, 2, 4, or 6 (5+1) audio "
             "channels, but "
          << frame.channels << " were decoded.";
    OnFatalError(error.str());
    return {};
  }
...
void SDLAudioPlayer::Present() {
  ...
  if (SDLAudioSpecsAreDifferent(pending_audio_spec_, device_spec_)) {
    ...
    SDL_PauseAudioDevice(device_, 0);  // 0 = Resume playback.
  }
