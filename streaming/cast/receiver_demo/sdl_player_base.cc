...
void SDLPlayerBase::RenderAndSchedulePresentation() {
  ...
  frames_to_render_.erase(it);
  const Clock::time_point presentation_time = RenderNextFrame(current_frame_);
  if (state_ == kError) {  // RenderNextFrame() called OnFatalError().
    // Call this method again soon, to call RenderWhileIdle() and schedule idle
    // presentations, to show the error indication from now on.
    ResumeRendering();
  } else {
  ...
  const auto measured_processing_time = now_() - current_frame_.start_time;
