...
bool SDLVideoPlayer::RenderWhileIdle(
  ...
  if (state() == kPresented && frame) {
    RenderNextFrame(*frame);
    const PlayerState new_player_state = state();
    if (new_player_state != kError) {
      return true;
    }
    // From this point, RenderNextFrame() called OnFatalError(), setting the
    // player state to kError. Thus, fall-through to the "red splash" rendering
    // below.
  }
...
Clock::time_point SDLVideoPlayer::RenderNextFrame(
    const SDLPlayerBase::PresentableFrame& frame) {
