...
class SenderReportParser {
 public:
  ...
  // Parses the RTCP |packet|, and sets |sender_report| with a parsed sender
  // report if the packet contained one. Returns false if the packet was
  // corrupt, but may return true even if |sender_report| is not set.
