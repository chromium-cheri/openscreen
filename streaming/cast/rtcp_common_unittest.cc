...
TEST(RtcpCommonTest, SerializesAndParsesHeaderForSenderReports) {
  ...
  EXPECT_EQ(original.item_count, parsed->item_count);
  EXPECT_EQ(original.subtype, parsed->subtype);
  EXPECT_EQ(original.size, parsed->size);
}
...
TEST(RtcpCommonTest, SerializesAndParsesHeaderForCastFeedback) {
  ...
  original.subtype = RtcpSubtype::kFeedback;
  original.size = 99 * sizeof(uint32_t);
...
TEST(RtcpCommonTest, SerializesAndParsesRtcpReportBlocks) {
  ...
  original.recipient_ssrc = kSsrc;
  original.packet_fraction_lost = 0x67;
  ...
  ASSERT_TRUE(parsed.has_value());
  EXPECT_EQ(original.recipient_ssrc, parsed->recipient_ssrc);
  EXPECT_EQ(original.packet_fraction_lost, parsed->packet_fraction_lost);
...
TEST(RtcpCommonTest, ParsesOneReportBlockFromMultipleBlocks) {
  ...
  expected.recipient_ssrc = kSsrc;
  expected.packet_fraction_lost = 0x67;
  ...
  for (int i = 0; i < kNumBlocks; ++i) {
    ...
    another.recipient_ssrc = expected.recipient_ssrc + i - 2;
    another.packet_fraction_lost = expected.packet_fraction_lost + i - 2;
  ...
  ASSERT_TRUE(parsed.has_value());
  EXPECT_EQ(expected.recipient_ssrc, parsed->recipient_ssrc);
  EXPECT_EQ(expected.packet_fraction_lost, parsed->packet_fraction_lost);
