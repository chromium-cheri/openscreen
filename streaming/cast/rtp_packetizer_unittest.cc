...
class RtpPacketizerTest : public testing::Test {
 public:
  ...
  // (i.e., all PacketParser unit tests are passing).
  void TestGeneratePacket(const EncryptedFrame& frame, PacketId packet_id) {
    SCOPED_TRACE(testing::Message() << "packet_id=" << packet_id);

    const int frame_payload_size = frame.data.size();
    const int packet_payload_size = 1449;
    ...
    uint8_t scratch[kMaxRtpPacketSize];
    memset(scratch, 0, sizeof(scratch));
    const auto packet = packetizer_.GeneratePacket(frame, packet_id, scratch);
    ASSERT_TRUE(IsSubspan(packet, scratch));
    ASSERT_TRUE(parser_.Parse(packet));

    // Check that RTP header fields match expected values.
    ASSERT_TRUE(parser_.has_rtp_header_and_payload());
    const RtpCastHeader& header = parser_.rtp_header();
    EXPECT_EQ(kPayloadType, header.payload_type);
    EXPECT_EQ(frame.rtp_timestamp, header.rtp_timestamp);
    EXPECT_EQ(frame.dependency == EncodedFrame::KEY, header.is_key_frame);
    EXPECT_EQ(frame.frame_id, header.frame_id);
    EXPECT_EQ(packet_id, header.packet_id);
    EXPECT_EQ(static_cast<PacketId>(num_packets - 1), header.max_packet_id);
    EXPECT_EQ(frame.referenced_frame_id, header.referenced_frame_id);
    ...
    if (packet_id == PacketId{0}) {
      EXPECT_EQ(frame.new_playout_delay, header.new_playout_delay);
    } else {
      EXPECT_EQ(std::chrono::milliseconds(0), header.new_playout_delay);
    }

    // Check that the RTP payload is correct for this packet.
    const auto payload = parser_.rtp_payload();
    ASSERT_TRUE(IsSubspan(payload, packet));
 ...
 private:
  ...
  PacketParser parser_{Ssrc{0}, ssrc_};
...
TEST_F(RtpPacketizerTest, GeneratesPacketsForSequenceOfFrames) {
  for (int i = 0; i < 10; ++i) {
    ...
    for (int j = 0; j < num_packets; ++j) {
      TestGeneratePacket(frame, static_cast<PacketId>(j));
...
TEST_F(RtpPacketizerTest, GeneratesPacketsForFrameWithLatencyChange) {
  ...
  for (int i = 0; i < num_packets; ++i) {
    TestGeneratePacket(frame, static_cast<PacketId>(i));
    if (testing::Test::HasFailure()) {
      return;
    }
  }
...
TEST_F(RtpPacketizerTest, GeneratesOnePacketForFrameWithNoPayload) {
  ...
  TestGeneratePacket(frame, PacketId{0});
}
...
TEST_F(RtpPacketizerTest, GeneratesPacketForRetransmission) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    TestGeneratePacket(frame, PacketId{3});
  }
}
