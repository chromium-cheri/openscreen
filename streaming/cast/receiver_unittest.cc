...
struct SimulatedFrame : public EncodedFrame {
  static constexpr RtpTimeTicks kRtpStartTime =
      RtpTimeTicks::FromTimeSinceOrigin(seconds(0), kRtpTimebase);
  static constexpr milliseconds kFrameDuration = milliseconds(10);
  static constexpr RtpTimeDelta kRtpFrameDuration =
      RtpTimeDelta::FromDuration(kFrameDuration, kRtpTimebase);
  ...
  SimulatedFrame(Clock::time_point first_frame_reference_time, int which) {
    ...
    rtp_timestamp = kRtpStartTime + kRtpFrameDuration * which;
...
// static
constexpr RtpTimeTicks SimulatedFrame::kRtpStartTime;
constexpr milliseconds SimulatedFrame::kFrameDuration;
constexpr RtpTimeDelta SimulatedFrame::kRtpFrameDuration;
...
TEST_F(ReceiverTest, ReceivesFramesInOrder) {
  ...
  sender()->SendSenderReport(start_time, SimulatedFrame::kRtpStartTime);
  AdvanceClockAndRunTasks(kOneWayNetworkDelay);
