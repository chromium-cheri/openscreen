...
class VideoEncoder {
  ...
  struct Statistics {
    // A normalized value indicating how close to the "ideal" utilization the
    // encoder was during this frame. Typically, encoders define this value
    // as (actual bit rate * actual qp) / (max qp * target bit rate), however
    // encoders are free to define this value as they see fit.
    double idealized_utilization;
  };

  class Observer {
   public:
    // TODO(jophba): should stats be a seperate method or interface?
