...
class VideoEncoder {
  ...
  // The decision on what encoder to use comes from the FrameSenderConfig, which
  // comes from:
  // CastSession::StartVideo
  // CastRtpStream::Start
  // CastStreamingNativeHandler::StartCastRtpStream
  //   which comes from the cast.streaming.rtpStream method.

  // See offer answer in mirroring session:
  // https://osscs.corp.google.com/chromium/chromium/src/+/master:components/mirroring/service/session.cc;l=808
  static openscreen::ErrorOr<std::vector<SenderConfiguration>>
  GetSupportedConfigurations();

  // Create an encoder with the attached configuration.
