...
Clock::time_point NtpTimeConverter::ToLocalTime(NtpTimestamp timestamp) const {
  ...
  // Overly-pedantic future-proofing: If the NTP timestamp appears to be a
  // point-in-time before 1970, assume the 2036 wrap-around has occurred, and
  // adjust to compensate.
  //
  // You can judge me as crazy for writing this code in the year 2019; but, it
  // was necessary to remove the DCHECK() we had here before, in order to allow
  // fuzz testing to proceed.
