...
void ScrubMissingPacketVector(std::vector<PacketNack>* packets) {
  // First, sort all elements. This also allows the filtering algorithm below to
  // be simpler.
  ...
  //      redundant.
  //
  // This is done by walking backwards over the sorted vector and deciding which
  // elements to keep. Those that are kept are stacked-up at the back of the
  // vector. After the "to-keep" pass, the garbage at the front of the vector is
  // truncated.
  static_assert(kAllPacketsLost == std::numeric_limits<FramePacketId>::max(),
                "de-dupe algorithm assumes a kAllPacketsLost == uint16max.");
  auto it = packets->rbegin();
  if (it != packets->rend()) {
    auto kept_it = it;  // Always keep the last element.
    for (++it; it != packets->rend(); ++it) {
      if (it->frame_id != kept_it->frame_id ||
          (kept_it->packet_id != kAllPacketsLost &&
           it->packet_id != kept_it->packet_id)) {  // Keep it.
        ++kept_it;
        *kept_it = *it;
      }
    }
    packets->erase(packets->begin(), (++kept_it).base());
  }
}
