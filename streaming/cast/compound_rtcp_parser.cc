...
// Canonicalizes the just-parsed lists of frame-level and packet-specific NACKs
// so that the CompoundRtcpParser::Client can make several simplifying
// assumptions when processing the results.
void ScrubMissingFrameAndPacketVectors(
    std::vector<FrameId>* frames,
    std::vector<std::pair<FrameId, FramePacketId>>* packets) {
  // First, sort and de-duplicate all elements. This also allows the filtering
  // algorithm below to be simpler.
  SortAndDedupeElements(frames);
  SortAndDedupeElements(packets);

  // Overwrite all redundant entries in |packets| to a null representation, and
  // then remove the nulls.
  constexpr auto kNullId = std::make_pair(FrameId(), FramePacketId{0});
  for (size_t i = 0, j = 0; i != frames->size() && j != packets->size(); ++i) {
    for (; j != packets->size() && (*packets)[j].first < (*frames)[i]; ++j)
      ;
    for (; j != packets->size() && (*packets)[j].first == (*frames)[i]; ++j) {
      (*packets)[j] = kNullId;
    }
  }
  const auto new_end = std::remove(packets->begin(), packets->end(), kNullId);
...
bool CompoundRtcpParser::Parse(absl::Span<const uint8_t> buffer) {
  ...
  if (!received_frames_.empty()) {
    ...
    client_->OnReceiverHasFrames(received_frames_);
  }
  if (!missing_frames_.empty() || !missing_packets_.empty()) {
    ScrubMissingFrameAndPacketVectors(&missing_frames_, &missing_packets_);
    if (!missing_frames_.empty()) {
      client_->OnReceiverIsMissingFrames(missing_frames_);
    }
    if (!missing_packets_.empty()) {
      client_->OnReceiverIsMissingPackets(missing_packets_);
    }
  }
...
bool CompoundRtcpParser::ParseFeedback(absl::Span<const uint8_t> in) {
  ...
  for (int i = 0; i < loss_field_count; ++i) {
    ...
    if (packet_id == kAllPacketsLost) {
      missing_frames_.push_back(frame_id);
    } else {
      missing_packets_.emplace_back(frame_id, packet_id);
...
void CompoundRtcpParser::Client::OnReceiverHasFrames(
    absl::Span<const FrameId> frame_ids) {}
void CompoundRtcpParser::Client::OnReceiverIsMissingFrames(
    absl::Span<const FrameId> frame_ids) {}
