...
void CanonicalizePacketNackVector(std::vector<PacketNack>* packets) {
  ...
  std::sort(packets->begin(), packets->end(),
            [](const PacketNack& a, const PacketNack& b) {
              // When comparing the tuples, map all packet_id values to
              // packet_id + 1, mod 0x10000. This results in the desired sorting
              // behavior since kAllPacketsLost (0xffff) wraps-around to 0x0000,
              // and all other values become N + 1.
