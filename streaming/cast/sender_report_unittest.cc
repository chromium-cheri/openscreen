...
TEST_F(SenderReportTest, Parsing) {
  ...
  // A RTCP packet only containing non-sender-reports may still be detected as
  // "well-formed," but the parser will not provide a Sender Report result.
  {
    absl::optional<RtcpSenderReport> parsed;
    EXPECT_TRUE(parser()->Parse(kOtherPacket, &parsed));
  ...
  for (const auto& combo : kCompoundCombinations) {
    ...
    absl::optional<RtcpSenderReport> parsed;
    EXPECT_TRUE(parser()->Parse(compound_packet, &parsed));
    ...
    EXPECT_EQ(kReceiverSsrc, parsed->report_block->recipient_ssrc);
...
TEST_F(SenderReportTest, WillNotParseEmptyPacket) {
  ...
  absl::optional<RtcpSenderReport> parsed;
  EXPECT_TRUE(
      parser()->Parse(absl::Span<const uint8_t>(kEmptyPacket, 0), &parsed));
  EXPECT_FALSE(parsed.has_value());
}
...
TEST_F(SenderReportTest, WillNotParseGarbage) {
  ...
  // clang-format on
  absl::optional<RtcpSenderReport> parsed;
  EXPECT_FALSE(parser()->Parse(kGarbage, &parsed));
  EXPECT_FALSE(parsed.has_value());
}
...
TEST_F(SenderReportTest, BuildPackets) {
  for (int i = 0; i <= 1; ++i) {
    ...
    if (with_report_block) {
      ...
      report_block.recipient_ssrc = kReceiverSsrc;
    }
    ...
    absl::optional<RtcpSenderReport> parsed;
    EXPECT_TRUE(parser()->Parse(result.first, &parsed));
    ASSERT_TRUE(parsed.has_value());
    ...
    if (with_report_block) {
      ASSERT_TRUE(parsed->report_block.has_value());
      EXPECT_EQ(original.report_block->recipient_ssrc,
                parsed->report_block->recipient_ssrc);
