...
void Environment::ResumeIncomingPackets(PacketConsumer* packet_consumer) {
  ...
  if (socket_) {
    socket_->StartReceivingMessages();
  }
}
...
void Environment::OnRead(UdpSocket* socket,
  ...
  if (packet_consumer_) {
    ...
    // stack (e.g., by using the SO_TIMESTAMP sockopt on POSIX systems).
    // However, there would still be the problem of mapping the timestamp to a
    // value in terms of platform::Clock. So, just sample the Clock here and
    // call that the "arrival time." While this can add variance within the
    // system, it should be adequate, assuming not too much time has elapsed
    // between the actual packet receive event and the when this code here is
    // executing.
