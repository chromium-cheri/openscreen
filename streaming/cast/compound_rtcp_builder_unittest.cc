...
class CompoundRtcpBuilderTest : public testing::Test {
 public:
  ...
  // conversions.
  platform::Clock::time_point ViaNtpTimestampTranslation(
      platform::Clock::time_point timestamp) const {
...
TEST_F(CompoundRtcpBuilderTest, TheBasics) {
  ...
  const auto send_time = platform::Clock::now();
  ...
  parser()->SetMaxFeedbackFrameId(checkpoint);
  ...
  ASSERT_TRUE(parser()->Parse(packet));
}
...
TEST_F(CompoundRtcpBuilderTest, WithReceiverReportBlock) {
  ...
  parser()->SetMaxFeedbackFrameId(checkpoint + 2);
  EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(
                               ViaNtpTimestampTranslation(send_time)));
  EXPECT_CALL(*(client()), OnReceiverCheckpoint(checkpoint, playout_delay));
  ...
  ASSERT_TRUE(parser()->Parse(second_packet));
  Mock::VerifyAndClearExpectations(client());
}
...
TEST_F(CompoundRtcpBuilderTest, WithPictureLossIndicator) {
  ...
  auto send_time = platform::Clock::now();
  uint8_t buffer[CompoundRtcpBuilder::kRequiredBufferSize];
  for (int status = 0; status <= 3; ++status) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      parser()->SetMaxFeedbackFrameId(checkpoint + 1);
      EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(
                                   ViaNtpTimestampTranslation(send_time)));
      EXPECT_CALL(*(client()), OnReceiverCheckpoint(checkpoint, playout_delay));
      EXPECT_CALL(*(client()), OnReceiverLostPicture())
...
TEST_F(CompoundRtcpBuilderTest, WithNacks) {
  ...
  const FrameId kFrameNacks[] = {FrameId::first() + 18, FrameId::first() + 19};
  const std::pair<FrameId, FramePacketId> kPacketNacks[] = {
  ...
  builder()->IncludeFeedbackInNextPacket(kFrameNacks, kPacketNacks, {});
  ...
  // NACK feedback.
  parser()->SetMaxFeedbackFrameId(FrameId::first() + 19);
  EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(
                               ViaNtpTimestampTranslation(send_time)));
  EXPECT_CALL(*(client()), OnReceiverCheckpoint(checkpoint, playout_delay));
  EXPECT_CALL(*(client()),
              OnReceiverIsMissingFrames(absl::MakeConstSpan(kFrameNacks)));
  EXPECT_CALL(*(client()),
              OnReceiverIsMissingPackets(absl::MakeConstSpan(kPacketNacks)));
  ASSERT_TRUE(parser()->Parse(packet));
  Mock::VerifyAndClearExpectations(client());
  ...
  EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(
                               ViaNtpTimestampTranslation(second_send_time)));
  EXPECT_CALL(*(client()), OnReceiverCheckpoint(checkpoint, playout_delay));
  EXPECT_CALL(*(client()), OnReceiverIsMissingFrames(_)).Times(0);
...
TEST_F(CompoundRtcpBuilderTest, WithAcks) {
  ...
  // One frame ACK will result in building an ACK bit vector of 2 bytes only.
  const FrameId kOneAck[] = {FrameId::first() + 24};

  // These frame ACKs were chosen so that the ACK bit vector must expand to be 6
  // (2 + 4) bytes long.
  const FrameId kMultipleAcks[] = {FrameId::first() + 25, FrameId::first() + 42,
                                   FrameId::first() + 43};

  const absl::Span<const FrameId> kTestCases[] = {kOneAck, kMultipleAcks};
  auto send_time = platform::Clock::now();
  uint8_t buffer[CompoundRtcpBuilder::kRequiredBufferSize];
  for (const auto& frame_acks : kTestCases) {
    ...
    const auto packet = builder()->BuildPacket(send_time, buffer);
    ASSERT_TRUE(packet.data());
    parser()->SetMaxFeedbackFrameId(FrameId::first() + 50);
    EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(
                                 ViaNtpTimestampTranslation(send_time)));
    EXPECT_CALL(*(client()), OnReceiverCheckpoint(checkpoint, playout_delay));
...
TEST_F(CompoundRtcpBuilderTest, WithEverythingThatCanFit) {
  ...
  std::vector<std::pair<FrameId, FramePacketId>> nacks;
  for (FrameId f = checkpoint + 1; f != checkpoint + 64; ++f) {
    nacks.emplace_back(f, FramePacketId{0});
  }
  ...
  parser()->SetMaxFeedbackFrameId(checkpoint + kMaxUnackedFrames);
  ...
  uint8_t buffer[CompoundRtcpBuilder::kRequiredBufferSize];
  const auto packet = builder()->BuildPacket(platform::Clock::now(), buffer);
  ASSERT_TRUE(packet.data());
  ...
  EXPECT_CALL(*(client()), OnReceiverHasFrames(_)).Times(0);
  EXPECT_CALL(*(client()), OnReceiverIsMissingFrames(_)).Times(0);
  std::vector<std::pair<FrameId, FramePacketId>> parsed_nacks;
  EXPECT_CALL(*(client()), OnReceiverIsMissingPackets(_))
      .WillOnce(Invoke(
          [&](absl::Span<const std::pair<FrameId, FramePacketId>> packet_ids) {
            parsed_nacks.assign(packet_ids.begin(), packet_ids.end());
          }));
  ASSERT_TRUE(parser()->Parse(packet));
  Mock::VerifyAndClearExpectations(client());
  ASSERT_LT(parsed_nacks.size(), nacks.size());  // Some should be dropped.
  EXPECT_TRUE(
      std::equal(parsed_nacks.begin(), parsed_nacks.end(), nacks.begin()));
  ...
  builder()->IncludeFeedbackInNextPacket({}, {nacks.data(), kFewerNackCount},
                                         acks);
  const auto second_packet =
      builder()->BuildPacket(platform::Clock::now(), buffer);
  ASSERT_TRUE(second_packet.data());
  ...
  EXPECT_CALL(*(client()), OnReceiverReferenceTimeAdvanced(_));
  EXPECT_CALL(*(client()), OnReceiverCheckpoint(checkpoint, _));
  std::vector<FrameId> parsed_acks;
  EXPECT_CALL(*(client()), OnReceiverHasFrames(_))
      .WillOnce(Invoke([&](absl::Span<const FrameId> frame_ids) {
        parsed_acks.assign(frame_ids.begin(), frame_ids.end());
      }));
  EXPECT_CALL(*(client()), OnReceiverIsMissingFrames(_)).Times(0);
  EXPECT_CALL(*(client()), OnReceiverIsMissingPackets(_))
      .WillOnce(Invoke(
  ...
  EXPECT_TRUE(std::equal(parsed_acks.begin(), parsed_acks.end(), acks.begin()));
  ...
  builder()->IncludeFeedbackInNextPacket(
      {}, {nacks.data(), kEvenFewerNackCount}, acks);
  ...
  ASSERT_TRUE(parser()->Parse(third_packet));
  Mock::VerifyAndClearExpectations(client());
  ASSERT_EQ(kEvenFewerNackCount, static_cast<int>(parsed_nacks.size()));
  EXPECT_TRUE(
      std::equal(parsed_nacks.begin(), parsed_nacks.end(), nacks.begin()));
  ASSERT_EQ(parsed_acks.size(), acks.size());
  EXPECT_TRUE(std::equal(parsed_acks.begin(), parsed_acks.end(), acks.begin()));
}
