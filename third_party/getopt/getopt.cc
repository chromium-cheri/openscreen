...
static int getopt_internal(int argc,
  ...
  if (longopts != NULL &&
    ...
    if (strncmp(argv[optind], "--", 2) == 0) {
      optwhere = 2;
      std::cout << ": has a long option= " << argv[optind];
    }
    longopt_match = -1;
    ...
    for (optindex = 0; longopts[optindex].name != NULL; optindex++) {
      if (strncmp(argv[optind] + optwhere, longopts[optindex].name,
                  match_chars) == 0) {
  ...
  if (longopt_match < 0 && shortopts != NULL) {
    ...
    has_arg = ((cp[1] == ':')
                   ? ((cp[2] == ':') ? optional_argument : required_argument)
                   : no_argument);
