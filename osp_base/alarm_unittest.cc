...
TEST_F(AlarmTest, RunsTaskAsClockAdvances) {
  ...
  alarm()->Arm([&]() { actual_run_time = platform::FakeClock::now(); },
               alarm_time);
...
TEST_F(AlarmTest, CancelsTaskWhenGoingOutOfScope) {
  ...
  platform::Clock::time_point actual_run_time{};
  {
    ...
    const platform::Clock::time_point alarm_time =
        platform::FakeClock::now() + kDelay;
    scoped_alarm.Arm([&]() { actual_run_time = platform::FakeClock::now(); },
                     alarm_time);
...
TEST_F(AlarmTest, Disarms) {
  ...
  // Disarm and then advance the clock well past the delay, and confirm the
...
TEST_F(AlarmTest, CancelsAndRearms) {
  ...
  // Run the test twice: Once when arming first with a long delay, then a
  // shorter delay; and once when arming first with a short delay, then a longer
  // delay. This is to test Alarm's internal scheduling/firing logic.
  for (int do_longer_then_shorter = 0; do_longer_then_shorter <= 1;
    ...
    int count1 = 0;
    alarm()->Arm([&]() { ++count1; }, platform::FakeClock::now() + delay1);
    ...
    // Arm a different lambda, that increments a different variable, to run
    // after |delay2|.
    int count2 = 0;
    alarm()->Arm([&]() { ++count2; }, platform::FakeClock::now() + delay2);

    // Confirm the second arming will fire at the right moment.
    clock()->Advance(delay2 / 2);
    ASSERT_EQ(0, count2);
    clock()->Advance(delay2 / 2);
    ASSERT_EQ(1, count2);

    // Confirm the second arming never fires a second time, and also that the
    // first one doesn't fire.
