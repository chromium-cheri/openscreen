...
// automatically canceled. It is safe for the client's Task to make re-entrant
// calls into all Alarm methods.
//
...
// the client, the Alarm posts a cancelable functor to the TaskRunner which,
// when invoked, then checks to see whether the Alarm instance still exists and,
// if so, calls its Fire() method. The Fire() method then determines: a) whether
// the invocation time of the client's Task has changed; and b) whether the
// Alarm was disarmed in the meantime. From this, it either: a) does nothing; b)
// re-posts a new cancelable functor to the TaskRunner, to try running the
// client's Task later; or c) runs the client's Task.
class Alarm {
 public:
 ...
 private:
  ...
  // "firing" to do it later; or just do nothing. See class-level design
  // comments.
  void Fire();
  ...
  Task client_task_;
  platform::Clock::time_point client_task_invoke_time_{};

  // When non-null, there is a task in the TaskRunner's queue that will call
  // Fire() some time in the future. This member should is exclusively
  // maintained by the CancelableFunctor class methods.
  CancelableFunctor* pending_fire_ = nullptr;
