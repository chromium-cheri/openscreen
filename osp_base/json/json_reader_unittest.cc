...
TEST(JsonReaderTest, ValidEmptyDocumentParsedCorrectly) {
  ...
  ASSERT_EQ(actual.value().getMemberNames().size(), 0);
}
...
TEST(JsonReaderTest, ValidDocumentParsedCorrectly) {
  ...
  const auto actual = reader.Read("{\"foo\": \"bar\", \"baz\": 1337}");

  ASSERT_TRUE(actual.is_value());
  ASSERT_EQ(actual.value().getMemberNames().size(), 2);
}
