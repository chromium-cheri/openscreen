...
class ReceiverConnectionDelegate final
 ...
 public:
  ...
  void OnTerminatedByRemote() override {
    OSP_LOG_INFO << "presentation terminated";
  }
...
void HandleReceiverCommand(absl::string_view command,
  ...
  if (command == "avail") {
    ...
    delegate.connection->SendString(argument);
...
void PollOnFileDescriptor(pollfd& file_descriptor,
                          NetworkServiceManager* manager,
                          ReceiverDelegate& delegate) {
...
void PublisherDemo(absl::string_view friendly_name) {
  ...
  // TODO(btolsch): Multihomed support is broken due to ServiceInfo holding
  // only one address.
  ...
  PollOnFileDescriptor(stdin_pollfd, network_service, receiver_delegate);
...
int main(int argc, char** argv) {
  ...
  const bool is_receiver_demo = (argc > 1);
