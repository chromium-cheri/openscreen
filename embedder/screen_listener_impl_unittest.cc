...
TEST(ScreenListenerImplTest, NormalStartStop) {
  ...
  ASSERT_EQ(ScreenListenerState::kStopped, screen_listener->state());
  EXPECT_TRUE(screen_listener->Start());
  EXPECT_FALSE(screen_listener->Start());
  EXPECT_EQ(ScreenListenerState::kStopped, screen_listener->state());
  ...
  screen_listener->SetState(ScreenListenerState::kStarting);
  EXPECT_EQ(ScreenListenerState::kStarting, screen_listener->state());
  screen_listener->SetState(ScreenListenerState::kRunning);
  EXPECT_EQ(ScreenListenerState::kRunning, screen_listener->state());
  ...
  screen_listener->SetState(ScreenListenerState::kStopping);
  EXPECT_EQ(ScreenListenerState::kStopping, screen_listener->state());
  screen_listener->SetState(ScreenListenerState::kStopped);
  EXPECT_EQ(ScreenListenerState::kStopped, screen_listener->state());
}
...
TEST(ScreenListenerImplTest, StartSuspended) {
  ...
  screen_listener->SetState(ScreenListenerState::kStarting);
  EXPECT_EQ(ScreenListenerState::kStarting, screen_listener->state());
  screen_listener->SetState(ScreenListenerState::kSuspended);
  EXPECT_EQ(ScreenListenerState::kSuspended, screen_listener->state());
...
TEST(ScreenListenerImplTest, SearchWhileRunning) {
  ...
  screen_listener->SetState(ScreenListenerState::kSearching);
  EXPECT_EQ(ScreenListenerState::kSearching, screen_listener->state());
...
TEST(ScreenListenerImplTest, ObserveTransitions) {
  ...
  screen_listener->SetState(ScreenListenerState::kSearching);
  EXPECT_CALL(observer, OnStarted()).Times(0);
  screen_listener->SetState(ScreenListenerState::kRunning);
