...
#ifndef API_PUBLIC_RECEIVER_LISTENER_H_
#define API_PUBLIC_RECEIVER_LISTENER_H_
...
struct ReceiverListenerError {
 public:
  ...
  ReceiverListenerError();
  ReceiverListenerError(Code error, const std::string& message);
  ReceiverListenerError(const ReceiverListenerError& other);
  ~ReceiverListenerError();

  ReceiverListenerError& operator=(const ReceiverListenerError& other);
...
class ReceiverListener {
 public:
  ...
  // behavior of a ReceiverListener instance.
  struct Metrics {
    Metrics();
    ~Metrics();
  ...
  class Observer {
   public:
    ...
    virtual void OnReceiverAdded(const ReceiverInfo&) = 0;
    virtual void OnReceiverChanged(const ReceiverInfo&) = 0;
    virtual void OnReceiverRemoved(const ReceiverInfo&) = 0;
    ...
    // Reports an error.
    virtual void OnError(ReceiverListenerError) = 0;
  ...
  const ReceiverListenerError& last_error() const { return last_error_; }

  // Returns the current list of receivers known to the ReceiverListener.
  virtual const std::vector<ReceiverInfo>& GetReceivers() const = 0;

 protected:
  explicit ReceiverListener(Observer* observer);

  State state_ = State::kStopped;
  ReceiverListenerError last_error_;
  Observer* const observer_;

  DISALLOW_COPY_AND_ASSIGN(ReceiverListener);
};

}  // namespace openscreen

#endif  // API_PUBLIC_RECEIVER_LISTENER_H_
