...
class Connection {
 public:
  ...
  const std::string& presentation_id() const { return presentation_.id; }
  const std::string& presentation_url() const { return presentation_.url; }
...
class ConnectionManager final : public MessageDemuxer::MessageCallback {
 ...
 private:
  ...
  ErrorOr<ConnectionState*> GetConnectionState(
      const std::string& presentation_id,
      uint64_t connection_id);

  // <presentation id, connection id> -> ConnectionState
  std::map<std::pair<std::string, uint64_t>, ConnectionState> connections_;
