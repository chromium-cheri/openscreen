...
struct StartError {
  StartError();
  StartError(StartErrorCode error, const std::string& message);
  StartError(const StartError& other);
  ~StartError();

  StartError& operator=(const StartError& other);

  StartErrorCode error;
  std::string message;
};
