...
// ScreenConnection object, although the information known about the other party
// may not be symmetrical.
//
// A ScreenConnection supports multiple protocols defined by the Open Screen
...
class ScreenConnectionObserver {
 public:
  ...
  // Called when a new connection was created between a pair of 5-tuples.
  virtual void OnConnectionAdded(const ScreenConnection& connection) = 0;
  // Called when the state of |connection| has changed.
  virtual void OnConnectionChanged(const ScreenConnection& connection) = 0;
  ...
  virtual void OnConnectionRemoved(const ScreenConnection& connection) = 0;

 protected:
  virtual ~ScreenConnectionObserver() = default;
};
