...
enum class ScreenPublisherState {
  kStopped = 0,
  kStarting,
  kRunning,
  kStopping,
...
enum class ScreenPublisherError {
  kNone = 0,
};

// Used to report an error from a ScreenListener implementation.
struct ScreenPublisherErrorInfo {
 public:
  ScreenPublisherErrorInfo();
  ScreenPublisherErrorInfo(ScreenPublisherError error,
                           const std::string& message);
  ScreenPublisherErrorInfo(const ScreenPublisherErrorInfo& other);
  ~ScreenPublisherErrorInfo();

  ScreenPublisherErrorInfo& operator=(const ScreenPublisherErrorInfo& other);

  ScreenPublisherError error;
  std::string message;
};
...
struct ScreenPublisherMetrics {
  ScreenPublisherMetrics();
  ~ScreenPublisherMetrics();
...
class ScreenPublisherObserver {
 public:
  virtual ~ScreenPublisherObserver() = default;
  ...
  // Reports an error.
  virtual void OnError(ScreenPublisherErrorInfo) = 0;

  // Reports metrics.
  virtual void OnMetrics(ScreenPublisherMetrics) = 0;
};

struct ScreenPublisherCommonConfig {
  ScreenPublisherCommonConfig();
  ~ScreenPublisherCommonConfig();

  // The human readable friendly name of the screen being published.
  ...
  // TODO: Figure out a way to specify this in a cross-platform way.
...
class ScreenPublisher {
 public:
  ...
  ScreenPublisherState state() const { return state_; }

  // Returns the last error reported by this publisher.
  ScreenPublisherErrorInfo GetLastError() const { return last_error_; }

 protected:
  explicit ScreenPublisher(ScreenPublisherObserver* observer);

  ScreenPublisherState state_ = ScreenPublisherState::kStopped;
  ScreenPublisherErrorInfo last_error_;
  ScreenPublisherObserver* observer_;
