...
class NetworkServiceManager final {
 public:
  ...
  static NetworkControlService* Create(
  ...
  // created by Create().
  static NetworkControlService* Get();
 ...
 private:
  ...
  const MdnsScreenListener* mdns_listener_;
  const MdnsScreenPublisher* mdns_publisher_;
  const ScreenConnectionClient* connection_client_;
  const ScreenConnectionServer* connection_server_;
}
