...
TEST(TaskRunnerTest, TaskRunnerFromFactoryExecutesTask) {
  ...
  std::thread t(
      [&runner] { static_cast<TaskRunnerImpl*>(runner.get())->Start(); });
  ...
  static_cast<TaskRunnerImpl*>(runner.get())->Stop();
  t.join();
}

TEST(TaskRunnerTest, TaskRunnerRunsDelayedTasks) {
  ...
  const auto kMinimumClockTaskShouldRunAt = platform::Clock::now() + kDelayTime;
  runner->PostTaskWithDelay(task, kDelayTime);

  while (platform::Clock::now() < kMinimumClockTaskShouldRunAt) {
    EXPECT_EQ(ran_tasks, "");
    std::this_thread::sleep_for(kTaskRunnerSleepTime);
  }
