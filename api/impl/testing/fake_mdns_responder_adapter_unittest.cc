...
TEST(FakeMdnsResponderAdapterTest, AQueries) {
  ...
  auto result = mdns_responder.StartAQuery(domain_name, default_socket);
  ...
  result = mdns_responder.StopAQuery(domain_name, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StartAQuery(domain_name, default_socket);
...
TEST(FakeMdnsResponderAdapterTest, AaaaQueries) {
  ...
  auto result = mdns_responder.StartAaaaQuery(domain_name, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StopAaaaQuery(domain_name, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StartAaaaQuery(domain_name, default_socket);
  EXPECT_NE(mdns::MdnsResponderErrorCode::kNoError, result);
  aaaa_events = mdns_responder.TakeAaaaResponses();
  EXPECT_TRUE(aaaa_events.empty());
}
...
TEST(FakeMdnsResponderAdapterTest, PtrQueries) {
  ...
  auto result = mdns_responder.StartPtrQuery(kTestServiceType, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StopPtrQuery(kTestServiceType, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  mdns_responder.AddPtrEvent(
      MakePtrEvent(kTestServiceInstance, kTestServiceName, kTestServiceProtocol,
                   default_socket));
  result = mdns_responder.StartPtrQuery(kTestServiceType, default_socket);
  EXPECT_NE(mdns::MdnsResponderErrorCode::kNoError, result);
  ptr_events = mdns_responder.TakePtrResponses();
  EXPECT_TRUE(ptr_events.empty());
}
...
TEST(FakeMdnsResponderAdapterTest, SrvQueries) {
  ...
  auto result = mdns_responder.StartSrvQuery(service_instance, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StopSrvQuery(service_instance, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  mdns_responder.AddSrvEvent(
      MakeSrvEvent(kTestServiceInstance, kTestServiceName, kTestServiceProtocol,
                   "alpha", 12345, default_socket));
  result = mdns_responder.StartSrvQuery(service_instance, default_socket);
  EXPECT_NE(mdns::MdnsResponderErrorCode::kNoError, result);
  srv_events = mdns_responder.TakeSrvResponses();
  EXPECT_TRUE(srv_events.empty());
}
...
TEST(FakeMdnsResponderAdapterTest, TxtQueries) {
  ...
  auto result = mdns_responder.StartTxtQuery(service_instance, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StopTxtQuery(service_instance, default_socket);
  EXPECT_EQ(mdns::MdnsResponderErrorCode::kNoError, result);
  ...
  result = mdns_responder.StartTxtQuery(service_instance, default_socket);
  EXPECT_NE(mdns::MdnsResponderErrorCode::kNoError, result);
  txt_events = mdns_responder.TakeTxtResponses();
  EXPECT_TRUE(txt_events.empty());
}
