...
TEST(FakeMdnsResponderAdapterTest, AQueries) {
  ...
  auto event = MakeAExample("alpha", IPv4Address{1, 2, 3, 4},
                            reinterpret_cast<platform::UdpSocketIPv4Ptr>(8));
  ...
  EXPECT_EQ((IPv4Address{1, 2, 3, 4}), a_events[0].address);
  ...
  mdns_responder.AddAEvent(
      MakeAExample("alpha", IPv4Address{1, 2, 3, 4},
                   reinterpret_cast<platform::UdpSocketIPv4Ptr>(8)));
...
TEST(FakeMdnsResponderAdapterTest, PtrQueries) {
  ...
  mdns_responder.Init();
  ASSERT_TRUE(mdns_responder.running());
  mdns_responder.AddPtrEvent(MakePtrExample(
      kTestServiceInstance, kTestServiceName, kTestServiceProtocol,
      reinterpret_cast<platform::UdpSocketIPv4Ptr>(8)));
...
TEST(FakeMdnsResponderAdapterTest, SrvQueries) {
  ...
  auto event = MakeSrvExample(kTestServiceInstance, kTestServiceName,
  ...
  mdns_responder.AddSrvEvent(MakeSrvExample(
      kTestServiceInstance, kTestServiceName, kTestServiceProtocol, "alpha",
      12345, reinterpret_cast<platform::UdpSocketIPv4Ptr>(16)));
...
TEST(FakeMdnsResponderAdapterTest, TxtQueries) {
  ...
  auto event = MakeTxtExample(kTestServiceInstance, kTestServiceName,
                              kTestServiceProtocol, txt_lines,
                              reinterpret_cast<platform::UdpSocketIPv4Ptr>(8));
  auto service_instance = event.service_instance;
  ...
  mdns_responder.AddTxtEvent(MakeTxtExample(
      kTestServiceInstance, kTestServiceName, kTestServiceProtocol, txt_lines,
      reinterpret_cast<platform::UdpSocketIPv4Ptr>(8)));
...
TEST(FakeMdnsResponderAdapterTest, RegisterInterfaces) {
  ...
  auto socket2 = reinterpret_cast<platform::UdpSocketIPv4Ptr>(24);
  auto result = mdns_responder.RegisterInterface(
      platform::InterfaceInfo{}, platform::IPv4Subnet{},
      socket1);
  EXPECT_TRUE(result);
