...
class UrlAvailabilityRequesterTest : public ::testing::Test {
 ...
 protected:
  ...
  std::string receiver_id_{"asdf"};
  std::string friendly_name_{"turtle"};
  ReceiverInfo info1_{receiver_id_, friendly_name_, 1, receiver_endpoint_};
};
...
TEST_F(UrlAvailabilityRequesterTest, AvailableObserverFirst) {
  ...
  EXPECT_CALL(mock_observer, OnReceiverAvailable(url1_, receiver_id_));
  EXPECT_CALL(mock_observer, OnReceiverUnavailable(url1_, receiver_id_))
      .Times(0);
  RunTasksUntilIdle();
}
...
TEST_F(UrlAvailabilityRequesterTest, Unavailable) {
  ...
  EXPECT_CALL(mock_observer, OnReceiverAvailable(url1_, receiver_id_)).Times(0);
  EXPECT_CALL(mock_observer, OnReceiverUnavailable(url1_, receiver_id_));
  RunTasksUntilIdle();
}
...
TEST_F(UrlAvailabilityRequesterTest, AvailabilityIsCached) {
  ...
  EXPECT_CALL(mock_observer1, OnReceiverAvailable(url1_, receiver_id_))
      .Times(0);
  EXPECT_CALL(mock_observer1, OnReceiverUnavailable(url1_, receiver_id_));
  RunTasksUntilIdle();

  MockReceiverObserver mock_observer2;
  EXPECT_CALL(mock_observer2, OnReceiverAvailable(url1_, receiver_id_))
      .Times(0);
  EXPECT_CALL(mock_observer2, OnReceiverUnavailable(url1_, receiver_id_));
...
TEST_F(UrlAvailabilityRequesterTest, PartiallyCachedAnswer) {
  ...
  EXPECT_CALL(mock_observer2, OnReceiverAvailable(url2_, receiver_id_))
      .Times(0);
  EXPECT_CALL(mock_observer2, OnReceiverUnavailable(url2_, receiver_id_));
  RunTasksUntilIdle();
}
...
TEST_F(UrlAvailabilityRequesterTest, MultipleOverlappingObservers) {
  ...
  EXPECT_CALL(mock_observer1, OnReceiverUnavailable(_, receiver_id_)).Times(0);
  EXPECT_CALL(mock_observer2, OnReceiverAvailable(_, receiver_id_)).Times(0);
  EXPECT_CALL(mock_observer2, OnReceiverUnavailable(url2_, receiver_id_));
  RunTasksUntilIdle();
}
...
TEST_F(UrlAvailabilityRequesterTest, RemoveObserver) {
  ...
  EXPECT_CALL(mock_observer1, OnReceiverUnavailable(_, receiver_id_)).Times(0);
  EXPECT_CALL(mock_observer2, OnReceiverUnavailable(_, receiver_id_)).Times(0);
  RunTasksUntilIdle();
}
...
TEST_F(UrlAvailabilityRequesterTest, EventUpdate) {
  ...
  EXPECT_CALL(mock_observer1, OnReceiverAvailable(url1_, receiver_id_));
  EXPECT_CALL(mock_observer1, OnReceiverAvailable(url2_, receiver_id_));
  EXPECT_CALL(mock_observer1, OnReceiverUnavailable(_, receiver_id_)).Times(0);
  RunTasksUntilIdle();
  ...
  EXPECT_CALL(mock_observer1, OnReceiverUnavailable(url2_, receiver_id_));
  RunTasksUntilIdle();
}
