...
void UrlAvailabilityRequester::AddObserver(const std::vector<std::string>& urls,
  ...
  for (auto& entry : receiver_by_receiver_id_) {
...
void UrlAvailabilityRequester::AddReceiver(const ReceiverInfo& info) {
  auto result = receiver_by_receiver_id_.emplace(
      info.receiver_id,
      MakeUnique<ReceiverRequester>(
          this, info.receiver_id,
...
void UrlAvailabilityRequester::ChangeReceiver(const ReceiverInfo& info) {}

void UrlAvailabilityRequester::RemoveReceiver(const ReceiverInfo& info) {
  auto receiver_entry = receiver_by_receiver_id_.find(info.receiver_id);
  if (receiver_entry != receiver_by_receiver_id_.end()) {
    auto& receiver = receiver_entry->second;
    receiver->RemoveReceiver();
    receiver_by_receiver_id_.erase(receiver_entry);
  }
}
...
void UrlAvailabilityRequester::RemoveAllReceivers() {
  ...
  receiver_by_receiver_id_.clear();
}
    ...
    if (observer) {
      switch (availability) {
        case msgs::kCompatible:
          observer->OnReceiverAvailable(url, receiver_id);
          break;
        case msgs::kNotCompatible:
        case msgs::kNotValid:
          observer->OnReceiverUnavailable(url, receiver_id);
          break;
      }
    }
  }
  ...
  if (!connection ||
    ...
    for (const auto& url : urls)
      for (auto& observer : listener->observers_by_url_[url])
        observer->OnRequestFailed(url, receiver_id);
  }
}
...
void UrlAvailabilityRequester::ReceiverRequester::OnConnectionFailed(
  ...
  std::string id = std::move(receiver_id);
  listener->receiver_by_receiver_id_.erase(id);
}
