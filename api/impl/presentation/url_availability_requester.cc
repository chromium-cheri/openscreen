...
void UrlAvailabilityRequester::AddObserver(const std::vector<std::string>& urls,
  ...
  for (auto& entry : client_by_screen_id_) {
    auto& client = entry.second;
    client->GetOrRequesetAvailabilities(urls, observer);
  }
}

void UrlAvailabilityRequester::RemoveObserver(
  ...
  for (const auto& url : urls) {
    ...
    if (observers.empty()) {
      ...
      for (auto& entry : client_by_screen_id_) {
        auto& client = entry.second;
        client->known_availability_by_url.erase(url);
      }
    }
  }
  ...
  for (auto& entry : client_by_screen_id_) {
    auto& client = entry.second;
    client->RemoveUnobservedRequests(unobserved_urls);
    client->RemoveUnobservedWatches(unobserved_urls);
  }
}

void UrlAvailabilityRequester::AddScreen(const ScreenInfo& info) {
  auto result = client_by_screen_id_.emplace(
      info.screen_id,
      MakeUnique<ScreenRequester>(
          this, info.screen_id,
          info.v4_endpoint.address ? info.v4_endpoint : info.v6_endpoint));
  std::unique_ptr<ScreenRequester>& client = result.first->second;
  std::vector<std::string> urls;
  urls.reserve(observers_by_url_.size());
  for (const auto& url : observers_by_url_)
    urls.push_back(url.first);
  client->RequestUrlAvailabilities(std::move(urls));
}
...
void UrlAvailabilityRequester::RemoveScreen(const ScreenInfo& info) {
  auto client_entry = client_by_screen_id_.find(info.screen_id);
  if (client_entry != client_by_screen_id_.end()) {
    auto& client = client_entry->second;
    client->RemoveScreen();
    client_by_screen_id_.erase(client_entry);
  }
}
...
platform::TimeDelta UrlAvailabilityRequester::RefreshWatches() {
  ...
  for (auto& entry : client_by_screen_id_) {
    auto& client = entry.second;
    platform::TimeDelta requested_schedule_time = client->RefreshWatches(now);
...
platform::TimeDelta UrlAvailabilityRequester::ScreenRequester::RefreshWatches(
  ...
  if (watch_by_id.empty())
    event_watch = MessageDemuxer::MessageWatch();
...
void UrlAvailabilityRequester::ScreenRequester::RemoveUnobservedRequests(
  ...
  for (auto entry = request_by_id.begin(); entry != request_by_id.end();) {
    ...
    auto split = std::partition(request.urls.begin(), request.urls.end(),
                                [&unobserved_urls](const std::string& url) {
                                  return unobserved_urls.find(url) ==
                                         unobserved_urls.end();
                                });
    if (split != request.urls.end()) {
      uint64_t request_id = entry->first;
      std::vector<std::string> remaining_urls;
      remaining_urls.reserve(std::distance(split, request.urls.end()));
      for (auto it = request.urls.begin(); it != split; ++it)
        remaining_urls.emplace_back(std::move(*it));
      if (!remaining_urls.empty()) {
        uint64_t new_request_id = next_request_id++;
        // TODO(btolsch): Just call RequestUrlAvailabilities if modifying
        // std::map while iterating is okay in libc++.
        ErrorOr<uint64_t> watch_id_or_error(0);
        if (!connection ||
            (watch_id_or_error = SendRequest(new_request_id, remaining_urls))
                .is_value()) {
          new_requests.emplace(
              new_request_id,
              Request{watch_id_or_error.value(), std::move(remaining_urls)});
        } else {
      ...
      if (connection) {
        watch_by_id.erase(entry->second.watch_id);
        orphaned_request_ids.insert(request_id);
      }
...
void UrlAvailabilityRequester::ScreenRequester::RemoveUnobservedWatches(
  ...
  for (auto entry = watch_by_id.begin(); entry != watch_by_id.end();) {
    Watch& watch = entry->second;
    auto split = std::partition(watch.urls.begin(), watch.urls.end(),
                                [&unobserved_urls](const std::string& url) {
                                  return unobserved_urls.find(url) ==
                                         unobserved_urls.end();
                                });
    if (split != watch.urls.end()) {
      std::vector<std::string> remaining_urls;
      remaining_urls.reserve(std::distance(split, watch.urls.end()));
      for (auto it = watch.urls.begin(); it != split; ++it)
        remaining_urls.emplace_back(std::move(*it));
      RequestUrlAvailabilities(std::move(remaining_urls));
      entry = watch_by_id.erase(entry);
    } else {
      ++entry;
    }
...
void UrlAvailabilityRequester::ScreenRequester::OnConnectionFailed(
  ...
  listener->client_by_screen_id_.erase(id);
}
    ...
    case msgs::Type::kPresentationUrlAvailabilityResponse: {
      ...
      if (result < 0) {
        ...
        if (request_entry == request_by_id.end()) {
          auto orphan_entry = orphaned_request_ids.find(response.request_id);
          if (orphan_entry == orphaned_request_ids.end()) {
            OSP_LOG_WARN << "bad response id: " << response.request_id;
            return Error::Code::kCborInvalidResponseId;
          }
          orphaned_request_ids.erase(orphan_entry);
          return result;
        }
