...
class MockReceiverDelegate final : public ReceiverDelegate {
 public:
  ...
  std::vector<msgs::PresentationUrlAvailability> OnUrlAvailabilityRequest(
    ...
    return OnUrlAvailabilityRequestMock(client_id, request_duration, &urls);
  }
  MOCK_METHOD3(OnUrlAvailabilityRequestMock,
  ...
  MOCK_METHOD3(StartPresentation,
               bool(const Connection::Info& info,
                    uint64_t source_id,
...
TEST_F(PresentationReceiverTest, QueryAvailability) {
  ...
  EXPECT_CALL(mock_receiver_delegate_, OnUrlAvailabilityRequestMock(_, _, _))
      .WillOnce(Invoke([this](uint64_t client_id, uint64_t request_duration,
                              std::vector<std::string>* urls) {
...
TEST_F(PresentationReceiverTest, StartPresentation) {
  ...
  const std::string presentation_id = "12345deadbeef";
  ...
  Connection connection(Connection::Info{presentation_id, url1_},
                        &null_connection_delegate, Connection::Role::kReceiver);
