...
void EncodePresentationInitiationResponse(
    const msgs::PresentationInitiationResponse& response,
    ProtocolConnection* stream) {
  msgs::CborEncodeBuffer buffer;
  if (msgs::EncodePresentationInitiationResponse(response, &buffer))
    stream->Write(buffer.data(), buffer.size());
  else
    OSP_LOG_WARN << "encode presentation-initiation-response error";
}

void EncodePresentationConnectionOpenResponse(
    const msgs::PresentationConnectionOpenResponse& response,
    ProtocolConnection* stream) {
  msgs::CborEncodeBuffer buffer;
  if (msgs::EncodePresentationConnectionOpenResponse(response, &buffer))
    stream->Write(buffer.data(), buffer.size());
  else
    OSP_LOG_WARN << "encode presentation-connection-open-response error";
}

void EncodePresentationTerminationResponse(
    const msgs::PresentationTerminationResponse& response,
    ProtocolConnection* stream) {
  msgs::CborEncodeBuffer buffer;
  if (msgs::EncodePresentationTerminationResponse(response, &buffer))
    stream->Write(buffer.data(), buffer.size());
  else
    OSP_LOG_WARN << "encode presentation-termination-response error";
}
  ...
  switch (message_type) {
    case msgs::Type::kPresentationUrlAvailabilityRequest: {
      ...
      if (decode_result < 0) {
        ...
        response.url_availabilities =
            delegate_->OnUrlAvailabilityRequest(0, 0, std::move(request.urls));
        msgs::CborEncodeBuffer buffer;
        if (msgs::EncodePresentationUrlAvailabilityResponse(response,
                                                            &buffer)) {
          OSP_VLOG(1) << "writing presentation-url-availability-response";
    ...
    case msgs::Type::kPresentationInitiationRequest: {
      ...
      if (result < 0) {
        ...
        auto emplace_result = initiation_responses_.emplace(
            request.presentation_id,
            InitiationResponse{NetworkServiceManager::Get()
                                   ->GetProtocolConnectionServer()
                                   ->CreateProtocolConnection(endpoint_id),
                               request.request_id, request.connection_id,
                               endpoint_id, request.url});
        if (!emplace_result.second) {
          ...
          EncodePresentationInitiationResponse(
              response, NetworkServiceManager::Get()
                            ->GetProtocolConnectionServer()
                            ->CreateProtocolConnection(endpoint_id)
                            .get());
          return result;
        }
        bool starting = delegate_->StartPresentation(
        ...
        initiation_responses_.erase(request.presentation_id);
        msgs::PresentationInitiationResponse response;
        response.request_id = request.request_id;
        response.result =
    ...
    case msgs::Type::kPresentationConnectionOpenRequest: {
      ...
      ssize_t result = msgs::DecodePresentationConnectionOpenRequest(
          buffer, buffer_size, &request);
      if (result < 0) {
        ...
        if (presentations_.find(request.presentation_id) ==
          ...
          EncodePresentationConnectionOpenResponse(
              response, NetworkServiceManager::Get()
                            ->GetProtocolConnectionServer()
                            ->CreateProtocolConnection(endpoint_id)
                            .get());
        ...
        // CDDL messages.
        std::vector<ConnectionResponse>& responses =
            connection_responses_[request.presentation_id];
        responses.emplace_back(ConnectionResponse{
            NetworkServiceManager::Get()
                ->GetProtocolConnectionServer()
                ->CreateProtocolConnection(endpoint_id),
        ...
        if (responses.empty())
          connection_responses_.erase(request.presentation_id);
        msgs::PresentationConnectionOpenResponse response;
        response.request_id = request.request_id;
        response.result =
            static_cast<decltype(response.result)>(msgs::kUnknownError);
    ...
    case msgs::Type::kPresentationTerminationRequest: {
      ...
      if (result < 0) {
        ...
        if (presentation_entry == presentations_.end()) {
          ...
          response.result = static_cast<decltype(response.result)>(
              msgs::kInvalidPresentationId);
          EncodePresentationTerminationResponse(
              response, NetworkServiceManager::Get()
                            ->GetProtocolConnectionServer()
                            ->CreateProtocolConnection(endpoint_id)
                            .get());
...
void Receiver::Init() {
  if (!connection_manager_) {
    connection_manager_ =
        MakeUnique<ConnectionManager>(NetworkServiceManager::Get()
                                          ->GetProtocolConnectionServer()
                                          ->message_demuxer());
  }
}
...
void Receiver::OnPresentationStarted(const std::string& presentation_id,
  ...
  auto it = initiation_responses_.find(presentation_id);
  if (it == initiation_responses_.end())
    return;
  InitiationResponse& initiation_response = it->second;
  msgs::PresentationInitiationResponse response;
  response.request_id = initiation_response.request_id;
  ProtocolConnection* stream = initiation_response.stream.get();
  OSP_VLOG(1) << "presentation started with stream id: "
  ...
  EncodePresentationInitiationResponse(response, stream);
  initiation_responses_.erase(it);
}

void Receiver::OnConnectionCreated(uint64_t request_id,
                                   Connection* connection,
                                   ResponseResult result) {
  auto entry = connection_responses_.find(connection->id());
  if (entry == connection_responses_.end()) {
    OSP_LOG_WARN << "connection created for unknown request";
    return;
  }
  std::vector<ConnectionResponse>& responses = entry->second;
  auto it = std::find_if(responses.begin(), responses.end(),
                         [request_id](const ConnectionResponse& response) {
  ...
  ConnectionResponse& connection_response = *it;
  ...
  presentations_[connection->id()].connections.push_back(connection);
  connection_manager_->AddConnection(connection);

  msgs::PresentationConnectionOpenResponse response;
  response.request_id = request_id;
  response.result = static_cast<decltype(response.result)>(msgs::kSuccess);
  EncodePresentationConnectionOpenResponse(response,
  ...
  if (responses.empty())
    connection_responses_.erase(entry);
}
...
void Receiver::OnPresentationTerminated(const std::string& presentation_id,
  ...
  std::unique_ptr<ProtocolConnection> stream =
      NetworkServiceManager::Get()
          ->GetProtocolConnectionServer()
          ->CreateProtocolConnection(presentation.endpoint_id);
  if (!stream)
    return;
  ...
  if (presentation.terminate_request_id) {
    ...
    response.result = static_cast<decltype(response.result)>(msgs::kSuccess);
    EncodePresentationTerminationResponse(response, stream.get());
...
void Receiver::OnConnectionDestroyed(Connection* connection) {
  auto presentation_entry = presentations_.find(connection->id());
  if (presentation_entry == presentations_.end())
    return;
  Presentation& presentation = presentation_entry->second;
