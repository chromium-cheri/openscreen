...
class Controller::MessageGroupStreams final
    : public ProtocolConnectionClient::ConnectionRequestCallback,
      ...
      public InitiationRequestResponseHandler::Delegate,
      public TerminationRequestResponseHandler::Delegate {
 public:
  MessageGroupStreams(Controller* controller, const std::string& service_id);

  uint64_t SendInitiationRequest(InitiationRequest request);
 ...
 private:
  ...
  InitiationRequestResponseHandler initiation_handler_;
  TerminationRequestResponseHandler termination_handler_;
};
...
uint64_t Controller::MessageGroupStreams::SendInitiationRequest(
...
void Controller::MessageGroupStreams::OnMatchedResponse(
    ...
    uint64_t endpoint_id) {
  if (response->result !=
      msgs::PresentationInitiationResponse_result::kSuccess) {
    OSP_LOG_INFO << "presentation-initiation-response for "
                 << request->request.url
                 << " failed: " << static_cast<int>(response->result);
    request->delegate->OnError(Error(Error::Code::kUnknownStartError));
    return;
  }
  ...
  controller_->OpenConnection(request->request.connection_id, endpoint_id,
                              service_id_, request->delegate,
                              std::move(connection),
                              NetworkServiceManager::Get()
                                  ->GetProtocolConnectionClient()
...
Controller::ConnectRequest Controller::StartPresentation(
  ...
  uint64_t request_id =
      group_streams_[service_id]->SendInitiationRequest(std::move(request));
