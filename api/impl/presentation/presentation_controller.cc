...
uint64_t Controller::MessageGroupStreams::SendConnectionOpenRequest(
  ...
  connection_open_handler_.WriteMessage(request_id, &request);
  return request_id;
}
...
void Controller::MessageGroupStreams::OnMatchedResponse(
  ...
  controller_->connection_manager_->AddConnection(request->connection.get());
...
void Controller::MessageGroupStreams::SendConnectionCloseRequest(
  ...
  connection_close_handler_.WriteMessage(absl::nullopt, &request);
}
