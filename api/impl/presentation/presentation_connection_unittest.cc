...
class MockConnectionDelegate final : public Connection::Delegate {
 public:
  ...
  MOCK_METHOD1(OnStringMessage, void(std::string message));
  MOCK_METHOD1(OnBinaryMessage, void(std::vector<uint8_t> data));
};
...
TEST_F(ConnectionTest, ConnectAndSend) {
  ...
  Connection controller(Connection::Info{id, url}, &mock_controller_delegate,
                        Connection::Role::kController);
  Connection receiver(Connection::Info{id, url}, &mock_receiver_delegate,
                      Connection::Role::kReceiver);

  EXPECT_EQ(id, controller.id());
  EXPECT_EQ(url, controller.url());
  EXPECT_EQ(id, receiver.id());
  EXPECT_EQ(url, receiver.url());
  ...
  NetworkServiceManager::Get()->GetProtocolConnectionClient()->Connect(
      quic_bridge_.receiver_endpoint, &mock_connect_request);
  ...
  EXPECT_CALL(quic_bridge_.mock_server_observer, OnIncomingConnectionMock(_))
      .WillOnce(Invoke([&receiver_stream](ProtocolConnection* stream) {
        receiver_stream.reset(stream);
      }));
  ...
  controller.SendString(std::move(message));

  std::string received;
  EXPECT_CALL(mock_receiver_delegate, OnStringMessage(expected_message))
      .WillOnce(
          Invoke([&received](std::string s) { received = std::move(s); }));
  quic_bridge_.RunTasksUntilIdle();

  std::string string_response = MakeEchoResponse(received);
  receiver.SendString(std::move(string_response));
  EXPECT_CALL(mock_controller_delegate, OnStringMessage(expected_response));
  quic_bridge_.RunTasksUntilIdle();
  ...
  std::vector<uint8_t> binary_response = MakeEchoResponse(received_data);
  receiver.SendBinary(std::move(binary_response));
