...
class MockObserver final : public ScreenPublisherObserver {
 public:
  ...
  MOCK_METHOD1(OnError, void(ScreenPublisherErrorInfo));

  MOCK_METHOD1(OnMetrics, void(ScreenPublisherMetrics));
};
...
TEST_F(ScreenPublisherImplTest, NormalStartStop) {
  ASSERT_EQ(ScreenPublisherState::kStopped, screen_publisher_->state());

  EXPECT_CALL(mock_delegate_, StartPublisher());
  EXPECT_TRUE(screen_publisher_->Start());
  EXPECT_FALSE(screen_publisher_->Start());
  EXPECT_EQ(ScreenPublisherState::kStarting, screen_publisher_->state());

  mock_delegate_.SetState(ScreenPublisherState::kRunning);
  EXPECT_EQ(ScreenPublisherState::kRunning, screen_publisher_->state());

  EXPECT_CALL(mock_delegate_, StopPublisher());
  EXPECT_TRUE(screen_publisher_->Stop());
  EXPECT_FALSE(screen_publisher_->Stop());
  EXPECT_EQ(ScreenPublisherState::kStopping, screen_publisher_->state());

  mock_delegate_.SetState(ScreenPublisherState::kStopped);
  EXPECT_EQ(ScreenPublisherState::kStopped, screen_publisher_->state());
}
...
TEST_F(ScreenPublisherImplTest, StartSuspended) {
  ...
  mock_delegate_.SetState(ScreenPublisherState::kSuspended);
  EXPECT_EQ(ScreenPublisherState::kSuspended, screen_publisher_->state());
}
...
TEST_F(ScreenPublisherImplTest, ObserverTransitions) {
  ...
  mock_delegate.SetState(ScreenPublisherState::kStopped);
}
