...
class TaskRunnerImpl : public TaskRunner {
 public:
  ...
  // Tasks will only be executed if RunUntilStopped has been called, and RequestStopSoon
  ...
  // Thread safe method for requesting the TaskRunnerImpl to stop running. This sets
  // a flag that will get checked in the run loop, typically after completing the
  // current task.
 ...
 private:
  ...
  // Loop method that runs tasks in the current thread, until the RequestStopSoon
  // method is called.
