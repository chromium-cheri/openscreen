...
QuicConnectionFactoryImpl::~QuicConnectionFactoryImpl() {
  ...
  if (server_socket_)
    platform::DestroyUdpSocket(server_socket_);
  platform::DestroyEventWaiter(waiter_);
}

void QuicConnectionFactoryImpl::SetServerDelegate(ServerDelegate* delegate,
                                                  IPAddress::Version ip_version,
                                                  uint16_t port) {
  server_delegate_ = delegate;
  server_socket_ = (ip_version == IPAddress::Version::kV4)
                       ? platform::CreateUdpSocketIPv4()
                       : platform::CreateUdpSocketIPv6();
  platform::BindUdpSocket(server_socket_, port, 0);
  platform::WatchUdpSocketReadable(waiter_, server_socket_);
}
    ...
    if (conn_it == connections_.end()) {
      if (server_delegate_) {
        OSP_VLOG(1) << __func__ << ": spawning connection from "
                    << packet.source;
        auto transport =
            MakeUnique<UdpTransport>(server_socket_, packet.source);
        ...
        auto result = MakeUnique<QuicConnectionImpl>(
            server_delegate_->NextConnectionDelegate(packet.source),
            std::move(transport),
...
std::unique_ptr<QuicConnection> QuicConnectionFactoryImpl::Connect(
  ...
  auto* socket = endpoint.address.IsV4() ? platform::CreateUdpSocketIPv4()
                                         : platform::CreateUdpSocketIPv6();
  platform::BindUdpSocket(socket, 0, 0);
  ...
  auto result = MakeUnique<QuicConnectionImpl>(
      connection_delegate, std::move(transport),
      quartc_factory_->CreateQuartcSession(session_config));
