...
QuicConnectionFactoryImpl::QuicConnectionFactoryImpl() {
  ...
  alarm_factory_ = absl::make_unique<::net::QuicChromiumAlarmFactory>(
  ...
  quartc_factory_ = absl::make_unique<::quic::QuartcFactory>(factory_config);
    ...
    if (conn_it == connections_.end()) {
      if (server_delegate_) {
        OSP_VLOG(1) << __func__ << ": spawning connection from "
                    << packet.source;
        auto transport =
            absl::make_unique<UdpTransport>(packet.socket, packet.source);
        ...
        auto result = absl::make_unique<QuicConnectionImpl>(
...
std::unique_ptr<QuicConnection> QuicConnectionFactoryImpl::Connect(
  ...
  auto transport = absl::make_unique<UdpTransport>(socket, endpoint);
