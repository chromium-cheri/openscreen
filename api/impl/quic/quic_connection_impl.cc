...
void QuicStreamImpl::Write(const uint8_t* data, size_t size) {
  stream_->WriteOrBufferData(
      ::quic::QuicStringPiece(reinterpret_cast<const char*>(data), size), false,
      nullptr);
}
    ...
    if (!stream->fin_sent()) {
      stream->WriteMemSlices(
          ::quic::QuicMemSliceSpan(
              ::quic::QuicMemSliceSpanImpl(nullptr, nullptr, 0)),
          true);
    }
  }
  delegate_->OnReceived(this, data, size);
}
...
void QuicConnectionImpl::OnIncomingStream(::quic::QuartcStream* stream) {
  auto public_stream = MakeUnique<QuicStreamImpl>(
      delegate_->NextStreamDelegate(session_->connection_id()), stream);
