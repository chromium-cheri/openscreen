...
class QuicServerTest : public ::testing::Test {
 protected:
  std::unique_ptr<ProtocolConnection> MakeServerStream() {
    ...
    EXPECT_CALL(quic_bridge_.mock_server_observer, OnIncomingConnectionMock(_))
        .WillOnce(::testing::SaveArg<0>(&stream));
...
TEST_F(QuicServerTest, Connect) {
  std::unique_ptr<ProtocolConnection> connection = MakeServerStream();
...
TEST_F(QuicServerTest, OpenImmediate) {
  ...
  std::unique_ptr<ProtocolConnection> connection1 = MakeServerStream();
