...
QuicClient::ConnectRequest QuicClient::OpenConnection(
  ...
  if (endpoint_entry != endpoint_map_.end()) {
    ...
    if (connection_entry != connections_.end()) {
      std::unique_ptr<QuicStream> stream =
          connection_entry->second.connection->MakeOutgoingStream(
              connection_entry->second.delegate.get());
      auto pc = MakeUnique<QuicProtocolConnection>(this, endpoint_entry->second,
                                                   stream->id());
      pc->set_stream(stream.get());
      connection_entry->second.delegate->AddStreamPair(
          ServiceStreamPair(std::move(stream), pc.get()));
...
std::unique_ptr<ProtocolConnection> QuicClient::OpenConnectionNow(
    const IPEndpoint& endpoint) {
  if (state_ != State::kRunning)
    return nullptr;
  auto endpoint_entry = endpoint_map_.find(endpoint);
...
uint64_t QuicClient::OnCryptoHandshakeComplete(
  ...
  for (auto& request : pending_entry->second.callbacks) {
    ...
    std::unique_ptr<QuicStream> stream =
        connection->MakeOutgoingStream(delegate);
    auto pc =
        MakeUnique<QuicProtocolConnection>(this, endpoint_id, stream->id());
    pc->set_stream(stream.get());
...
void QuicClient::OnConnectionClosed(uint64_t endpoint_id,
  ...
  delete_connections_.emplace_back(std::move(connection_entry->second));
