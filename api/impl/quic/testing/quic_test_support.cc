...
FakeQuicBridge::FakeQuicBridge() {
  fake_bridge =
      std::make_unique<FakeQuicConnectionFactoryBridge>(controller_endpoint);
  platform::TimeDelta now = platform::TimeDelta::FromMilliseconds(1298424);
  auto fake_clock = std::make_unique<FakeClock>(now);
  ...
  fake_clock = std::make_unique<FakeClock>(now);
