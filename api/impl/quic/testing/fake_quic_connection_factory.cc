...
void FakeQuicConnectionFactoryBridge::OnOutgoingStream(
    QuicConnection* connection,
    QuicStream* stream) {
  FakeQuicConnection* connectee = nullptr;
  if (connection == connections_.controller) {
    connectee = connections_.receiver;
  } else if (connection == connections_.receiver) {
    connectee = connections_.controller;
  }

  if (connectee) {
    connectee->delegate()->OnIncomingStream(connectee->id(),
                                            connectee->MakeIncomingStream());
  }
}
