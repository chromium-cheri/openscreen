...
#ifndef API_IMPL_QUIC_TESTING_FAKE_QUIC_BRIDGE_H_
#define API_IMPL_QUIC_TESTING_FAKE_QUIC_BRIDGE_H_
...
class MockServerObserver final : public ProtocolConnectionServer::Observer {
 public:
  ...
  void OnIncomingConnection(
    ...
    OnIncomingConnectionMock(connections.back().get());
  }
  MOCK_METHOD1(OnIncomingConnectionMock, void(ProtocolConnection* connection));

  // The MockServerObserver ends up owning all of the
  // connections, because otherwise tests will leak
  // memory if you don't subscribe to incoming connections.
  std::vector<std::unique_ptr<ProtocolConnection>> connections;
};

struct FakeQuicBridge {
  FakeQuicBridge();
  ~FakeQuicBridge();

  void RunTasksUntilIdle();

  FakeClock* controller_fake_clock;
  FakeClock* receiver_fake_clock;
  const IPEndpoint controller_endpoint{{192, 168, 1, 3}, 4321};
  const IPEndpoint receiver_endpoint{{192, 168, 1, 17}, 1234};
...
#endif  // API_IMPL_QUIC_TESTING_FAKE_QUIC_BRIDGE_H_
