...
TEST_F(QuicClientTest, Connect) {
  ...
  ProtocolConnectionClient::ConnectRequest request =
      client_->OpenConnection(server_endpoint_, &connection_callback);
  ASSERT_TRUE(request);
...
TEST_F(QuicClientTest, OpenImmediate) {
  ...
  connection2 = client_->OpenConnectionNow(server_endpoint_);
...
TEST_F(QuicClientTest, States) {
  ...
  std::unique_ptr<ProtocolConnection> connection2 =
      client_->OpenConnectionNow(server_endpoint_);
  EXPECT_FALSE(connection2);
  ...
  request = client_->OpenConnection(server_endpoint_, &connection_callback);
  ASSERT_TRUE(request);
  RunTasksUntilIdle();
  ASSERT_TRUE(connection1);
