...
class MockReceiverListenerObserver final : public ReceiverListener::Observer {
 public:
  ~MockReceiverListenerObserver() override = default;
  ...
  MOCK_METHOD1(OnReceiverAdded, void(const ReceiverInfo&));
  MOCK_METHOD1(OnReceiverChanged, void(const ReceiverInfo&));
  MOCK_METHOD1(OnReceiverRemoved, void(const ReceiverInfo&));
...
class MockControllerPublisherObserver final
    : public ControllerPublisher::Observer {
 public:
  ~MockControllerPublisherObserver() override = default;
...
class MdnsResponderServiceTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    receiver_listener_ =
        MakeUnique<ReceiverListenerImpl>(&observer_, mdns_service_.get());

    mdns_service_->SetServiceConfig(kTestHostname, kTestServiceInstance,
                                    kTestPort, {}, {{"model", "shifty"}});
    controller_publisher_ = MakeUnique<ControllerPublisherImpl>(
        &publisher_observer_, mdns_service_.get());
  }

  MockReceiverListenerObserver observer_;
  ...
  MockControllerPublisherObserver publisher_observer_;
...
TEST_F(MdnsResponderServiceTest, BasicServiceStates) {
  ...
  EXPECT_CALL(observer_, OnReceiverAdded(_))
      .WillOnce(::testing::Invoke([&receiver_id](const ReceiverInfo& info) {
  ...
  EXPECT_CALL(observer_, OnReceiverChanged(_))
      .WillOnce(::testing::Invoke([&receiver_id](const ReceiverInfo& info) {
        EXPECT_EQ(receiver_id, info.receiver_id);
        EXPECT_EQ(kTestServiceInstance, info.friendly_name);
...
TEST_F(MdnsResponderServiceTest, NetworkInterfaceIndex) {
  ...
  EXPECT_CALL(observer_, OnReceiverAdded(_))
      .WillOnce(::testing::Invoke([](const ReceiverInfo& info) {
