...
class MockMdnsDelegate final : public ScreenListenerImpl::Delegate {
 public:
  ...
  MOCK_METHOD0(ListenerStart, void());
  MOCK_METHOD0(ListenerStartAndSuspend, void());
  MOCK_METHOD0(ListenerStop, void());
  MOCK_METHOD0(ListenerSuspend, void());
  MOCK_METHOD0(ListenerResume, void());
  MOCK_METHOD1(ListenerSearchNow, void(ScreenListenerState));
};
...
TEST_F(ScreenListenerImplTest, NormalStartStop) {
  ...
  EXPECT_CALL(mock_delegate_, ListenerStart());
  ...
  screen_listener_->SetState(ScreenListenerState::kRunning);
  EXPECT_EQ(ScreenListenerState::kRunning, screen_listener_->state());

  EXPECT_CALL(mock_delegate_, ListenerStop());
  ...
  screen_listener_->SetState(ScreenListenerState::kStopped);
...
TEST_F(ScreenListenerImplTest, StartSuspended) {
  EXPECT_CALL(mock_delegate_, ListenerStartAndSuspend());
  EXPECT_CALL(mock_delegate_, ListenerStart()).Times(0);
  EXPECT_TRUE(screen_listener_->StartAndSuspend());
  EXPECT_FALSE(screen_listener_->Start());
  EXPECT_EQ(ScreenListenerState::kStarting, screen_listener_->state());

  screen_listener_->SetState(ScreenListenerState::kSuspended);
  EXPECT_EQ(ScreenListenerState::kSuspended, screen_listener_->state());
}

TEST_F(ScreenListenerImplTest, SuspendWhileStarting) {
  EXPECT_CALL(mock_delegate_, ListenerStart()).Times(1);
  EXPECT_CALL(mock_delegate_, ListenerSuspend()).Times(1);
  EXPECT_TRUE(screen_listener_->Start());
...
TEST_F(ScreenListenerImplTest, SuspendAndResume) {
  ...
  EXPECT_CALL(mock_delegate_, ListenerResume()).Times(0);
  EXPECT_CALL(mock_delegate_, ListenerSuspend()).Times(2);
  ...
  EXPECT_CALL(mock_delegate_, ListenerStart()).Times(0);
  EXPECT_CALL(mock_delegate_, ListenerSuspend()).Times(0);
  EXPECT_CALL(mock_delegate_, ListenerResume()).Times(2);
  EXPECT_FALSE(screen_listener_->Start());
...
TEST_F(ScreenListenerImplTest, SearchWhileRunning) {
  EXPECT_CALL(mock_delegate_, ListenerSearchNow(_)).Times(0);
  EXPECT_FALSE(screen_listener_->SearchNow());
  EXPECT_TRUE(screen_listener_->Start());
  screen_listener_->SetState(ScreenListenerState::kRunning);

  EXPECT_CALL(mock_delegate_, ListenerSearchNow(ScreenListenerState::kRunning))
      .Times(2);
  EXPECT_TRUE(screen_listener_->SearchNow());
  EXPECT_TRUE(screen_listener_->SearchNow());

  screen_listener_->SetState(ScreenListenerState::kSearching);
...
TEST_F(ScreenListenerImplTest, SearchWhileSuspended) {
  ...
  EXPECT_CALL(mock_delegate_,
              ListenerSearchNow(ScreenListenerState::kSuspended))
      .Times(2);
  EXPECT_TRUE(screen_listener_->SearchNow());
  EXPECT_TRUE(screen_listener_->SearchNow());
...
TEST_F(ScreenListenerImplTest, ScreenObserverPassThrough) {
  ...
  auto verify_info_member = [this](const ScreenInfo& info) {
    const auto& screens = screen_listener_->GetScreens();
  ...
  EXPECT_CALL(observer, OnScreenRemoved(screen1_alt_name))
      .WillOnce(::testing::Invoke([this](const ScreenInfo& info) {
        const auto& screens = screen_listener_->GetScreens();
        EXPECT_TRUE(std::find(screens.begin(), screens.end(), info) ==
                    screens.end());
      }));
  ...
  EXPECT_CALL(observer, OnAllScreensRemoved())
      .WillOnce(::testing::Invoke(
          [this]() { EXPECT_TRUE(screen_listener_->GetScreens().empty()); }));
