...
void AddEventsForNewService(FakeMdnsResponderAdapter* mdns_responder,
  ...
  mdns_responder->AddPtrEvent(
      MakePtrExample(service_instance, service_name, service_protocol, socket));
  mdns_responder->AddSrvEvent(MakeSrvExample(service_instance, service_name,
  ...
  mdns_responder->AddAEvent(MakeAExample(hostname, address, socket));
}
