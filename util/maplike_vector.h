...
class MaplikeVector final : private std::vector<std::pair<Key, Value>> {
 public:
  static constexpr std::size_t kMaxSize = 32;
  MaplikeVector() = default;
  explicit MaplikeVector(std::size_t reserved_size) {
    OSP_DCHECK(reserved_size < kMaxSize)
  ...
  // Adds a new key value pair to the back of the vector.
  void Insert(Key key, Value value) {
    OSP_DCHECK(this->size() < kMaxSize)
        << "With this many elements you probably want a map instead";
