...
TEST(SimpleFractionTest, Finiteness) {
  EXPECT_TRUE((SimpleFraction{-0, -1}).is_finite());
  EXPECT_TRUE((SimpleFraction{-0, 1}).is_finite());
  EXPECT_TRUE((SimpleFraction{1234, 20}).is_finite());
  EXPECT_TRUE((SimpleFraction{kMin + 1, 20}).is_finite());
  EXPECT_TRUE((SimpleFraction{kMax - 1, 20}).is_finite());
  EXPECT_TRUE((SimpleFraction{0, kMin}).is_finite());
  EXPECT_TRUE((SimpleFraction{0, kMax}).is_finite());

  EXPECT_FALSE((SimpleFraction{kMin, 1}).is_finite());
  EXPECT_FALSE((SimpleFraction{kMax, 1}).is_finite());
  EXPECT_FALSE((SimpleFraction{kMin, 0}).is_finite());
  EXPECT_FALSE((SimpleFraction{kMax, 0}).is_finite());
  EXPECT_FALSE((SimpleFraction{0, 0}).is_finite());
  EXPECT_FALSE((SimpleFraction{-0, -0}).is_finite());
}
