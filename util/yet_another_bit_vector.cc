...
YetAnotherBitVector& YetAnotherBitVector::operator=(
  ...
  bits_ = other.bits_;
  other.size_ = 0;
  // Note: No need to null-out |other.bits_|, since a zero |other.size_|
  // indicates it has no array allocated.
  return *this;
}
...
void YetAnotherBitVector::SetAll() {
  if (using_array_storage()) {
    if (int end_bit_offset = (size_ % kBitsPerInteger)) {
      // Set all but the bits in the last integer outside the defined range.
...
void YetAnotherBitVector::ShiftRight(int steps) {
  ...
  if (using_array_storage()) {
    ...
    if (steps >= kBitsPerInteger) {
      ...
      const int integer_steps = std::min(steps / kBitsPerInteger, num_integers);
    ...
    for (int i = array_size(); i-- > 0;) {
      const uint64_t outgoing_carry_bits =
          bits_.as_array[i] & MakeBitmask(0, steps);
...
#define CountTrailingZeros(bits) __builtin_ctzll(bits)
  ...
  if (using_array_storage()) {
    const uint64_t* const last = &bits_.as_array[array_size() - 1];
    for (const uint64_t* p = bits_.as_array;; ++p) {
      if (*p != 0) {
        return ((p - bits_.as_array) * kBitsPerInteger) +
               CountTrailingZeros(*p);
      }
      if (/* the integer is zero and */ p == last) {
        return size_;  // All bits are unset.
      }
    }
  }
