...
constexpr uint64_t MakeBitmask(int begin, int count) {
  ...
  // power of 2. Edge case: 2^count mod UINT64_MAX is zero when count >= 64.
  constexpr uint64_t kPowerOfTwoLargerThanUint64Max = 0;
  const uint64_t some_power_of_two =
      (count >= std::numeric_limits<uint64_t>::digits)
          ? kPowerOfTwoLargerThanUint64Max
          : (uint64_t{1} << count);
