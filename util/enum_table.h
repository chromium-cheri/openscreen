...
// NOTE: Directly lifted from Chrome's components/cast_channel/enum_table*
//
...
class
 ...
 private:
  ...
  OSP_DISALLOW_COPY_AND_ASSIGN(GenericEnumTableEntry);
};
...
class EnumTable {
 public:
  ...
  class Entry : public GenericEnumTableEntry {
   public:
    ...
    OSP_DISALLOW_COPY_AND_ASSIGN(Entry);
  };
  ...
  constexpr EnumTable(std::initializer_list<Entry> data, E max_value)
      : EnumTable(data, true) {
#ifndef NDEBUG
    // NOTE(jrw): This is compiled out when NDEBUG is defined, even if
    // OSP_DCHECKS are normally enabled because OSP_DCHECK_ALWAYS_ON is defined.
    // The reason for this is that if OSP_DCHECKs are enabled, global EnumTable
  ...
  template <E Value>
  constexpr absl::string_view GetString() const {
 ...
 private:
  ...
  OSP_DISALLOW_COPY_AND_ASSIGN(EnumTable);
};
