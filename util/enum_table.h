...
//
//
// When the conditions in the previous section cannot be satisfied, the enum
// talbe should have the |IsSorted| template parameter set to false. Doing so
// has some unfortunate side-effects: there is no automatic way to detect when
// an enum value is missing from the table, and looking up a non-constant enum
// value requires a linear search.

namespace openscreen {

class GenericEnumTableEntry {
 public:
 ...
 private:
  ...
  constexpr bool has_str() const { return chars_ != nullptr; }
 ...
 private:
  ...
  template <typename E, bool IsSorted>
...
inline constexpr GenericEnumTableEntry::GenericEnumTableEntry(int32_t value)
    : chars_(nullptr), length_(UINT32_MAX), value_(value) {}

inline constexpr GenericEnumTableEntry::GenericEnumTableEntry(
    int32_t value,
    absl::string_view str)
    : chars_(str.data()),
      length_(static_cast<uint32_t>(str.length())),
      value_(value) {}
...
template <typename E, bool IsSorted = true>
class EnumTable {
 public:
  ...
  constexpr EnumTable(std::initializer_list<Entry> data, E max_value)
    ...
    OSP_DCHECK(data.end()[-1].value_ == int_max_value)
  ...
  absl::string_view GetString() const {
    for (const auto& entry : data_) {
      if (entry.value_ == static_cast<int32_t>(Value) && entry.has_str())
        return entry.str();
    }
  ...
  absl::optional<E> GetEnum(absl::string_view str) const {
    ...
    return entry ? static_cast<E>(entry->value_) : absl::optional<E>();
  }
  ...
  constexpr EnumTable(std::initializer_list<Entry> data) {
    ...
    if (IsSorted) {
      ...
      OSP_DCHECK(found) << "Entries are actually sorted, please remove false "
                           "IsSorted template "
                           "parameter.";
    }
    ...
    for (std::size_t i = 0; i < data.size(); i++) {
      ...
      for (std::size_t j = i + 1; j < data.size(); j++) {
        ...
        OSP_DCHECK(ei.value_ != ej.value_)
