...
TEST(EnumTableTest, TestGetString) {
  ...
  EXPECT_EQ("ZERO", first);
  EXPECT_EQ("ZERO", kSorted.GetString(MyEnum::kZero).value());
  EXPECT_EQ("ONE", kSorted.GetString(MyEnum::kOne).value());
  EXPECT_EQ("TWO", kSorted.GetString(MyEnum::kTwo).value());
}

TEST(EnumTableTest, TestGetStringUnsorted) {
  EXPECT_EQ("ZERO", kUnsorted.GetString(MyEnum::kZero).value());
  EXPECT_EQ("ONE", kUnsorted.GetString(MyEnum::kOne).value());
  EXPECT_EQ("TWO", kUnsorted.GetString(MyEnum::kTwo).value());
}

TEST(EnumTableTest, TestGetMissingString) {
  EXPECT_EQ("ZERO", kSortedMissing.GetString(MyEnum::kZero).value());
  EXPECT_EQ(absl::nullopt, kSortedMissing.GetString(MyEnum::kOne));
  EXPECT_EQ("TWO", kSortedMissing.GetString(MyEnum::kTwo).value());
}

TEST(EnumTableTest, TestGetMissingStringUnsorted) {
  EXPECT_EQ(absl::nullopt, kUnsortedMissing.GetString(MyEnum::kZero));
  EXPECT_EQ("ONE", kUnsortedMissing.GetString(MyEnum::kOne).value());
  EXPECT_EQ("TWO", kUnsortedMissing.GetString(MyEnum::kTwo).value());
}
