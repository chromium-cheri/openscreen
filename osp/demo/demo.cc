...
void ListenerDemo() {
  ...
  platform::TaskRunnerThread task_runner_thread(
      std::make_unique<platform::TaskRunnerImpl>(platform::Clock::now));
  platform::NetworkReaderThread network_reader_thread(
      std::make_unique<platform::NetworkReader>());
  platform::UdpSocket::SetLifetimeObserver(network_reader_thread.get());
  ...
  auto mdns_listener = MdnsServiceListenerFactory::Create(
      listener_config, &listener_observer, task_runner_thread.get());
  ...
  auto connection_client = ProtocolConnectionClientFactory::Create(
      &demuxer, &client_observer, task_runner_thread.get());
...
void PublisherDemo(absl::string_view friendly_name) {
  ...
  auto mdns_publisher = MdnsServicePublisherFactory::Create(
      publisher_config, &publisher_observer, task_runner_thread.get());
  ...
  auto connection_server = ProtocolConnectionServerFactory::Create(
      server_config, &demuxer, &server_observer, task_runner_thread.get());
