...
using PlatformClient = openscreen::platform::PlatformClient;
...
void ListenerDemo() {
  ...
  auto mdns_listener = MdnsServiceListenerFactory::Create(
      ...
      PlatformClient::GetInstance()->task_runner());
  ...
  auto connection_client = ProtocolConnectionClientFactory::Create(
      &demuxer, &client_observer, PlatformClient::GetInstance()->task_runner());
