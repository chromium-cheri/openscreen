...
int main(int argc, char** argv) {
  ...
  openscreen::platform::TaskRunnerThread task_runner_thread(
      std::make_unique<openscreen::platform::TaskRunnerImpl>(
          openscreen::platform::Clock::now));
  openscreen::platform::NetworkReaderThread network_reader_thread(
      std::make_unique<openscreen::platform::NetworkReader>());
  openscreen::platform::UdpSocket::SetLifetimeObserver(
      network_reader_thread.get());

  openscreen::BrowseDemo(task_runner_thread.get(), labels[0], labels[1],
                         service_instance);
