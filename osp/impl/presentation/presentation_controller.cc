...
Controller::MessageGroupStreams::~MessageGroupStreams() {
  ...
  // destruction process, which may cause error that delete one intance twice.
...
void Controller::BuildConnection(const std::string& instance_name,
                                 const std::string& password) {
  auto requset_entry = connect_requests_by_instance_name_.find(instance_name);
  ...
  auto result = connect_requests_by_instance_name_.insert(
      {instance_name, openscreen::osp::ConnectRequest()});
  auto* client = NetworkServiceManager::Get()->GetProtocolConnectionClient();
  client->SetPassword(instance_name, password);
...
void Controller::OnConnectSucceed(uint64_t request_id,
  ...
  OSP_VLOG << "Controller succeed to build the underlying connection to: "
           << instance_name;
}
