...
void MdnsResponderService::OnRead(std::unique_ptr<Packet> packet,
                                  platform::NetworkRunner* network_runner) {
  if (!mdns_responder_) {
    return;
  }
...
void MdnsResponderService::StartAndSuspendListener() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::StartAndSuspendListenerInternal, this));
}

void MdnsResponderService::StopListener() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::StopListenerInternal, this));
}

void MdnsResponderService::SuspendListener() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::SuspendListenerInternal, this));
}

void MdnsResponderService::ResumeListener() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::ResumeListenerInternal, this));
}

void MdnsResponderService::SearchNow(ServiceListener::State from) {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::SearchNowInternal, this, from));
}

void MdnsResponderService::StartPublisher() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::StartPublisherInternal, this));
}

void MdnsResponderService::StartAndSuspendPublisher() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::StartAndSuspendPublisherInternal, this));
}

void MdnsResponderService::StopPublisher() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::StopPublisherInternal, this));
}

void MdnsResponderService::SuspendPublisher() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::SuspendPublisherInternal, this));
}

void MdnsResponderService::ResumePublisher() {
  network_runner_->PostTask(
      std::bind(&MdnsResponderService::ResumePublisherInternal, this));
}
...
void MdnsResponderService::StartListenerInternal() {
  ...
  network_runner_->PostRepeatedTask(
...
void MdnsResponderService::SuspendListenerInternal() {
  ...
  for (const auto& interface : bound_interfaces_) {
    network_runner_->CancelRead(interface.socket);
  }
}
...
void MdnsResponderService::ResumeListenerInternal() {
  ...
  for (const auto& interface : bound_interfaces_) {
    network_runner_->ReadRepeatedly(interface.socket, this);
  }
}
