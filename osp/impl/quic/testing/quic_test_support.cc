...
void FakeQuicBridge::RunCurrentTasks() {
  ...
  if (!client_idle || !server_idle) {
    ...
    network_runner_->PostTask(
        std::bind(&FakeQuicBridge::RunTasksUntilIdle, this));
  }
}
...
void FakeQuicBridge::RunTasksUntilIdle() {
  ...
  std::function<void()> callback =
      std::bind(&FakeQuicBridge::RunCurrentTasks, this);
  network_runner_->PostTask(callback);
