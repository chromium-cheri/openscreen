...
#include "third_party/quiche/src/quiche/quic/core/quic_constants.h"
...
void OpenScreenSessionBase::OnHandshakeCallbackDone() {
  QuicSession::OnHandshakeCallbackDone();
  visitor_->OnCryptoHandshakeComplete();
}

QuicStream* OpenScreenSessionBase::CreateOutgoingStream(
    QuicStream::Delegate* delegate) {
  OSP_DCHECK(connection()->connected());
  ...
  std::unique_ptr<QuicStreamImpl> stream = std::make_unique<QuicStreamImpl>(
...
quic::QuicStream* OpenScreenSessionBase::CreateIncomingStream(
  ...
  if (!connection()->connected()) {
    OSP_VLOG << ("CreateIncomingStream is called when disconnected");
    return nullptr;
  }
