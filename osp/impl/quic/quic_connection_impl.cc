...
#include "third_party/quiche/src/quiche/quic/core/quic_packets.h"
...
QuicStreamImpl::QuicStreamImpl(Delegate* delegate,
    ...
    : osp::QuicStream(delegate), quic::QuicStream(id, session, false, type) {}
...
void QuicConnectionImpl::OnRead(UdpSocket* socket, ErrorOr<UdpPacket> packet) {
  ...
  session_->ProcessUdpPacket(ToQuicSocketAddress(packet.value().destination()),
