...
const size_t kMaxConnectionsToCreate = 256;
const char kFingerPrint[] =
    "50:87:8D:CA:1B:9B:67:76:CB:87:88:1C:43:20:82:7A:91:F5:9B:74:4D:85:95:D0:"
    "76:E6:0B:50:7F:D3:29:D9";
const char kCertificatesPath[] = "osp/impl/quic/certificates/open-screen.pem";
const char kPrivateKeyPath[] = "osp/impl/quic/certificates/open-screen.key";
...
void QuicConnectionFactoryImpl::OnRead(UdpSocket* socket,
  ...
  if (conn_it == connections_.end() || server_delegate_) {
    if (!server_delegate_ || !dispatcher) {
      return;
    }
...
std::unique_ptr<QuicConnection> QuicConnectionFactoryImpl::Connect(
  ...
  OpenScreenSessionBase* session = new OpenScreenClientSession(
      std::move(connection), crypto_client_config_.get(), connection_impl.get(),
      config_, quic::QuicServerId("openscreen.com", 10), supported_versions_);
