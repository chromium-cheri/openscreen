...
namespace {
class OnceClosureWrapper {
 public:
  explicit OnceClosureWrapper(::base::OnceClosure task) {
...
class QuicTaskRunner final : public ::base::TaskRunner {
 public:
  QuicTaskRunner(platform::TaskRunner* task_runner);
...
bool QuicTaskRunner::PostDelayedTask(const ::base::Location& whence,
  ...
  task_runner_->PostTaskWithDelay(OnceClosureWrapper{std::move(task)}, wait);
  return true;
}
