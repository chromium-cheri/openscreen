...
// TODO(Wei): Determine which Certificate, PrivateKey and Fingerprint to use.
...
std::unique_ptr<quic::ProofSource> CreateProofSource() {
  ...
  OSP_DCHECK(chain) << "Failed to create the quic::ProofSource::Chain.";
...
void QuicConnectionFactoryImpl::SetServerDelegate(
  ...
  OSP_LOG_INFO << "QuicConnectionFactoryImpl::SetServerDelegate";
...
void QuicConnectionFactoryImpl::OnBound(UdpSocket* socket) {
  OSP_LOG_INFO << "Udp socket is bound, local endpoint is: "
               << socket->GetLocalEndpoint().ToString();
}
...
void QuicConnectionFactoryImpl::OnSendError(UdpSocket* socket, Error error) {
  OSP_LOG_INFO << "QuicConnectionFactoryImpl::OnSendError";
...
void QuicConnectionFactoryImpl::OnRead(UdpSocket* socket,
  ...
  if (conn_it == connections_.end()) {
    ...
    OSP_LOG_INFO << "Server connection ID: " << server_connection_id.ToString();
    auto connection = std::make_unique<quic::QuicConnection>(
        server_connection_id, ToQuicSocketAddress(socket->GetLocalEndpoint()),
    ...
    connection->set_debug_visitor(&debug_visitor_);
    ...
    // Prepare parameters for constructing OpenScreenServerSession.
    auto quic_crypto_server_config =
        std::make_unique<quic::QuicCryptoServerConfig>(
            kSourceAddressTokenSecret, quic::QuicRandom::GetInstance(),
            CreateProofSource(), quic::KeyExchangeSource::Default());
    auto compressed_certs_cache =
    ...
    std::unique_ptr<OpenScreenSessionBase> session =
        std::make_unique<OpenScreenServerSession>(
            std::move(connection), std::move(quic_crypto_server_config),
            std::move(compressed_certs_cache), connection_impl_ptr,
            quic::QuicConfig(), supported_versions_);
...
std::unique_ptr<QuicConnection> QuicConnectionFactoryImpl::Connect(
  ...
  OSP_LOG_INFO << "QuicConnectionFactoryImpl::Connect";
  ...
  socket->Bind();
  quic::QuicPacketWriter* writer =
      new PacketWriterImpl(socket.get(), remote_endpoint);
  quic::QuicConnectionId server_connection_id =
  ...
  quic::QuicConnectionId client_connection_id =
      quic::QuicUtils::CreateRandomConnectionId(helper_->GetRandomGenerator());
  OSP_LOG_INFO << "Client connection ID: " << client_connection_id.ToString();
  connection->set_client_connection_id(client_connection_id);

  auto connection_impl = std::make_unique<QuicConnectionImpl>(
      this, connection_delegate, helper_->GetClock());

  // Prepare parameters for constructing OpenScreenClientSession.
  auto proof_verifier =
  ...
  auto quic_crypto_client_config =
      std::make_unique<quic::QuicCryptoClientConfig>(std::move(proof_verifier),
                                                     nullptr);
  std::unique_ptr<OpenScreenSessionBase> session =
      std::make_unique<OpenScreenClientSession>(
          std::move(connection), std::move(quic_crypto_client_config),
          connection_impl.get(), quic::QuicConfig(),
          quic::QuicServerId("openscreen.test", 10), supported_versions_);
  connection_impl->set_session(std::move(session));
