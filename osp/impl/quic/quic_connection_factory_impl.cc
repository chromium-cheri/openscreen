...
namespace {
class OnceClosureExecutor {
 public:
  explicit OnceClosureExecutor(::base::OnceClosure task) {
    closure_ = std::move(task);
  }

  void operator()() { std::move(closure_).Run(); }
...
class QuicTaskRunner final : public ::base::TaskRunner {
 public:
  QuicTaskRunner(platform::NetworkRunner* network_runner);
...
QuicTaskRunner::QuicTaskRunner(platform::NetworkRunner* network_runner)
...
bool QuicTaskRunner::PostDelayedTask(const ::base::Location& whence,
  ...
  network_runner_->PostTaskWithDelay(OnceClosureExecutor{std::move(task)},
                                     wait);
  return true;
}
...
void QuicConnectionFactoryImpl::ScheduleCleanUp(
    std::function<uint32_t()> clean_up_function) {
  network_runner_->PostRepeatedTask(clean_up_function,
