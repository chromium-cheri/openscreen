...
AuthenticationBob::AuthenticationBob(MessageDemuxer& demuxer,
                                     Delegate& delegate,
...
void AuthenticationBob::StartAuthentication(uint64_t instance_id) {
  ...
  msgs::AuthSpake2Handshake message = {
      .initiation_token =
          msgs::AuthInitiationToken{
              .has_token = true,
              .token = data_entry->second.auth_token,
          },
    ...
    case msgs::Type::kAuthSpake2Handshake: {
      ...
      if (result < 0) {
        ...
        if (!initiation_token.has_token ||
            initiation_token.token != data_entry->second.auth_token) {
        ...
        if (handshake.psk_status == msgs::AuthSpake2PskStatus::kPskShown) {
          ...
          data_entry->second.shared_key =
              ComputeSharedKey(fingerprint_, handshake.public_value,
                               data_entry->second.password);
          msgs::AuthSpake2Handshake message = {
              .initiation_token =
                  msgs::AuthInitiationToken{
                      .has_token = true,
