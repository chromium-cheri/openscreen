...
ErrorOr<size_t> ConvertDecodeResult(size_t result) {
  if (result < 0) {
...
class MessageDemuxerTest : public ::testing::Test {
 protected:
  ...
  void ExpectDecodedRequest(
    ...
    EXPECT_EQ(decode_result, static_cast<size_t>(buffer_.size() - 2));
...
TEST_F(MessageDemuxerTest, WatchAfterMultipleData) {
  ...
  size_t decode_result1 = 0;
  size_t decode_result2 = 0;
  ...
  EXPECT_EQ(decode_result2, static_cast<size_t>(buffer.size() - 2));
