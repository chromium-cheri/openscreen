...
class Connection {
 public:
  ...
  uint64_t instance_number() const {
    OSP_CHECK(instance_number_);
    return instance_number_.value();
  }
 ...
 private:
  ...
  std::optional<uint64_t> instance_number_;
...
class ConnectionManager final : public MessageDemuxer::MessageCallback {
 ...
 private:
  ...
  // table then needs to be <instance number, connection id> since connection id
