...
def builder(builder_type, name, properties, os, cpu):
  ...
  triggers = None
  if builder_type == "ci":
    triggers = ["chromium-trigger" if recipe_id == "chromium"
                else "master-gitiles-trigger"]
  ...
  view_type = "CI" if builder_type == "ci" else "Try"
  luci.console_view_entry(
    builder = "{}/{}".format(builder_type, name),
    console_view = "OpenScreen {} Builders".format(view_type),
...
try_builder("openscreen_presubmit",
...
try_and_ci_builders("linux64_gcc_debug",
                    get_properties(is_gcc=True, use_goma=False))
try_and_ci_builders("linux64_tsan",
                    get_properties(is_release=True, is_tsan=True))
try_and_ci_builders("linux_arm64_debug",
...
try_and_ci_builders("chromium_mac_debug",
