...
void ExpectStreamingMessageValid(const char* message) {
  ErrorOr<Json::Value> message_root = json::Parse(message);
...
TEST(JsonValidationTest, ExampleStreamingMessages) {
  ExpectStreamingMessageValid(kAnswer);
  ExpectStreamingMessageValid(kCapabilitiesResponse);
  ExpectStreamingMessageValid(kGetCapabilities);
  ExpectStreamingMessageValid(kGetStatus);
  ExpectStreamingMessageValid(kOffer);
  ExpectStreamingMessageValid(kRpc);
  ExpectStreamingMessageValid(kStatusResponse);
}

TEST(JsonValidationTest, ExampleReceiverMessages) {
  ExpectReceiverMessageValid(kGetAppAvailability);
  ExpectReceiverMessageValid(kGetAppAvailabilityResponse);
  ExpectReceiverMessageValid(kLaunch);
  ExpectReceiverMessageValid(kStop);
}
}  // namespace cast
}  // namespace openscreen
