...
class SenderSocketsClient final
 ...
 public:
  ...
  void OnConnected(SenderSocketFactory* factory,
    ...
    OSP_DVLOG << "Connected to endpoint: " << endpoint;
    socket_ = socket.get();
...
class CastSocketE2ETest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    ASSERT_FALSE(loopback.value().addresses.empty());
    loopback_address_ = loopback.value().addresses[0].address;
  }
...
// This tests the most basic setup of a complete CastSocket.  This means
...
// authentication.
TEST_F(CastSocketE2ETest, Connect) {
  uint16_t port = 65321;
  receiver_tls_factory_->SetListenCredentials(receiver_tls_creds_);
  task_runner_->PostTask([this, port]() {
    receiver_tls_factory_->Listen(IPEndpoint{loopback_address_, port},
                                  TlsListenOptions{1u});
  });

  task_runner_->PostTask([this, port]() {
    sender_factory_->Connect(IPEndpoint{loopback_address_, port},
