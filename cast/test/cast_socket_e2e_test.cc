...
class CastSocketE2ETest : public ::testing::Test {
 ...
 protected:
  IPAddress GetLoopbackV4Address() {
    ...
    OSP_DCHECK(loopback);
    auto it = std::find_if(
        loopback->addresses.begin(), loopback->addresses.end(),
        [](const IPSubnet& subnet) { return subnet.address.IsV4(); });
    OSP_DCHECK(it != loopback->addresses.end());
    return it->address;
  }
  ...
  IPAddress GetLoopbackV6Address() {
    ...
    auto it = std::find_if(
        loopback->addresses.begin(), loopback->addresses.end(),
        [](const IPSubnet& subnet) { return subnet.address.IsV6(); });
    OSP_DCHECK(it != loopback->addresses.end());
    return it->address;
  }
