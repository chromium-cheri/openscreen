...
void ExpectFailureOnParse(absl::string_view body) {
  openscreen::ErrorOr<Json::Value> root = openscreen::json::Read(body);
...
TEST(OfferTest, CanParseValidButStreamlessOffer) {
  openscreen::ErrorOr<Json::Value> root = openscreen::json::Read(R"({
    "castMode": "mirroring",
    "supportedStreams": []
  })");
  ASSERT_TRUE(root.is_value());
...
TEST(OfferTest, CanParseValidOffer) {
  openscreen::ErrorOr<Json::Value> root = openscreen::json::Read(kValidOffer);
  ASSERT_TRUE(root.is_value());
