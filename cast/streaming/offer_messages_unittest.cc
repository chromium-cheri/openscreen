...
void ExpectEqualsValidOffer(const Offer& offer) {
  ...
  EXPECT_DOUBLE_EQ(60.0, vs_one.max_frame_rate());
  ...
  EXPECT_DOUBLE_EQ(30000.0 / 1001.0, vs_two.max_frame_rate());
...
TEST(OfferTest, ToJsonFailsWithInvalidStreams) {
  ...
  video_stream_invalid.video_streams[0].max_frame_rate_denominator = 0;
