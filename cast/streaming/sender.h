...
class Sender final : public SenderPacketRouter::Sender,
 ...
 public:
  ...
  // frame or has decided to skip over it. Note: Frame cancellations may occur
  // out-of-order.
 ...
 private:
  ...
  // the corresponding entry in |pending_frames_| and notifies the
  // FrameCancelObserver.
  ...
  // sent. This is used to compute the network round trip time, when
  // processing Receiver Reports later on.
  static constexpr int kSenderReportHistorySize = 32;
  std::array<SenderReportTiming, kSenderReportHistorySize> report_send_times_{};
  int report_send_times_tail_ = 0;  // Index of next element to overwrite.

  // These control whether the Sender knows it needs to send a key frame to the
