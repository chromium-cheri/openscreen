...
ResponseType GetResponseType(const Json::Value& root_node) {
  ...
  return ResponseTypeFromString(type);
}
...
std::unique_ptr<ReceiverError> ParseError(const Json::Value& value) {
  ...
  if (!json::ParseAndValidateInt(value["code"], &(error->code)) ||
      !json::ParseAndValidateString(value["description"],
                                    &(error->description))) {
    return {};
  }
...
std::unique_ptr<ReceiverStatus> ParseStatus(const Json::Value& value) {
  ...
  if (!json::ParseAndValidateDouble(value["wifiSnr"], &(status->wifi_snr)) ||
      !json::ParseAndValidateIntArray(value["wifiSpeed"],
                                      &(status->wifi_speed))) {
    return {};
  }
  return status;
...
std::unique_ptr<ReceiverResponse> ReceiverResponse::Parse(
  ...
  const bool succeeded = reader->parse(
      ...
      &root_node, &error_msg);
  if (!succeeded) {
    OSP_DVLOG << "Failed to parse reciever message: " << error_msg;
    return nullptr;
  }
  ...
  std::string result;
  if (!root_node ||
      !json::ParseAndValidateInt(root_node["sessionId"],
                                 &(response->session_id_)) ||
      !json::ParseAndValidateInt(root_node["seqNum"],
                                 &(response->sequence_number_)) ||
  ...
  switch (response->type_) {
    ...
    case ResponseType::RPC: {
      std::string raw_rpc;
      if (!json::ParseAndValidateString(root_node["rpc"], &raw_rpc) ||
          !base64::Decode(raw_rpc, &(response->rpc_))) {
        response->valid_ = false;
      }
    } break;
