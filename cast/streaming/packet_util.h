...
// just after the field.
template <typename Integer>
inline void AppendField(Integer value, ByteView* out) {
...
// reserved space.
inline ByteView ReserveSpace(int num_bytes, ByteView* out) {
  const ByteView reserved = out->subspan(0, num_bytes);
