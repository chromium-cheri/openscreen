...
bool Dimensions::ParseAndValidate(const Json::Value& root, Dimensions* out) {
  if (!json::ParseAndValidateInt(root[kWidth], &(out->resolution.width)) ||
      !json::ParseAndValidateInt(root[kHeight], &(out->resolution.height)) ||
...
bool Dimensions::IsValid() const {
  return resolution.IsValid() && frame_rate.is_positive();
}

Json::Value Dimensions::ToJson() const {
  OSP_DCHECK(IsValid());
  Json::Value root = resolution.ToJson();
...
int Dimensions::effective_bit_rate() const {
  return resolution.width * resolution.height * static_cast<double>(frame_rate);
}
