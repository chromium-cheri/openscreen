...
constexpr int kAesHexBytesSize = 16;
ErrorOr<std::array<uint8_t, kAesHexBytesSize>> ParseAesHexBytes(
    const Json::Value& parent,
    const std::string& field) {
  const int aes_string_length = kAesHexBytesSize * kHexDigitsPerByte;
  auto hex_string = ParseString(parent, field);
  if (!hex_string) {
    return hex_string.error();
  }

  uint64_t quads[kHexDigitsPerByte];
  int chars_scanned;
  if (hex_string.value().size() == aes_string_length &&
      sscanf(hex_string.value().c_str(), "%16" SCNx64 "%16" SCNx64 "%n",
             &quads[0], &quads[1], &chars_scanned) == kHexDigitsPerByte &&
      chars_scanned == aes_string_length &&
      std::none_of(hex_string.value().begin(), hex_string.value().end(),
                   [](char c) { return std::isspace(c); })) {
    std::array<uint8_t, kAesHexBytesSize> bytes;
...
std::string AesHexBytesToString(
    const std::array<uint8_t, kAesHexBytesSize>& bytes) {
  std::array<char, kAesHexBytesSize * kHexDigitsPerByte> buffer;
  for (size_t i = 0; i < kAesHexBytesSize; ++i) {
