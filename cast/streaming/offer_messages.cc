...
ErrorOr<RtpPayloadType> ParseRtpPayloadType(const Json::Value& parent, const std::string& field) {
...
ErrorOr<int> ParseRtpTimebase(const Json::Value& parent, const std::string& field) {
...
ErrorOr<std::array<uint8_t, 16>> ParseAesHexBytes(const Json::Value& parent, const std::string& field) {
...
ErrorOr<Stream> ParseStream(const Json::Value& value, Stream::Type type) {
  ...
  return Stream{index.value(),
                type,
                ValueOrDefault(channels, type == Stream::Type::kAudioSource ? kDefaultNumAudioChannels : kDefaultNumVideoChannels),
                codec_name.value(),
...
ErrorOr<std::vector<Resolution>> ParseResolutions(const Json::Value& parent, const std::string& field) {
