...
ErrorOr<Json::Value> Offer::ToJson() const {
  ...
  ErrorOr<const char*> parsed_cast_mode =
      GetEnumName(kCastModeNames, cast_mode);
  if (parsed_cast_mode.is_error()) {
    return std::move(parsed_cast_mode.error());
  }
  root["castMode"] = parsed_cast_mode.value();
