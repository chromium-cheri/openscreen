...
bool ParseRtpPayloadType(const Json::Value& value, RtpPayloadType* out) {
  int t;
  if (!json::ParseAndValidateInt(value, &t)) {
    return false;
  }
...
bool ParseRtpTimebase(const Json::Value& value, int* out) {
  std::string raw_timebase;
  if (!json::ParseAndValidateString(value, &raw_timebase)) {
    return false;
  }
...
bool ParseAesHexBytes(const Json::Value& value,
                      std::array<uint8_t, kAesBytesSize>* out) {
  std::string hex_string;
  if (!json::ParseAndValidateString(value, &hex_string)) {
    OSP_LOG_WARN << "Missing hex string";
    return false;
  }
...
ErrorOr<Stream> ParseStream(const Json::Value& value, Stream::Type type) {
  ...
  if (!json::ParseAndValidateInt(value["index"], &stream.index) ||
      !json::ParseAndValidateUint(value["ssrc"], &stream.ssrc) ||
      !ParseRtpPayloadType(value["rtpPayloadType"], &stream.rtp_payload_type) ||
      !ParseRtpTimebase(value["timeBase"], &stream.rtp_timebase)) {
    return Error(Error::Code::kJsonParseError,
                 "Offer stream has missing or invalid mandatory field");
  }

  if (!json::ParseAndValidateInt(value["channels"], &stream.channels)) {
  ...
  if (!ParseAesHexBytes(value["aesKey"], &stream.aes_key) ||
      !ParseAesHexBytes(value["aesIvMask"], &stream.aes_iv_mask)) {
  ...
  int target_delay;
  if (json::ParseAndValidateInt(value["targetDelay"], &target_delay)) {
  ...
  if (!json::ParseBool(value["receiverRtcpEventLog"],
                       &stream.receiver_rtcp_event_log)) {
    stream.receiver_rtcp_event_log = false;
  }
  if (!json::ParseAndValidateString(value["receiverRtcpDscp"],
...
ErrorOr<AudioStream> ParseAudioStream(const Json::Value& value) {
  ...
  if (!json::ParseAndValidateInt(value["bitRate"], &audio_stream.bit_rate) ||
      audio_stream.bit_rate < 0 ||
      !json::ParseAndValidateString(value[kCodecName], &codec_name)) {
...
bool ParseResolutions(const Json::Value& value, std::vector<Resolution>* out) {
  out->clear();
...
ErrorOr<VideoStream> ParseVideoStream(const Json::Value& value) {
  ...
  std::string codec_name;
  if (!json::ParseAndValidateString(value[kCodecName], &codec_name)) {
  ...
  if (json::ParseAndValidateString(value["maxFrameRate"],
                                   &raw_max_frame_rate)) {
  ...
  ParseResolutions(value["resolutions"], &video_stream.resolutions);
  json::ParseAndValidateString(value["profile"], &video_stream.profile);
  json::ParseAndValidateString(value["protection"], &video_stream.protection);
  json::ParseAndValidateString(value["level"], &video_stream.level);
  json::ParseAndValidateString(value["errorRecoveryMode"],
                               &video_stream.error_recovery_mode);
  if (!json::ParseAndValidateInt(value["maxBitRate"],
                                 &video_stream.max_bit_rate)) {
...
ErrorOr<Offer> Offer::Parse(const Json::Value& root) {
  ...
  for (Json::ArrayIndex i = 0; i < supported_streams.size(); ++i) {
    ...
    std::string type;
    if (!json::ParseAndValidateString(fields[kStreamType], &type)) {
    ...
    if (type == kAudioSourceType) {
      ...
      if (!stream) {
        if (stream.error().code() == Error::Code::kUnknownCodec) {
          ...
          OSP_LOG_WARN << "Dropping stream due to error: " << stream.error();
          return stream.error();
        }
      }
