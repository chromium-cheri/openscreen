...
std::string AesHexBytesToString(const std::array<uint8_t, N>& bytes) {
  ...
  std::array<char, N * kBytesToStringFactor + 1> buffer;

  for (size_t i = 0; i < N; ++i) {
    // Each call will write three chars (incl. a null terminator).
    const int r = snprintf(buffer.data() + i * kBytesToStringFactor,
                           kBytesToStringFactor + 1, "%02x", bytes[i]);
  ...
  // a copy is necessary here. Note the "- 1" for removing the null terminator.
...
ErrorOr<VideoStream> ParseVideoStream(const Json::Value& value) {
  ...
  int max_frame_rate_numerator = kDefaultMaxFrameRate;
  int max_frame_rate_denominator = 1;
  if (max_frame_rate.is_value()) {
    auto parsed = Fraction::FromString(max_frame_rate.value());
    if (parsed.is_value() && parsed.value().is_finite() &&
        parsed.value().is_positive()) {
      max_frame_rate_numerator = parsed.value().numerator;
      max_frame_rate_denominator = parsed.value().denominator;
    }
  }
...
ErrorOr<Json::Value> VideoStream::ToJson() const {
  if (max_bit_rate <= 0 || max_frame_rate_numerator <= 0 ||
      max_frame_rate_denominator <= 0) {
  ...
  stream["maxFrameRate"] =
      Fraction{max_frame_rate_numerator, max_frame_rate_denominator}.ToString();
