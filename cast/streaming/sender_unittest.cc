...
class MockFrameCancelObserver : public Sender::FrameCancelObserver {
 public:
...
TEST_F(SenderTest, SendsFramesEfficiently) {
  ...
  NiceMock<MockFrameCancelObserver> observer;
  ...
  sender()->SetFrameCancelObserver(&observer);
...
// Tests that the Sender rejects frames if too many (i.e., by count, not media
// duration) are in-flight at once.
TEST_F(SenderTest, RejectsEnqueuingIfTooManyFramesAreInFlight) {
  ...
  EXPECT_EQ(Sender::MAX_FRAMES_IN_FLIGHT,
  ...
  // Finally, attempting to equeue another frame should fail again.
...
// client, but only when producing a key frame is absolutely necessary.
TEST_F(SenderTest, ManagesReceiverPictureLossWorkflow) {
  ...
  // loss to the Sender. The Sender should then return true when NeedsKeyFrame()
  // is called.
  EXPECT_FALSE(sender()->NeedsKeyFrame());
  ...
  // Send a non-key frame, and expect NeedsKeyFrame() still returns true. This
  // accounts for the case where a client's media encoder had frames in its
  // processing pipeline before NeedsKeyFrame() began returning true.
  ...
  // its picture loss again to the Sender. NeedsKeyFrame() should NOT return
  // true because the Sender knows the Receiver hasn't acknowledged the key
  // frame (just sent) yet.
  ...
  // Now, simulate the Receiver geting the key frame, but NOT recovering. This
  // should cause NeedsKeyFrame() to return true again.
