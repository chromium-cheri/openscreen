...
class MockMessager {
 public:
  ...
  WeakPtr<MockMessager> GetWeakPtr() { return weak_factory_.GetWeakPtr(); }
 ...
 private:
  ...
  WeakPtrFactory<MockMessager> weak_factory_{this};
};
...
class RpcBrokerTest : public testing::Test {
 protected:
  void SetUp() override {
    fake_messager_ = std::make_unique<MockMessager>();
    ASSERT_FALSE(fake_messager_->received_count());

    rpc_broker_ = std::make_unique<RpcBroker>(
        [p = fake_messager_->GetWeakPtr()](std::vector<uint8_t> message) {
          if (p) {
            p->OnSentRpc(message);
          }
        });
    ...
    rpc_broker_->RegisterMessageReceiverCallback(
        handle, [p = fake_messager_->GetWeakPtr()](const RpcMessage& message) {
          if (p) {
  ...
  std::unique_ptr<MockMessager> fake_messager_;
