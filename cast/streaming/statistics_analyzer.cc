...
void StatisticsAnalyzer::RecordFrameLatencies(const FrameEvent frame_event) {
  ...
  switch (frame_event.type) {
    case StatisticsEventType::kFrameCaptureBegin:
      OSP_LOG_ERROR << __func__ << ": capture begin event. timestamp = "
                    << ToString(frame_event.timestamp)
    ...
    case StatisticsEventType::kFramePlayedOut: {
      ...
      if (it->second.capture_begin_time != Clock::time_point::min()) {
        ...
        OSP_LOG_ERROR << __func__ << ": e2e_latency=" << ToString(e2e_latency)
                      << ", capture_begin_time="
                      << ToString(it->second.capture_begin_time)
                      << ", timestamp=" << ToString(frame_event.timestamp)
                      << ", adj_timestamp=" << ToString(*adjusted_timestamp);
