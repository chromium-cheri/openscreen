...
ErrorOr<Json::Value> ReceiverMessage::ToJson() const {
  if (type == ReceiverMessage::Type::kUnknown) {
    return Error(Error::Code::kTypeError,
                 "Unknown message type, cannot serialize");
  }

  Json::Value root;
  ErrorOr<const char*> name = GetEnumName(kMessageTypeNames, type);
  OSP_DCHECK(name.is_value());
  root[kMessageType] = name.value();
