...
// Generally speaking, due to the range of human hearing (20-20kHz) and the
// Nyquist sampling theorem, 44,100Hz captures should capture all the fidelity
// of the audio source.
...
struct Audio {
  inline bool operator==(const Audio& other) const {
    return std::tie(bit_rate_limits, max_delay, max_channels,
                    max_sample_rate) ==
...
constexpr int kDefaultVideoMaxPixelsPerSecond =
    kDefaultMaxDimensions.effective_bit_rate() / 8;
