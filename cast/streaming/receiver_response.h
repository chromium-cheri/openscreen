...
#ifndef CAST_STREAMING_RECEIVER_RESPONSE_H_
#define CAST_STREAMING_RECEIVER_RESPONSE_H_
...
class ReceiverResponse {
 public:
  ...
  // OSP_DCHECK rules below.
  const Answer& answer() const {
    OSP_DCHECK(valid_ && type_ == ResponseType::ANSWER);
    return *answer_;
  }

  const std::string& rpc() const {
    OSP_DCHECK(valid_ && type_ == ResponseType::RPC);
    return rpc_;
  }

  const ReceiverStatus& status() const {
    OSP_DCHECK(valid_ && type_ == ResponseType::STATUS_RESPONSE);
    return *status_;
  }

  const ReceiverCapability& capabilities() const {
    OSP_DCHECK(valid_ && type_ == ResponseType::CAPABILITIES_RESPONSE);
 ...
 private:
  ...
  // value. Currently, we enforce this by OSP_DCHECKing on their accessors,
  // instead of subclasses or a sum type/union/variant. ResponseType associated
  // with each type is documented below.
...
#endif  // CAST_STREAMING_RECEIVER_RESPONSE_H_
