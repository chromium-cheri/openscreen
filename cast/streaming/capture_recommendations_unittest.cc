...
constexpr Recommendations kDefaultRecommendations{
    ...
    Video{BitRateLimits{300000, 1920 * 1080 * 30}, Dimensions{320, 240, 30},
          Dimensions{1920, 1080, 30}, true, milliseconds(4000),
          1920 * 1080 * 30 / 8}};
...
constexpr DisplayDescription kValidDisplayTiny{
    cast::Dimensions{300, 200, SimpleFraction{30, 1}}, AspectRatio{3, 2},
    AspectRatioConstraint::kFixed};

constexpr DisplayDescription kValidDisplayMismatched{
    cast::Dimensions{300, 200, SimpleFraction{30, 1}}, AspectRatio{3, 4},
    AspectRatioConstraint::kFixed};
...
constexpr Constraints kValidConstraintsHighEnd{
    ...
    {6000000, cast::Dimensions{640, 480, SimpleFraction{30, 1}},
     cast::Dimensions{3840, 2160, SimpleFraction{144, 1}}, 600000, 6000000,
...
TEST(CaptureRecommendationsTest, UsesDefaultsIfNoReceiverInformationAvailable) {
  EXPECT_EQ(kDefaultRecommendations, Recommend(Answer{}));
}
...
TEST(CaptureRecommendationsTest, OnlyDimensions) {
  ...
  EXPECT_EQ(expected, Recommend(answer));
}
...
TEST(CaptureRecommendationsTest, MismatchedDisplayAndAspectRatio) {
  Recommendations expected = kDefaultRecommendations;
  expected.video.minimum = Dimensions{150, 200, 30.0};
...
TEST(CaptureRecommendationsTest, TinyDisplay) {
  Recommendations expected = kDefaultRecommendations;
  expected.video.minimum = Dimensions{300, 200, 30.0};
...
// defaults we will accept them, with the exception of maximum dimensions
// exceeding 1080P.
...
TEST(CaptureRecommendationsTest, HandlesLowEnd) {
  const Recommendations kExpected{
      ...
      Video{BitRateLimits{300000, 1000000}, Dimensions{320, 240, 30},
            Dimensions{1200, 800, 30}, true, milliseconds(1000), 60000}};
  Answer answer;
...
TEST(CaptureRecommendationsTest, UsesIntersectionOfDisplayAndConstraints) {
  const Recommendations kExpected{
      Audio{BitRateLimits{96000, 500000}, milliseconds(6000), 5, 96100},
      Video{BitRateLimits{600000, 6000000}, Dimensions{640, 480, 30},
            // Max dimensions should be 1080P, since that's the display
