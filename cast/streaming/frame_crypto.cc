...
EncryptedFrame FrameCrypto::Encrypt(const EncodedFrame& encoded_frame) const {
  ...
  EncryptCommon(encoded_frame.frame_id, encoded_frame.data,
                absl::Span<uint8_t>(result.owned_data_.data(),
                                    result.owned_data_.size()));
  return result;
}
...
void FrameCrypto::Decrypt(const EncryptedFrame& encrypted_frame,
  ...
  if (encrypted_frame.data.size() < encoded_frame->data.size()) {
    encoded_frame->data =
        ByteView(encoded_frame->data.data(), encrypted_frame.data.size());
  }
  EncryptCommon(encrypted_frame.frame_id, encrypted_frame.data,
                absl::Span<uint8_t>(encoded_frame->data.data(),
