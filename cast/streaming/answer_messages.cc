...
// 300kbps is sender default
static constexpr int kDefaultMinBitRate = 300 * 1000;
...
ErrorOr<SimpleFraction> ParseSimpleFraction(const Json::Value& value) {
  if (value.isInt()) {
...
ErrorOr<AudioConstraints> AudioConstraints::Parse(const Json::Value& root) {
  ...
  if (!min_bit_rate) {
    return std::move(min_bit_rate.error());
  }
...
ErrorOr<Dimensions> Dimensions::Parse(const Json::Value& root) {
  ...
  auto frame_rate = ParseSimpleFraction(root["frameRate"]);
  if (!frame_rate) {
...
ErrorOr<VideoConstraints> VideoConstraints::Parse(const Json::Value& root) {
  ...
  auto min_bit_rate = ParseInt(root["minBitRate"]);
  if (!min_bit_rate) {
    min_bit_rate = kDefaultMinBitRate;
  }
...
ErrorOr<Answer> Answer::Parse(const Json::Value& root) {
  ...
  // In practice, many receivers are non-compliant and do not send
  // these arrays, even though they are non-optional.
  ...
  const bool supports_wifi_status_reporting =
  ...
  return Answer{cast_mode,
                ...
                supports_wifi_status_reporting,
