...
// by the receiver, the sender will use defaults stored below. This should
// represent the true operational minimum. See kDefaultAudioMinBitRate and
// kDefaultVideoMinBitRate below.
...
// Sender constraint means that the sender must provide video frames of a fixed
// aspect ratio. If this value is set, the dimensions object must be passed or
// an error case will occur.
...
// stream indicated in kSendIndexes. Stored as a uint32_t, any value is valid.
...
template <typename T>
Optional<T> ParseOptional(const Json::Value& value) {
  T tentative_out;
  if (T::ParseAndValidate(value, &tentative_out)) {
    return Optional<T>(std::move(tentative_out));
  }
  return {};
}
...
bool VideoConstraints::ParseAndValidate(const Json::Value& root,
  ...
  out->min_dimensions = ParseOptional<Dimensions>(root[kMinDimensions]);
  return out->IsValid();
}
...
bool DisplayDescription::ParseAndValidate(const Json::Value& root,
  ...
  out->dimensions = ParseOptional<Dimensions>(root[kDimensions]);
  out->aspect_ratio = ParseOptional<AspectRatio>(root[kAspectRatio]);
...
bool Answer::ParseAndValidate(const Json::Value& root, Answer* out) {
  ...
  // the valid case.
  out->constraints = ParseOptional<Constraints>(root[kConstraints]);
  out->display = ParseOptional<DisplayDescription>(root[kDisplay]);
