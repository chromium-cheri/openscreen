...
class ReceiverSession final : public Environment::SocketSubscriber {
 public:
  ...
  // The screen dimension in pixels and its frame rate, used for setting
  // both physical screen dimensions as well as minimum and maximum achievable
  // dimensions.
  struct DimensionLimits {
    ...
    // Frame rate, provided as a ratio for use with the mirroring spec.
    std::pair<int, int> frame_rate = {kDefaultFrameRate, 1};
  };
  ...
  struct VideoLimits {
    ...
    DimensionLimits max_dimensions = {1920, 1080, {kDefaultFrameRate, 1}};
