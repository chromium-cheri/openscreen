...
ClockOffsetEstimator::BoundCalculator::BoundCalculator() = default;
ClockOffsetEstimator::BoundCalculator::~BoundCalculator() = default;

void ClockOffsetEstimator::BoundCalculator::SetSent(RtpTimeTicks rtp,
                                                    uint16_t packet_id,
                                                    bool audio,
                                                    Clock::time_point t) {
...
void ClockOffsetEstimator::BoundCalculator::SetReceived(RtpTimeTicks rtp,
                                                        uint16_t packet_id,
                                                        bool audio,
                                                        Clock::time_point t) {
  const uint64_t key = MakeEventKey(rtp, packet_id, audio);
...
void ClockOffsetEstimator::BoundCalculator::UpdateBound(
...
void ClockOffsetEstimator::BoundCalculator::CheckUpdate(uint64_t key) {
...
ClockOffsetEstimator::ClockOffsetEstimator(ClockOffsetEstimator&&) = default;
ClockOffsetEstimator& ClockOffsetEstimator::operator=(ClockOffsetEstimator&&) =
    default;
ClockOffsetEstimator::~ClockOffsetEstimator() = default;

void ClockOffsetEstimator::OnFrameEvent(const FrameEvent& frame_event) {
  OSP_LOG_ERROR << __func__ << ": timestamp=" << ToString(frame_event.timestamp)
                << ", rtp_timestamp=" << frame_event.rtp_timestamp;
...
bool ClockOffsetEstimator::GetReceiverOffsetBounds(
...
absl::optional<Clock::duration> ClockOffsetEstimator::GetEstimatedOffset()
    const {
  Clock::duration lower_bound;
  ...
  const Clock::duration estimate = (upper_bound - lower_bound) / 2;
  OSP_LOG_ERROR << __func__ << ": lower_bound=" << ToString(lower_bound)
                << ", upper_bound=" << ToString(upper_bound)
                << ", estimate=" << ToString(estimate);
  return estimate;
}

void ClockOffsetEstimator::OnPacketEvent(const PacketEvent& packet_event) {
