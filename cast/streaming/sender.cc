...
int Sender::GetInFlightFrameCount() const {
  ...
  for (FrameId frame_id = checkpoint_frame_id_ + 1;
    ...
    // If CancelPendingFrame() was not yet called for this frame, then the frame
    // is still in-flight.
    if (get_slot_for(frame_id)->frame) {
      ++count;
    }
  }
  return count;
...
Clock::duration Sender::GetInFlightMediaDuration(
  ...
  if (last_enqueued_frame_id_ == checkpoint_frame_id_) {
...
Sender::EnqueueFrameResult Sender::EnqueueFrame(const EncodedFrame& frame) {
  ...
  if (slot->frame) {
    ...
    // last. Thus, if this slot is occupied, then by implication all slots must
    // be occupied. In other words, the "max unacked frames" limit has been
    // reached.
...
absl::Span<uint8_t> Sender::GetRtcpPacketForImmediateSend(
  ...
  if (!result.first.empty()) {
    // Remember the StatusReportId that refers to this report. This information
    // is used later, in OnReceiverReport(), to compute the current network
    // round trip time.
    SenderReportTiming& entry = report_send_times_[report_send_times_tail_];
    entry.id = result.second;
    entry.when = sender_report.reference_time;
    report_send_times_tail_ =
        (report_send_times_tail_ + 1) % kSenderReportHistorySize;
  }
...
void Sender::OnReceiverReport(const RtcpReportBlock& receiver_report) {
  ...
  const auto it =
      std::find_if(report_send_times_.begin(), report_send_times_.end(),
                   [&](const SenderReportTiming& entry) {
                     return entry.id == receiver_report.last_status_report_id &&
                            entry.when != SenderPacketRouter::kNever;
                   });
  if (it == report_send_times_.end()) {
    return;  // Not found. It's likely the Receiver Report is too old.
  }
  const Clock::time_point& when_sender_report_was_sent = it->when;
  ...
  const Clock::duration measurement =
      std::max((rtcp_packet_arrival_time_ - when_sender_report_was_sent) -
                   duration_cast<Clock::duration>(
                       receiver_report.delay_since_last_report),
    ...
    if (nack_it->packet_id == kAllPacketsLost) {
      ...
      do {
        if (nack_it->packet_id < range_end) {
          ...
          OSP_LOG_WARN
              << "Ingoring NACK for packet that doesn't exist in frame "
...
Sender::ChosenPacketAndWhen Sender::ChooseKickstartPacket() {
  ...
  // |target_playout_delay_|. The reason for the specific ratio here is based
  // on lost knowledge (from legacy implementations); but it makes sense
  // (i.e., to be a good "network citizen") to be less agressive for larger
  // playout delay windows, and more aggressive for shorter ones to avoid
  // too-late packet arrivals.
