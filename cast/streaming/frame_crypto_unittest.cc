...
TEST(FrameCryptoTest, EncryptsAndDecryptsFrames) {
  ...
  frame0.data = reinterpret_cast<const uint8_t*>(kPayload);
  frame0.data_len = sizeof(kPayload);
  ...
  EXPECT_NE(0, memcmp(frame0.const_data(), encrypted_frame0.const_data(),
                      frame0.data_len));
  ...
  EXPECT_NE(0, memcmp(frame1.const_data(), encrypted_frame1.const_data(),
                      frame1.data_len));
  ASSERT_EQ(encrypted_frame0.data_len, encrypted_frame1.data_len);
  EXPECT_NE(0,
            memcmp(encrypted_frame0.const_data(), encrypted_frame1.const_data(),
  ...
  EXPECT_EQ(0, memcmp(frame0.const_data(), decrypted_frame0.const_data(),
                      frame0.data_len));
  ...
  EXPECT_EQ(0, memcmp(frame1.const_data(), decrypted_frame1.const_data(),
                      frame1.data_len));
}
