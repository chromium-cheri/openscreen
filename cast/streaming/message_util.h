...
inline openscreen::Error CreateParseError(const std::string& type) {
  return openscreen::Error(openscreen::Error::Code::kJsonParseError, "Failed to parse " + type);
}

inline openscreen::Error CreateParameterError(const std::string& type) {
  return openscreen::Error(openscreen::Error::Code::kParameterInvalid, "Invalid parameter: " + type);
}

inline openscreen::ErrorOr<bool> ParseBool(const Json::Value& parent, const std::string& field) {
...
inline openscreen::ErrorOr<int> ParseInt(const Json::Value& parent, const std::string& field) {
  const Json::Value& value = parent[field];
  if (!value.isInt()) {
...
inline openscreen::ErrorOr<uint32_t> ParseUint(const Json::Value& parent, const std::string& field) {
  const Json::Value& value = parent[field];
  if (!value.isUInt()) {
...
inline openscreen::ErrorOr<std::string> ParseString(const Json::Value& parent, const std::string& field) {
  const Json::Value& value = parent[field];
  if (!value.isString()) {
