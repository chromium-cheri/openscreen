...
int ValueOrDefault(int value, int default_) {
  return (value > 0) ? value : default_;
}
...
AudioStream CreateStream(int index, const AudioCaptureOption& config) {
  return AudioStream{
      Stream{index,
             ...
             {} /* receiver_rtcp_dscp */,
             config.sample_rate},
      ValueOrDefault(config.bit_rate,
...
SenderSession::SenderSession(IPAddress remote_address,
  ...
  message_port_->SetClient(this, std::to_string(session_id_));
}
