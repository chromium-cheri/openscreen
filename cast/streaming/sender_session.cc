...
std::vector<AudioStream> CreateStreamList(
    const std::vector<AudioConfig>& configs) {
  return {};
}

std::vector<VideoStream> CreateStreamList(
...
Offer SenderSession::ConstructOffer(
  ...
  return Offer{{CastMode::Type::kMirroring},
               ...
               CreateStreamList(audio_configs),
               CreateStreamList(video_configs)};
}
