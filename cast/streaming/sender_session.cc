...
static const Error kDefaultCapabilitiesResponseError(
...
void SenderSession::OnAnswer(ErrorOr<ReceiverMessage> message) {
  if (!message) {
    ...
    if (message.error().code() == Error::Code::kMessageTimeout) {
      config_.client->OnError(this, kAnswerTimeoutError);
    } else {
  ...
  if (!message.value().valid ||
    ...
    HandleErrorMessage(message.value(), kDefaultAnswerError);
    return;
  }
...
void SenderSession::OnCapabilitiesResponse(ErrorOr<ReceiverMessage> message) {
  ...
  if (!message.value().valid ||
    ...
    HandleErrorMessage(message.value(), kDefaultCapabilitiesResponseError);
    return;
  }
...
void SenderSession::OnRpcMessage(ErrorOr<ReceiverMessage> message) {
  ...
  if (!message.value().valid ||
    ...
    HandleErrorMessage(message.value(), kDefaultRpcError);
    return;
  }
...
void SenderSession::HandleErrorMessage(ReceiverMessage message,
  ...
  if (absl::holds_alternative<ReceiverError>(message.body)) {
    ...
    // If we were unable to successfully parse the error code from the receiver,
    // fallback to the default error code.
