...
SenderSession::SenderSession(std::string sender_id,
...
void SenderSession::OnMessage(const std::string& sender_id,
  ...
  if (message_namespace != kCastWebrtcNamespace) {
    OSP_DLOG_INFO << "Received message from unhandled namespace: "
                  << message_namespace;
    return;
  }
...
void SenderSession::SpawnVideoSender(ConfiguredSenders* senders,
  ...
  const VideoCaptureOption& config =
      current_negotiation_
          ->video_configs[send_index -
                          current_negotiation_->audio_configs.size()];
  const RtpPayloadType payload_type = GetPayloadType(config.codec);
...
ConfiguredSenders SenderSession::SpawnSenders(const Answer& answer) {
  ...
  for (size_t i = 0; i < answer.send_indexes.size(); ++i) {
    ...
    if (send_index < current_negotiation_->audio_configs.size()) {
      SpawnAudioSender(&senders, receiver_ssrc, send_index, i);
    } else if (send_index < (current_negotiation_->audio_configs.size() +
                             current_negotiation_->video_configs.size())) {
      SpawnVideoSender(&senders, receiver_ssrc, send_index, i);
    }
  }
  return senders;
}
