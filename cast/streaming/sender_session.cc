...
Offer CreateRemotingOffer(const AudioCaptureConfig& audio_config,
  ...
  audio_stream.codec = AudioCodec::kRemote;
  audio_stream.stream.rtp_payload_type =
      GetPayloadType(AudioCodec::kRemote, use_android_rtp_hack);
  offer.audio_streams.push_back(std::move(audio_stream));

  VideoStream video_stream =
      CreateStream(1, video_config, use_android_rtp_hack);
  video_stream.codec = VideoCodec::kRemote;
  video_stream.stream.rtp_payload_type =
      GetPayloadType(VideoCodec::kRemote, use_android_rtp_hack);
  ...
  for (MediaCapability c : capability.media_capabilities) {
    switch (c) {
      ...
      case MediaCapability::kVideo:
        out.video.push_back(VideoCapability::kBaselineSet);
        break;
...
void SenderSession::OnAnswer(ReceiverMessage message) {
  ...
  if (current_negotiation_->offer.cast_mode == CastMode::kMirroring) {
    ...
    config_.client->OnMirroringNegotiated(
...
void SenderSession::OnCapabilitiesResponse(ReceiverMessage message) {
  ...
  broker_ = std::make_unique<RpcBroker>([this](std::string message) {
...
void SenderSession::OnRpcMessage(ReceiverMessage message) {
  ...
  // The receiver message body should be a std::string-held byte array
  // that has been successfully base64 decoded--meaning it's ready for
  // access by protobuf.
  broker_->ProcessMessageFromRemote(absl::get<std::string>(message.body));
}
