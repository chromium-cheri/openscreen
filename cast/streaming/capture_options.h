...
struct AudioCaptureOption {
  ...
  // of "zero" suggests that the bitrate should be automatically selected.
  int bit_rate = 0;
...
struct DisplayResolution {
  // Width in pixels.
  int width = k1080pWidth;

  // Height in pixels.
  int height = k1080pHeight;
};
...
struct VideoCaptureOption {
  ...
  // zero means that the maximum bit rate should be automatically selected.
  int max_bit_rate = 0;
