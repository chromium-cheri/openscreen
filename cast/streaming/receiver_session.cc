...
void ReceiverSession::OnEnvironmentReady() {
  if (pending_session_) {
...
void ReceiverSession::OnEnvironmentInvalidated(Error error) {
  if (pending_session_) {
  ...
  client_->OnError(
      this,
      Error(Error::Code::kEnvironmentFailure,
            "The environment has been invalidated and should be replaced."));
}
