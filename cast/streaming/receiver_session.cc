...
void SendErrorAnswerReply(ReceiverSessionMessager* messager,
                          int sequence_number,
                          const char* message) {
  const Error error(Error::Code::kParseError, message);
  OSP_DLOG_WARN << message;
  messager->SendMessage(ReceiverMessage{
...
ReceiverSession::ReceiverSession(Client* const client,
    ...
    : client_(client),
      ...
      messager_(message_port,
                session_id_,
                // TODO: where should I get this from?
                "some sender somewhere",
                [this](Error error) {
...
void ReceiverSession::OnOffer(SenderMessage message) {
  ...
  if (!message.valid) {
    SendErrorAnswerReply(&messager_, message.sequence_number,
...
Answer ReceiverSession::ConstructAnswer(
    const SenderMessage& offer_message,
