...
// TODO(jophba): move offer/answer logic?
// OFFER message.
...
ReceiverSession::ReceiverSession(Client* client, ReceiverPacketRouter* packet_router,
  channel::VirtualConnectionRouter* connection_router)
    : client_(client), packet_router_(packet_router), connection_router_(connection_router) {
  OSP_DCHECK(client_);
...
void ReceiverSession::OnMessage(channel::VirtualConnectionRouter* router,
  ...
  // Goal: produce an OnOffer event
  channel::VirtualConnection {message.destination_id(), message.source_id(), socket->id()};

  OSP_DCHECK(message.payload_type() == channel::PayloadType::String);
  const ErrorOr<Json::Value> message_payload = json_reader_.Read(message.payload_utf8());a64l
