...
ReceiverSession::ConfiguredReceivers ReceiverSession::SpawnReceivers(
  ...
  if (properties.selected_video) {
    ...
    video_config = VideoCaptureConfig{
        ...
        {properties.selected_video->max_frame_rate.numerator,
    ...
    for (const auto& limit : preferences_.video_limits) {
      if (limit.codec == properties.selected_video->codec ||
        ...
        constraints.video = VideoConstraints{
            limit.max_pixels_per_second,
            absl::nullopt, /* min dimensions */
            Dimensions{
                {limit.max_dimensions.width, limit.max_dimensions.height},
                {limit.max_dimensions.frame_rate.first,
                 limit.max_dimensions.frame_rate.second},
            },
            limit.min_bit_rate,
            limit.max_bit_rate,
            limit.max_delay,
  ...
  if (preferences_.display_description) {
    ...
    display = DisplayDescription{
        Dimensions{
            ...
            SimpleFraction{d->dimensions.frame_rate.first,
                           d->dimensions.frame_rate.second},
        },
        absl::nullopt,
        d->can_scale_content ? AspectRatioConstraint::kVariable
