...
RemotingPreferences::RemotingPreferences(bool supports_chrome_audio_codecs,
                                         bool supports_4k)
    : supports_chrome_audio_codecs(supports_chrome_audio_codecs),
...
void ReceiverSession::InitializeSession(const SessionProperties& properties) {
  ...
  if (properties.mode == CastMode::kMirroring) {
    ...
    broker_ = std::make_unique<RpcBroker>([this](std::vector<uint8_t> message) {
      Error error = this->messager_.SendMessage(
