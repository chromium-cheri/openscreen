...
void ReceiverSession::OnSocketBound() {
  if (pending_session_) {
...
void ReceiverSession::OnSocketClose(Error error) {
  if (pending_session_) {
...
void ReceiverSession::OnOffer(SenderMessage message) {
  ...
  switch (environment_->socket_state()) {
    // If the UDP socket failed to bind at all or is already closed, we can
    // respond right away.
    case UdpSocketState::kClosed:
      SendErrorAnswerReply(message.sequence_number,
                           "UDP socket is closed, likely due to a bind error.");
      break;

    // If successfully bound, we can send an Answer message right away.
    case UdpSocketState::kBound:
      InitializeSession(*properties);
      break;

    // Else we need to store the properties we just created until we get a
    // bound or error event.
    case UdpSocketState::kNotReady:
