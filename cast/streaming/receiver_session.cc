...
ConfiguredReceivers ReceiverSession::SpawnReceivers(const AudioStream* audio,
  ...
  return ConfiguredReceivers{
      ...
      // after Estelle-Fuchsia migration
      ConfiguredReceiver<AudioStream>{current_audio_receiver_.get(),
                                      current_audio_receiver_->config(),
                                      *audio},
      ConfiguredReceiver<VideoStream>{current_video_receiver_.get(),
                                      current_video_receiver_->config(),
                                      *video}};
}
