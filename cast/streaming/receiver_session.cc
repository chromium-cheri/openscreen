...
// mutate them.
template <typename T>
bool IsMissingCodecs(std::vector<T> first, std::vector<T> second) {
  ...
  results.reserve(second.size());

  auto diff_it = set_difference(second.begin(), second.end(), first.begin(),
                                first.end(), results.begin());
  return diff_it != results.end();
}
...
bool HasLessRestrictiveLimits(std::vector<T> first, std::vector<T> second) {
  ...
  T* second_applies_to_all_codecs_struct =
      !second.empty() && second.front().applies_to_all_codecs
          ? &(*second_it++)
          : &fake_applies_to_all_codecs_struct;
  if (!first_applies_to_all_codecs_struct->SupportsAllAllowedBy(
  ...
  while (first_it != first.end() || second_it != second.end()) {
    ...
    if (!first_codec_struct->SupportsAllAllowedBy(*second_codec_struct)) {
      return false;
    }
  }
...
bool ReceiverSession::VideoLimits::SupportsAllAllowedBy(
  ...
  return (applies_to_all_codecs == second.applies_to_all_codecs) &&
         ...
         (max_dimensions.SupportsAllAllowedBy(second.max_dimensions));
}

bool ReceiverSession::AudioLimits::SupportsAllAllowedBy(
...
bool ReceiverSession::Display::SupportsAllAllowedBy(
    const ReceiverSession::Display& other) const {
  return dimensions.SupportsAllAllowedBy(other.dimensions) &&
         (can_scale_content || !other.can_scale_content);
}

bool ReceiverSession::RemotingPreferences::SupportsAllAllowedBy(
...
bool ReceiverSession::Preferences::SupportsAllAllowedBy(
    const ReceiverSession::Preferences& other) const {
  // Check simple cases first.
  if ((!!display_description != !!other.display_description) ||
      (display_description && !display_description->SupportsAllAllowedBy(
                                  *other.display_description))) {
    return false;
  } else if (other.remoting &&
             (!remoting || !remoting->SupportsAllAllowedBy(*other.remoting))) {
    return false;
  }
