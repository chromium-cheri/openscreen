...
// packets can be sent together as one lager transmission unit, and this can be
...
class SenderPacketRouter : public Environment::PacketConsumer {
 public:
  ...
  void RecordFeedback(int payload_bytes_acknowledged,
                      Clock::time_point feedback_time_minus_rtt) {
  ...
  absl::optional<int> ComputeEffectiveBitrate() const {
    ...
    return max_burst_bitrate_ / 4;
  }
  absl::optional<int> PredictAvailableBitrate(Clock::time_point begin,
                                              Clock::time_point end) const {
    // return std::min(estimator_.PredictAvailableBitrate(begin, end),
    //                 max_burst_bitrate_);
    return max_burst_bitrate_ / 4;
  }
 ...
 private:
  ...
  // bitrates.
  // TODO(crbug.com/openscreen/55): BandwidthEstimator estimator_;
};
