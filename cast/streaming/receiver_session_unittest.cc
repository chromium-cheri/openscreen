...
TEST_F(ReceiverSessionTest, CanNegotiateWithCustomConstraints) {
  ...
  auto constraints = std::make_unique<Constraints>(Constraints{
      ...
      VideoConstraints{
          3.14159,
          Optional<Dimensions>(Dimensions{320, 240, SimpleFraction{24, 1}}),
          Dimensions{1920, 1080, SimpleFraction{144, 1}}, 3000, 90000000,
          milliseconds(1000)}});

  auto display = std::make_unique<DisplayDescription>(DisplayDescription{
      Optional<Dimensions>(Dimensions{640, 480, SimpleFraction{60, 1}}),
      Optional<AspectRatio>(AspectRatio{16, 9}),
