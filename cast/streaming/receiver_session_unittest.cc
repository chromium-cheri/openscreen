...
TEST_F(ReceiverSessionTest, VideoLimitsSupportsAllAllowedBy) {
  ReceiverSession::VideoLimits first;
  ReceiverSession::VideoLimits second = first;

  EXPECT_TRUE(first.SupportsAllAllowedBy(second));
  EXPECT_TRUE(second.SupportsAllAllowedBy(first));

  first.max_pixels_per_second += 1;
  EXPECT_TRUE(first.SupportsAllAllowedBy(second));
  EXPECT_FALSE(second.SupportsAllAllowedBy(first));
  ...
  EXPECT_FALSE(first.SupportsAllAllowedBy(second));
  EXPECT_TRUE(second.SupportsAllAllowedBy(first));
...
TEST_F(ReceiverSessionTest, AudioLimitsSupportsAllAllowedBy) {
...
TEST_F(ReceiverSessionTest, DisplaySupportsAllAllowedBy) {
...
TEST_F(ReceiverSessionTest, RemotingPreferencesSupportsAllAllowedBy) {
...
TEST_F(ReceiverSessionTest, PreferencesSupportsAllAllowedBy) {
