...
class ReceiverSessionTest : public ::testing::Test {
 public:
  ...
  std::unique_ptr<MockEnvironment> MakeEnvironment() {
    ...
    ON_CALL(*environment_, socket_state())
        .WillByDefault(Return(UdpSocketState::kBound));
    return environment_;
  }
...
TEST_F(ReceiverSessionTest, DelaysAnswerUntilUdpSocketIsBound) {
  EXPECT_CALL(*environment_, socket_state())
      .WillOnce(Return(UdpSocketState::kNotReady));
  ...
  // socket_state will not be called again--we just need to get the bind event.
  EXPECT_CALL(*environment_, GetBoundLocalEndpoint())
...
TEST_F(ReceiverSessionTest, ReturnsErrorAnswerIfSocketIsClosed) {
  EXPECT_CALL(*environment_, socket_state())
      .WillOnce(Return(UdpSocketState::kClosed));

  // Unlike when the socket is not ready, if it is closed we can immediately
  // send an answer.
  message_port_->ReceiveMessage(kValidOfferMessage);
  const auto& messages = message_port_->posted_messages();
  ASSERT_EQ(1u, messages.size());
...
TEST_F(ReceiverSessionTest, ReturnsErrorAnswerIfSocketErrorOccurs) {
  EXPECT_CALL(*environment_, socket_state())
      .WillOnce(Return(UdpSocketState::kNotReady));
  ...
  // Simulate the environment calling back into us with the socket closing.
  session_->OnSocketClose(Error::Code::kSocketBindFailure);
  const auto& messages = message_port_->posted_messages();
  ASSERT_EQ(1u, messages.size());
