...
class RtpPacketizerTest : public testing::Test {
 public:
  ...
  EncryptedFrame CreateFrame(FrameId frame_id,
    ...
    EncodedFrame frame;
    frame.dependency = is_key_frame ? EncodedFrame::Dependency::kKeyFrame
  ...
  void TestGeneratePacket(const EncryptedFrame& frame,
    ...
    EXPECT_EQ(frame.dependency == EncodedFrame::Dependency::kKeyFrame,
              result->is_key_frame);
    ...
    const absl::Span<const uint8_t> expected_bytes(
        frame.data + (packet_id * packet_payload_size), expected_payload_size);
