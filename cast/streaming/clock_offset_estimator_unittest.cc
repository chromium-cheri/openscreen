...
class ClockOffsetEstimatorTest : public ::testing::Test {
 public:
  ClockOffsetEstimatorTest()
      : sender_time_(Clock::now()), receiver_clock_(Clock::now()) {}

  ~ClockOffsetEstimatorTest() override = default;
  ...
  ClockOffsetEstimator estimator_;
};
...
TEST_F(ClockOffsetEstimatorTest, EstimateOffset) {
...
TEST_F(ClockOffsetEstimatorTest, EventCArrivesBeforeEventB) {
  ...
  EXPECT_EQ(milliseconds(70), lower_bound);
  EXPECT_EQ(milliseconds(110), upper_bound);
  EXPECT_GE(kTrueOffset, lower_bound);
  EXPECT_LE(kTrueOffset, upper_bound);
}

TEST_F(ClockOffsetEstimatorTest, MultipleIterations) {
  constexpr milliseconds kTrueOffset(100);
