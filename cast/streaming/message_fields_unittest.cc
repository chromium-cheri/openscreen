...
TEST(MessageFieldsTest, CanParseEnumToString) {
  EXPECT_EQ("aac", CodecToString(AudioCodec::kAac));
  EXPECT_EQ("vp8", CodecToString(VideoCodec::kVp8));
}
...
TEST(MessageFieldsTest, Identity) {
  EXPECT_EQ("opus", CodecToString(StringToAudioCodec("opus")));
  EXPECT_EQ("vp8", CodecToString(StringToVideoCodec("vp8")));
}
