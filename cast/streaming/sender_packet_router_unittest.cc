...
absl::Span<uint8_t> MakeFakePacketWithFlag(char flag,
  ...
  const auto packet_size = sizeof(Clock::duration::rep) + sizeof(flag);
  buffer = buffer.subspan(0, packet_size);
  OSP_CHECK_EQ(buffer.size(), packet_size);
  WriteBigEndian(send_time.time_since_epoch().count(), buffer.data());
  buffer[sizeof(Clock::duration::rep)] = flag;
  return buffer;
}
...
Clock::time_point ParseTimestamp(absl::Span<const uint8_t> fake_packet) {
  Clock::duration send_time_since_origin{};
  if (fake_packet.size() >= sizeof(Clock::duration::rep)) {
    send_time_since_origin = Clock::duration(
        ReadBigEndian<Clock::duration::rep>(fake_packet.data()));
  }
  return Clock::time_point() + send_time_since_origin;
}
...
// Tests that the SenderPacketRouter ignores packets that Senders are not
// expected to process.
TEST_F(SenderPacketRouterTest, IgnoresInvalidInboundPackets) {
  ...
  // It should ignore packets from unexpected sources.
  {
  ...
  // It should ignore garbage packets.
  {
    EXPECT_CALL(*audio_sender(), OnReceivedRtcpPacket(_, _)).Times(0);
    SimulatePacketArrivedNow(kUnexpectedEndpoint,
  ...
  // It should ignore valid RTP packets, since Senders do not process RTP
  // packets.
  {
    EXPECT_CALL(*audio_sender(), OnReceivedRtcpPacket(_, _)).Times(0);
    SimulatePacketArrivedNow(kUnexpectedEndpoint,
  ...
  // It should ignore valid RTCP packets from unknown receivers.
  std::vector<uint8_t> rtcp_packet_not_for_me =
...
TEST_F(SenderPacketRouterTest, SchedulesAndTransmitsRTPBursts) {
  ...
  EXPECT_CALL(*video_sender(), GetRtpPacketForImmediateSend(_, _))
      // First burst (t=0ms).
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 0.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 1.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 2.
      // Second burst (t=10ms).
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 3.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 4.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 5.
      // Third burst (t=20ms).
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 6.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 7.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, packet 8.
      // Fourth burst (t=30ms).
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 0, last packet.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 1, packet 0.
      .WillOnce(Invoke(&MakeFakePacket))  // Frame 1, packet 1.
      // Fifth burst (t=40ms).
      .WillOnce(Invoke(&MakeFakePacket))       // Frame 1, packet 2.
      .WillOnce(Invoke(&MakeFakePacket))       // Frame 1, last packet.
      .WillOnce(Invoke(&ToEmptyPacketBuffer))  // Done for now.
      // Sixth burst (t=65ms; kickstart after a pause in transmission).
      .WillOnce(Invoke(&ToEmptyPacketBuffer));  // Nothing to send.
  ...
  EXPECT_CALL(*video_sender(), GetRtpResumeTime())
      // After the first through fourth bursts, the Sender wants to transmit
      // more right away.
      .WillOnce(Invoke([&] { return env()->now(); }))
      .WillOnce(Invoke([&] { return env()->now(); }))
      .WillOnce(Invoke([&] { return env()->now(); }))
      .WillOnce(Invoke([&] { return env()->now(); }))
      // After the fifth burst, the Sender requests resuming for kickstart
      // later.
      .WillOnce(Return(kickstart_time))
  ...
  // sending again.
  EXPECT_CALL(*video_sender(), GetRtpPacketForImmediateSend(_, _))
      // Seventh burst (t=100ms; resuming after a pause).
...
TEST_F(SenderPacketRouterTest, SchedulesAndTransmitsAccountingForPriority) {
  ...
  EXPECT_CALL(*audio_sender(), GetRtpPacketForImmediateSend(_, _))
      .WillRepeatedly(
          Invoke([&num_audio_get_rtp_calls](Clock::time_point send_time,
                                            absl::Span<uint8_t> buffer) {
  ...
  EXPECT_CALL(*video_sender(), GetRtpPacketForImmediateSend(_, _))
      .WillRepeatedly(
          Invoke([&num_video_get_rtp_calls](Clock::time_point send_time,
                                            absl::Span<uint8_t> buffer) {
            // Alternate between returning a single packet and a "done for now"
            // signal.
