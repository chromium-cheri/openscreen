...
LIBAOM_ENVVAR = 'OPENSCREEN_HAVE_AOM'
...
<OPENSCREEN_ROOT_DIR>/out/Default will be used. In addition, one can choose to
run AV1-specific tests by setting the OPENSCREEN_HAVE_AOM environment variable
to "true" (case is ignored).
...
class StandaloneCastTest(unittest.TestCase):
    ...
    @unittest.skipUnless(os.getenv(LIBAOM_ENVVAR) and
                        os.environ[LIBAOM_ENVVAR].lower() == 'true',
