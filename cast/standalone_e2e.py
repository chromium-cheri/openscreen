...
OPENSCREEN_ROOT_FOLDER = pathlib.Path(__file__).parent.parent.resolve()
...
def _launch_receiver():
    ...
    return subprocess.Popen(
        [CAST_RECEIVER_BINARY,
        '-d', './generated_root_cast_receiver.crt',
        '-p', './generated_root_cast_receiver.key',
...
def _check_logs(logs):
    ...
    logging.debug(logs)
    logging.info('Finished running binaries')
    # TODO: add log checking for major events.
...
def test():
    ...
    output = (receiver_process.communicate(timeout=PROCESS_TIMEOUT),
                        sender_process.communicate(timeout=PROCESS_TIMEOUT))
