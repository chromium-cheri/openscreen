...
// clang-format off
std::vector<std::string> kRemoteDisplayAppStreamingAudioVideoAppIds {
  "35708D08",
  "3185A70D",
  "B7AD3A0A",
  "A4804AE7",
...
bool IsCastStreamingReceiverAppId(const std::string& app_id) {
  ...
  const auto& app_ids = GetRemoteDisplayAppStreamingAudioVideoAppIds();
  return std::find(app_ids.begin(), app_ids.end(), app_id) != app_ids.end();
}
...
const std::vector<std::string>& GetRemoteDisplayAppStreamingAudioVideoAppIds() {
  return kRemoteDisplayAppStreamingAudioVideoAppIds;
}
