...
bool ServiceInfo::IsValid() const {
  return (
      ...
      discovery::DnsSdTxtRecord::IsValidTxtValue(
          kCapabilitiesKey,
          std::to_string(static_cast<uint64_t>(capabilities))) &&
...
discovery::DnsSdInstance ServiceInfoToDnsSdInstance(const ServiceInfo& info) {
  ...
  const bool did_set_everything =
      ...
      txt.SetValue(kVersionKey,
          ...
          .ok() &&
      txt.SetValue(kCapabilitiesKey,
                   std::to_string(static_cast<uint64_t>(info.capabilities)))
          .ok() &&
...
ErrorOr<ServiceInfo> DnsSdInstanceEndpointToServiceInfo(
  ...
  int16_t version;
  int chars_scanned;
  if (sscanf(a_decimal_number.c_str(), "%" SCNd16 "%n", &version,
             &chars_scanned) != 1) {
    return {Error::Code::kParameterInvalid,
            "Missing Cast protocol version in record."};
  }
  if (chars_scanned > 2 || version < 2 ||
      version > std::numeric_limits<uint8_t>::max()) {
    return {Error::Code::kParameterInvalid,
  ...
  if (sscanf(a_decimal_number.c_str(), "%" SCNu64 "%n", &capabilities_bitset,
             &chars_scanned) != 1) {
    return {Error::Code::kParameterInvalid,
            "Missing device capabilities bitfield in record."};
  }
  if (chars_scanned > 19) {
    return {Error::Code::kParameterInvalid,
            "Invalid device capabilities bitfield in record."};
  }
  record.capabilities = static_cast<ReceiverCapabilities>(capabilities_bitset);
