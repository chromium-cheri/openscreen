...
const size_t kMaxReceiverNameSize = 63;
...
std::string CalculateInstanceId(const ReceiverInfo& info) {
  ...
  // Append the virtual receiver ID to the instance name separated by a single
  ...
  return std::string(instance_name, 0, kMaxReceiverNameSize);
}
