...
ErrorOr<ReceiverInfo> DnsSdInstanceEndpointToReceiverInfo(
  ...
  auto id_value = endpoint.txt().GetStringValue(kUniqueIdKey);
  if (!id_value) {
    return {Error::Code::kParameterInvalid,
            "Missing receiver unique ID in record."};
  }
  record.unique_id = id_value.value();
  ...
  auto name_value = endpoint.txt().GetStringValue(kFriendlyNameKey);
  if (!name_value) {
    return {Error::Code::kParameterInvalid,
            "Missing receiver friendly name in record."};
  }
  record.friendly_name = name_value.value();
