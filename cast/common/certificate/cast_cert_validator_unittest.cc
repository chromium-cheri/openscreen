...
void RunTest(Error::Code expected_result,
  ...
  std::vector<std::string> string_certs;
  for (const auto& cert : certs) {
    string_certs.emplace_back(reinterpret_cast<const char*>(cert.data()),
                              cert.size());
  }

  Error result =
      VerifyDeviceCert(string_certs, time, &context, &policy, nullptr,
