...
bssl::UniquePtr<BASIC_CONSTRAINTS> GetBasicConstraints(X509* cert) {
  ...
  X509_EXTENSION* basic_constraints_extension =
      X509_get_ext(cert, basic_constraints_index);
...
CastCertError VerifyCertificateChain(const std::vector<CertPathStep>& path,
  ...
  if (basic_constraints->pathlen) {
    if (basic_constraints->pathlen->length > 1) {
      ...
      if (pathlen < 0 || basic_constraints->pathlen->length != 1) {
        return CastCertError::kErrCertsVerifyGeneric;
      }
    }
  }
  ...
  for (; i < path.size() - 1; ++i) {
    ...
    if (!is_root) {
      ...
      if (X509_NAME_cmp(X509_get_subject_name(issuer),
        ...
        ++observed_pathlen;
        if (pathlen != -1 && observed_pathlen > pathlen) {
    ...
    // NOTE: (!self-issued || target) -> verify name constraint.  Target case is
    // after the loop.
    if (!(issuer->ex_flags & EXFLAG_SI)) {
    ...
    if (policy_mappings_index != -1) {
      ...
      POLICY_MAPPINGS* policy_mappings = reinterpret_cast<POLICY_MAPPINGS*>(
          X509V3_EXT_d2i(policy_mappings_extension));
      uint32_t policy_mapping_count = sk_POLICY_MAPPING_num(policy_mappings);
    ...
    for (int i = 0; i < extension_count; ++i) {
      ...
      if (extension->critical > 0) {
        int nid = OBJ_obj2nid(extension->object);
