...
CastCertError VerifyDeviceCertUsingCustomTrustStore(
  ...
  for (size_t i = 0; i < certs.size(); ++i) {
    const uint8_t* data = (const uint8_t*)certs[i].data();
      ...
      for (; i < path.size() - 1 && error == CastCertError::OK; ++i) {
        ...
        if (!VerifySignedData(
                ...
                {subject->cert_info->enc.enc,
                 (uint32_t)subject->cert_info->enc.len},
                {X509_CINF_get_signature(subject)->data,
                 (uint32_t)X509_CINF_get_signature(subject)->length})) {
          break;
        }
      }
    ...
    if (pos != -1) {
      ...
      if (policies) {
        ...
        for (uint32_t i = 0; i < policy_count; ++i) {
          ...
          if (info->policyid->length == (int)audio_only_policy_oid.length &&
