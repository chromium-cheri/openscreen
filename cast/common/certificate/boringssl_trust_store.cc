...
std::string GetCertSubjectString(X509* cert) {
  X509_NAME* subject_name = X509_get_subject_name(cert);
  return subject_name ? std::string(X509_NAME_oneline(subject_name, 0, 0)) : "";
}
        ...
        if (max_pathlen == 0) {
          return Error(
              ...
              absl::StrCat("Invalid max_pathlen (was ", max_pathlen,
                           ") for subject: ", GetCertSubjectString(issuer)));
        }
        --max_pathlen;
      } else {
      ...
      default:
        return Error(
            ...
            absl::StrCat("Unknown digest algorithm ", nid,
                         ", subject: ", GetCertSubjectString(subject)));
    }
    uint8_t* tbs = nullptr;
    ...
    if (!VerifySignedData(
      ...
      return Error(
          ...
          absl::StrCat("Failure to verify signature over cert with subject: ",
                       GetCertSubjectString(subject), ", public key from cert ",
                       "with subject: ", GetCertSubjectString(issuer)));
    }
  }
  ...
  if (valid_time != Error::Code::kNone) {
    return Error(
        ...
        absl::StrCat("FindCertificatePath: Failed to verify certificate time, "
                     "subject:",
                     GetCertSubjectString(target_cert.get())));
  }
