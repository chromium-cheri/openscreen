...
ErrorOr<DateTime> GetNotBeforeTime(X509* cert) {
  ...
  if (!not_before_asn1) {
    return Error(Error::Code::kParseError, "Failed to retrieve cert notBefore");
  }
  DateTime result;
  if (!ParseAsn1GeneralizedTime(not_before_asn1.get(), &result)) {
    return Error(Error::Code::kParseError, "Failed to parse cert notBefore");
  }
  return result;
}
...
ErrorOr<DateTime> GetNotAfterTime(X509* cert) {
  ...
  if (!ParseAsn1GeneralizedTime(not_after_asn1.get(), &result)) {
    return Error(Error::Code::kParseError, "Failed to parse cert notAfter");
  }
  return result;
}
