...
Error FindCertificatePath(const std::vector<std::string>& der_certs,
  ...
  if (!target_cert) {
    OSP_VLOG << "FindCertificatePath: Invalid target certificate";
  ...
  if (error != Error::Code::kNone) {
    OSP_VLOG << "FindCertificatePath: Failed to verify certificate time";
    return error;
  }
  bssl::UniquePtr<EVP_PKEY> public_key{X509_get_pubkey(target_cert.get())};
  if (!VerifyPublicKeyLength(public_key.get())) {
    OSP_VLOG << "FindCertificatePath: Failed with invalid public key length";
    return Error::Code::kErrCertsVerifyGeneric;
  }
  ...
  if (!key_usage) {
    OSP_VLOG << "FindCertificatePath: Failed with no key usage";
    return Error::Code::kErrCertsRestrictions;
  }
  int bit =
      ASN1_BIT_STRING_get_bit(key_usage.get(), KeyUsageBits::kDigitalSignature);
  if (bit == 0) {
    OSP_VLOG << "FindCertificatePath: Failed to get digital signature";
    return Error::Code::kErrCertsRestrictions;
  }
    ...
    if (!next_issuer) {
      if (path_index == first_index) {
        ...
        if (last_error == Error::Code::kNone) {
          OSP_VLOG << "FindCertificatePath: Failed after trying all "
