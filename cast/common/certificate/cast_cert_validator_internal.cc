...
bool ConvertTimeSeconds(uint64_t seconds, DateTime* time) {
  ...
  static constexpr uint64_t kDaysPerQuadYear = 365 * 4 + 1;
  static constexpr uint64_t kDaysPerQuadCentury =
      (24 * 4 + 1) * kDaysPerQuadYear + 3 * 4 * 365;

  static constexpr uint64_t kSecondsPerFirstTwoYears = 365 * 2 * kSecondsPerDay;
  static constexpr uint64_t kSecondsPerNextTwentyEightYears =
      7 * kDaysPerQuadYear * kSecondsPerDay;

  static constexpr uint64_t kDaysPerMonth[] = {
      31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31,
  };

  uint64_t first_two_years = !!(seconds / (kSecondsPerFirstTwoYears));
  seconds -= first_two_years * kSecondsPerFirstTwoYears;

  uint64_t twenty_eight_years = !!(seconds / kSecondsPerNextTwentyEightYears);
  seconds -= twenty_eight_years * kSecondsPerNextTwentyEightYears;

  uint64_t years = first_two_years * 2 + twenty_eight_years * 28;
  if (twenty_eight_years) {
    uint64_t quad_centuries = seconds / (kDaysPerQuadCentury * kSecondsPerDay);
    seconds -= quad_centuries * kDaysPerQuadCentury * kSecondsPerDay;
    years += quad_centuries * 400;

    uint64_t first_century = seconds / (25 * kDaysPerQuadYear * kSecondsPerDay);
    seconds -= first_century * 25 * kDaysPerQuadYear * kSecondsPerDay;
    years += first_century * 100;
    if (first_century) {
      uint64_t next_centuries =
          seconds / ((24 * kDaysPerQuadYear + 365 * 4) * kSecondsPerDay);
      seconds -=
          next_centuries * (24 * kDaysPerQuadYear + 365 * 4) * kSecondsPerDay;
      years += next_centuries * 100;

      uint64_t first_quad_year = seconds / (365 * 4 * kSecondsPerDay);
      seconds -= first_quad_year * 365 * 4 * kSecondsPerDay;
      years += first_quad_year * 4;
    }
  }

  uint64_t quad_years = seconds / (kDaysPerQuadYear * kSecondsPerDay);
  seconds -= quad_years * kDaysPerQuadYear * kSecondsPerDay;
  years += quad_years * 4;

  uint64_t leap_year = seconds / (366 * kSecondsPerDay);
  seconds -= leap_year * 366 * kSecondsPerDay;
  years += leap_year;

  uint64_t next_years = seconds / (365 * kSecondsPerDay);
  seconds -= next_years * 365 * kSecondsPerDay;
  years += next_years;
  ...
  uint64_t months = 0;
  for (int i = 0; i < 12; ++i) {
    uint64_t days_per_month = kDaysPerMonth[i] + (i == 1 && is_leap_year);
    uint64_t month = !!(seconds / (days_per_month * kSecondsPerDay));
    seconds -= month * days_per_month * kSecondsPerDay;
    months += month;
  }

  uint64_t days = seconds / kSecondsPerDay;
  seconds -= days * kSecondsPerDay;
  uint64_t hours = seconds / (kMinutesPerHour * kSecondsPerMinute);
  seconds -= hours * kMinutesPerHour * kSecondsPerMinute;
  uint64_t minutes = seconds / kSecondsPerMinute;
