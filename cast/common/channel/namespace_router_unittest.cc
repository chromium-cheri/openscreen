...
TEST_F(NamespaceRouterTest, NoHandlersNoop) {
  ...
  router_.OnMessage(socket(), std::move(message));
}
...
TEST_F(NamespaceRouterTest, MultipleHandlers) {
  ...
  EXPECT_CALL(media_handler, OnMessage(_, _)).Times(0);
  EXPECT_CALL(auth_handler, OnMessage(_, _))
      .WillOnce(Invoke([](CastSocket*, CastMessage&& message) {
        EXPECT_EQ(message.namespace_(), "auth");
      }));
  EXPECT_CALL(connection_handler, OnMessage(_, _))
      .WillOnce(Invoke([](CastSocket*, CastMessage&& message) {
  ...
  router_.OnMessage(socket(), std::move(auth_message));

  CastMessage connection_message;
  connection_message.set_namespace_("connection");
  router_.OnMessage(socket(), std::move(connection_message));
}
...
TEST_F(NamespaceRouterTest, RemoveHandler) {
  ...
  EXPECT_CALL(handler1, OnMessage(_, _)).Times(0);
  EXPECT_CALL(handler2, OnMessage(_, _))
  ...
  router_.OnMessage(socket(), std::move(message1));

  CastMessage message2;
  message2.set_namespace_("two");
  router_.OnMessage(socket(), std::move(message2));
}
