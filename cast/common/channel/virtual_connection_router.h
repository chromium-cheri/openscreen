...
// an appropriate VirtualConnection.
//
...
//    (e.g.  sink "resolved", begin app probing, or something) and that calls
//    VCRouter::TakeSocket.
//
...
class VirtualConnectionRouter final : public CastSocket::Client {
 public:
  ...
  void AddHandlerForLocalId(const std::string& local_id,
                            CastMessageHandler* endpoint);
  void RemoveHandlerForLocalId(const std::string& local_id);
  ...
  Error SendMessage(const VirtualConnection& vconn, CastMessage&& message);
