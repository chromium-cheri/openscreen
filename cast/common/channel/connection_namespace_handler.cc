...
int GetConnectionType(int conn_type) {
...
void ConnectionNamespaceHandler::HandleConnect(VirtualConnectionRouter* router,
  ...
  VirtualConnection vconn{std::move(message.destination_id()),
                          std::move(message.source_id()), socket->socket_id()};
  if (!vc_policy_->IsConnectionAllowed(vconn)) {
    SendClose(router, vconn);
    return;
  }
  ...
  if (maybe_conn_type) {
    conn_type = maybe_conn_type.value();
    conn_type = GetConnectionType(conn_type);
  }
  ...
  if (negotiated_version) {
    SendConnectedResponse(router, vconn, negotiated_version.value());
  }
...
void ConnectionNamespaceHandler::HandleClose(VirtualConnectionRouter* router,
  ...
  VirtualConnection vconn{std::move(message.destination_id()),
                          std::move(message.source_id()), socket->socket_id()};
  if (!vc_manager_->GetConnectionData(vconn)) {
    return;
  }
...
void ConnectionNamespaceHandler::SendClose(VirtualConnectionRouter* router,
  ...
  CastMessage message =
      MakeSimpleUTF8Message(kConnectionNamespace, std::move(payload));
  router->SendMessage(vconn, std::move(message));
}
