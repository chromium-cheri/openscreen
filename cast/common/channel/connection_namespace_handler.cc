...
void ConnectionNamespaceHandler::OnMessage(VirtualConnectionRouter* router,
  ...
  ErrorOr<Json::Value> result = openscreen::json::Read(message.payload_utf8());
  if (result.is_error()) {
    return;
  }
...
void ConnectionNamespaceHandler::SendClose(VirtualConnectionRouter* router,
  ...
  ErrorOr<std::string> result = openscreen::json::Write(close_message);
  if (result.is_error()) {
    return;
  }
...
void ConnectionNamespaceHandler::SendConnectedResponse(
  ...
  ErrorOr<std::string> result = openscreen::json::Write(connected_message);
  if (result.is_error()) {
    return;
  }
