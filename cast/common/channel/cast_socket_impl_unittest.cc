...
class MockCastSocketDelegate final : public CastSocket::Delegate {
 public:
  ~MockCastSocketDelegate() override = default;

  MOCK_METHOD(void, OnError, (CastSocket * socket, const Error& error));
...
class CastSocketImplTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    ErrorOr<std::string> serialized_or_error =
        MessageFramer::Serialize(message_);
    ASSERT_TRUE(serialized_or_error);
    frame_str_ = std::move(serialized_or_error.value());
  }
 ...
 protected:
  ...
  CastSocketImpl socket_{std::move(moved_connection_), &mock_delegate_, 1};
...
TEST_F(CastSocketImplTest, SendMessage) {
  EXPECT_CALL(*connection_, Write(_, _))
      .WillOnce(Invoke([this](const void* data, size_t len) {
        EXPECT_EQ(frame_str_,
                  std::string(reinterpret_cast<const char*>(data), len));
      }));
  ASSERT_TRUE(socket_.SendMessage(message_).ok());
}

TEST_F(CastSocketImplTest, ReadCompleteMessage) {
  const uint8_t* data = reinterpret_cast<const uint8_t*>(frame_str_.data());
  EXPECT_CALL(mock_delegate_, OnMessage(_, _))
      .WillOnce(Invoke([this](CastSocket* socket, CastMessage* message) {
        EXPECT_EQ(message_.SerializeAsString(), message->SerializeAsString());
      }));
  connection_->OnRead(std::vector<uint8_t>(data, data + frame_str_.size()));
  task_runner_.RunTasksUntilIdle();
}

TEST_F(CastSocketImplTest, ReadChunkedMessage) {
  const uint8_t* data = reinterpret_cast<const uint8_t*>(frame_str_.data());
  EXPECT_CALL(mock_delegate_, OnMessage(_, _)).Times(0);
  ...
  connection_->OnRead(
      std::vector<uint8_t>(data + 10, data + frame_str_.size()));
  task_runner_.RunTasksUntilIdle();

  std::string double_message = frame_str_ + frame_str_;
  data = reinterpret_cast<const uint8_t*>(double_message.data());
  EXPECT_CALL(mock_delegate_, OnMessage(_, _))
      .WillOnce(Invoke([this](CastSocket* socket, CastMessage* message) {
        EXPECT_EQ(message_.SerializeAsString(), message->SerializeAsString());
      }));
  connection_->OnRead(
      std::vector<uint8_t>(data, data + frame_str_.size() + 10));
  task_runner_.RunTasksUntilIdle();
  ...
  connection_->OnRead(std::vector<uint8_t>(data + frame_str_.size() + 10,
                                           data + double_message.size()));
  task_runner_.RunTasksUntilIdle();
}

TEST_F(CastSocketImplTest, SendMessageWhileBlocked) {
