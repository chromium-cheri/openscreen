...
TEST(MdnsRecordTest, ReadUnknownRecordType) {
  ...
  EXPECT_EQ(record.record_class(), kClassIN | kCacheFlushBit);
  EXPECT_EQ(record.ttl(), UINT32_C(120));
  RawRecordRdata raw_rdata(kCnameRdata, kCnameRdata + sizeof(kCnameRdata));
...
TEST(MdnsQuestionTest, CopyAndAssign) {
  ...
  name.PushLabel("testing");
  name.PushLabel("local");
  MdnsQuestion question(std::move(name), kTypePTR, kClassIN | kUnicastResponseBit);
...
TEST(MdnsQuestionTest, Write) {
  ...
  DomainName name;
  name.PushLabel("wire");
  name.PushLabel("format");
  name.PushLabel("local");
  MdnsQuestion question(std::move(name), kTypePTR,
