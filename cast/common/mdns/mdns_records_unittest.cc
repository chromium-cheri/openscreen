...
TEST(MdnsRawRecordRdataTest, Construct) {
  ...
  EXPECT_EQ(rdata3.max_wire_size(), UINT64_C(10));
...
TEST(MdnsRawRecordRdataTest, Compare) {
  const uint8_t kRawRdata1[] = {
      0x05, 'c', 'n', 'a', 'm', 'e', 0xc0, 0x00,
  };
  const uint8_t kRawRdata2[] = {
      0x05, 'r', 'd', 'a', 't', 'a',
  };
