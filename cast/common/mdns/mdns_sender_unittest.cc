...
TEST_F(MdnsSenderTest, SendMulticastIPv4) {
  std::unique_ptr<openscreen::platform::MockUdpSocketDefaults> socket_info =
      MockUdpSocket::CreateDefault(openscreen::IPAddress::Version::kV4);
  MdnsSender sender(&socket_info->socket_);
  EXPECT_CALL(socket_info->socket_,
...
TEST_F(MdnsSenderTest, MessageTooBig) {
  ...
  std::unique_ptr<openscreen::platform::MockUdpSocketDefaults> socket_info =
      MockUdpSocket::CreateDefault(openscreen::IPAddress::Version::kV4);
  MdnsSender sender(&socket_info->socket_);
  EXPECT_CALL(socket_info->socket_, SendMessage(_, _, _)).Times(0);
...
TEST_F(MdnsSenderTest, ReturnsErrorOnSocketFailure) {
  std::unique_ptr<openscreen::platform::MockUdpSocketDefaults> socket_info =
      MockUdpSocket::CreateDefault(openscreen::IPAddress::Version::kV4);
  MdnsSender sender(&socket_info->socket_);
  EXPECT_CALL(socket_info->socket_, SendMessage(_, _, _))
