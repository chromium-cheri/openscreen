...
class MdnsRecordTracker {
 public:
  ...
  // error if called on an instance of MdnsRecordTracker that has already been
  // started.
  Error Start(MdnsRecord record);

  // Stops sending query for automatic record refresh. This cancels record
  // expiration notification as well. Returns error if called on an instance of
...
class MdnsQuestionTracker {
 public:
  ...
  // error if called on an instance of MdnsQuestionTracker that has not yet been
