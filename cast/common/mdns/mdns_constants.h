...
//
// Parsed representation: "239.255.255.250" (IPv4 site-local)
extern const uint8_t kDefaultSiteLocalGroupIPv4[4];
...
//
// Parsed representation: "FF05::C" (IPv6 site-local)
extern const uint8_t kDefaultSiteLocalGroupIPv6[16];
...
const uint16_t kDefaultMulticastPort = 5353;
...
// TODO(maclellant): Figure out the exact size that the senders are using and
// sync with them. We want to verify that we are using the same maximum
// packet size.
// TODO(maclellant): The spec also suggests keeping all UDP messsages below
// 512 bytes, since that is where some fragmentation may occur. If possible
// we should measure the rate of fragmented messages and see if lowering the
// max size alleviates it.
const size_t kMaxMulticastMessageSize = 1500 - 20 - 8;
...
// enabled.
const bool kDefaultSupportSiteLocalGroup = true;
...
// Copied from: net::dns_protocol::Header.
struct Header {
  uint16_t id;
  uint16_t flags;
  uint16_t qdcount;
...
const uint16_t kFlagResponse = 0x8000;
const uint16_t kFlagAA = 0x0400;
const uint16_t kFlagTC = 0x0200;
const uint16_t kFlagRD = 0x0100;
const uint16_t kFlagRA = 0x0080;
const uint16_t kFlagZ = 0x0040;  // Unused field
const uint16_t kFlagAD = 0x0020;
const uint16_t kFlagCD = 0x0010;
...
const uint16_t kOpcodeMask = 0x7800;
const uint8_t kOpcodeQUERY = 0;
const uint8_t kOpcodeIQUERY = 1;
const uint8_t kOpcodeSTATUS = 2;
const uint8_t kOpcodeUNASSIGNED = 3;  // Unused for now
const uint8_t kOpcodeNOTIFY = 4;
const uint8_t kOpcodeUPDATE = 5;
...
// shifting into correct position.
const uint16_t kRcodeMask = 0x000F;
const uint8_t kRcodeNOERROR = 0;
const uint8_t kRcodeFORMERR = 1;
const uint8_t kRcodeSERVFAIL = 2;
const uint8_t kRcodeNXDOMAIN = 3;
const uint8_t kRcodeNOTIMP = 4;
const uint8_t kRcodeREFUSED = 5;
...
// Maximum number of octets allowed in a single domain name. This
const size_t kMaxDomainNameLength = 255;
// Maximum number of octets allowed in a single domain name label.
const size_t kMaxLabelLength = 63;
...
const uint8_t kLabelMask = 0xC0;
const uint8_t kLabelPointer = 0xC0;
const uint8_t kLabelDirect = 0x00;
const uint8_t kLabelTermination = 0x00;
const uint16_t kLabelOffsetMask = 0x3FFF;
...
const uint16_t kTypeA = 1;
const uint16_t kTypeCNAME = 5;
const uint16_t kTypePTR = 12;
const uint16_t kTypeTXT = 16;
const uint16_t kTypeAAAA = 28;
const uint16_t kTypeSRV = 33;
const uint16_t kTypeNSEC = 47;
const uint16_t kTypeANY = 255;  // Only allowed for QTYPE

// DNS CLASS masks and values.
const uint16_t kClassMask = 0x7FFF;
const uint16_t kClassIN = 1;
const uint16_t kClassANY = 255;  // Only allowed for QCLASS
...
const uint16_t kCacheFlushBit = 0x8000;
...
const uint16_t kUnicastResponseBit = 0x8000;
...
const uint32_t kDefaultRecordTTL = 255;
...
const size_t kTXTMaxEntrySize = 255;
...
const uint8_t kTXTEmptyRdata = 0;
