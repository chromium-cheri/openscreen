...
void ARecordRdata::set_ipv4_address(IPAddress address) {
  OSP_CHECK(address.IsV4());
  ipv4_address_ = std::move(address);
}
...
void AAAARecordRdata::set_ipv6_address(IPAddress address) {
  OSP_CHECK(address.IsV6());
  ipv6_address_ = std::move(address);
}
