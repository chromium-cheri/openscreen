...
TEST(MdnsReceiverTest, ReceiveQuery) {
  ...
  MdnsReceiver receiver(&socket_info->socket_, &runner, &delegate);

  EXPECT_CALL(runner, ReadRepeatedly(&socket_info->socket_, _))
  ...
  EXPECT_CALL(runner, CancelRead(&socket_info->socket_))
      .WillOnce(Return(Error::Code::kNone));
  receiver.Stop();
}
