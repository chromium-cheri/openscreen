...
MdnsRecord::MdnsRecord(DomainName name,
    ...
    : name_(std::move(name)),
      ...
      ttl_(ttl),
      rdata_(std::move(rdata)) {
  OSP_CHECK(!name.empty());
  OSP_CHECK(
      (type == kTypeSRV && absl::holds_alternative<SrvRecordRdata>(rdata)) ||
      (type == kTypeA && absl::holds_alternative<ARecordRdata>(rdata)) ||
      (type == kTypeAAAA && absl::holds_alternative<AAAARecordRdata>(rdata)) ||
      (type == kTypePTR && absl::holds_alternative<PtrRecordRdata>(rdata)) ||
      (type == kTypeTXT && absl::holds_alternative<TxtRecordRdata>(rdata)) ||
      absl::holds_alternative<RawRecordRdata>(rdata));
}

// TODO(yakimakha): should comparison include ttl and top bit of record class
// which is cache invalidation bit?
