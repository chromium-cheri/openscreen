...
Error MdnsRecordTracker::Update(MdnsRecord new_record) {
  if (!record_.has_value()) {
    return Error::Code::kOperationInvalid;
  }
  ...
  if (IsGoodbyeRecord(new_record)) {
    ...
    // TODO(yakimakha): What to do if RDATA is also different?
    // TODO: will move constructors be called here or not?
    new_record = MdnsRecord(std::move(new_record.name()), new_record.dns_type(),
                            new_record.dns_class(), new_record.record_type(),
                            kGoodbyeRecordTtl, std::move(new_record.rdata()));
  }

  error = Start(std::move(new_record));
  if (!error.ok()) {
    return error;
  }
