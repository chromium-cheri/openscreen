...
bool MdnsWriter::Write(const TxtRecordRdata& rdata) {
  ...
  for (const std::string& entry : rdata.texts()) {
    if (!entry.empty() && !Write(absl::string_view(entry))) {
      return false;
    }
  }
  ...
  if (cursor.delta() == sizeof(uint16_t) && !Write(kTXTEmptyRdata)) {
    return false;
  }
...
bool MdnsWriter::Write(const Rdata& rdata) {
  return absl::visit([this](const auto& rdata) { return Write(rdata); }, rdata);
}
