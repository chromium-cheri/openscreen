...
// the size of the |Integer| into the |Integer| length field at |begin|. The
// method returns true if the number of bytes between |begin| and |end| fits in
// |Integer| type, returns false otherwise.
template <typename Integer>
bool UpdateRecordLength(const uint8_t* end, uint8_t* begin) {
  OSP_DCHECK_LE(begin + sizeof(Integer), end);
  ptrdiff_t record_length = end - begin - sizeof(Integer);
  if (record_length <= std::numeric_limits<Integer>::max()) {
    openscreen::WriteBigEndian<Integer>(static_cast<Integer>(record_length),
                                        begin);
    return true;
  }
  return false;
...
bool MdnsWriter::WriteCharacterString(const std::string& value) {
...
bool MdnsWriter::WriteSrvRecordRdata(const SrvRecordRdata& rdata) {
  ...
  if (Skip(sizeof(uint16_t)) && Write<uint16_t>(rdata.priority()) &&
      ...
      UpdateRecordLength<uint16_t>(current(), rollback_position)) {
    return true;
  }
  set_current(rollback_position);
  return false;
...
bool MdnsWriter::WriteTxtRecordRdata(const TxtRecordRdata& rdata) {
  ...
  if (!texts.empty()) {
    auto write_string_predicate = [this](const std::string& entry) -> bool {
      OSP_DCHECK(entry.length() <= kTXTMaxEntrySize);
      return WriteCharacterString(entry);
    };
    // Leave space at the beginning at |rollback_position| to write the record
    // length. It's cheaper to update it at the end than precompute the lenght.
    if (Skip(sizeof(uint16_t)) &&
        std::all_of(texts.begin(), texts.end(), write_string_predicate) &&
        UpdateRecordLength<uint16_t>(current(), rollback_position)) {
      return true;
    }
  } else {
