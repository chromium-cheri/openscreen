// Copyright 2019 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef CAST_COMMON_MDNS_MDNS_RANDOM_H_
#define CAST_COMMON_MDNS_MDNS_RANDOM_H_

#include <random>

#include "platform/api/time.h"

namespace cast {
namespace mdns {

using Clock = openscreen::platform::Clock;

class MdnsRandom {
 public:
  Clock::duration GetInitialQueryDelay() {
    return std::chrono::milliseconds{initial_query_delay_(random_engine_)};
  }

  double GetRecordTtlVariation() {
    return record_ttl_variation_(random_engine_);
  }

  Clock::duration GetSharedRecordResponseDelay() {
    return std::chrono::milliseconds{
        shared_record_response_delay_(random_engine_)};
  }

  Clock::duration GetTruncatedQueryResponseDelay() {
    return std::chrono::milliseconds{
        truncated_query_response_delay_(random_engine_)};
  }

 private:
  // RFC 6762 Section 5.2
  // https://tools.ietf.org/html/rfc6762#section-5.2

  static constexpr int64_t kMinimumInitialQueryDelay = 20;
  static constexpr int64_t kMaximumInitialQueryDelay = 120;

  static constexpr double kMinimumTtlVariation = 0.0;
  static constexpr double kMaximumTtlVariation = 2.0;

  // RFC 6762 Section 6
  // https://tools.ietf.org/html/rfc6762#section-6

  static constexpr int64_t kMinimumSharedRecordResponseDelay = 20;
  static constexpr int64_t kMaximumSharedRecordResponseDelay = 120;

  static constexpr int64_t kMinimumTruncatedQueryResponseDelay = 400;
  static constexpr int64_t kMaximumTruncatedQueryResponseDelay = 500;

  std::default_random_engine random_engine_{std::random_device{}()};
  std::uniform_int_distribution<int64_t> initial_query_delay_{
      kMinimumInitialQueryDelay, kMaximumInitialQueryDelay};
  std::uniform_real_distribution<double> record_ttl_variation_{
      kMinimumTtlVariation, kMaximumTtlVariation};
  std::uniform_int_distribution<int64_t> shared_record_response_delay_{
      kMinimumSharedRecordResponseDelay, kMaximumSharedRecordResponseDelay};
  std::uniform_int_distribution<int64_t> truncated_query_response_delay_{
      kMinimumTruncatedQueryResponseDelay, kMaximumTruncatedQueryResponseDelay};
};

}  // namespace mdns
}  // namespace cast

#endif  // CAST_COMMON_MDNS_MDNS_RANDOM_H_