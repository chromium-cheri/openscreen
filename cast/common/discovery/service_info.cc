...
// not be important.
std::string GetInstanceId(const ServiceInfo& info) {
...
bool IsValid(const ServiceInfo& service) {
  std::string instance_id = GetInstanceId(service);
  ...
  if (!discovery::DnsSdTxtRecord::IsValidTxtValue(kUniqueIdKey,
                                                  service.unique_id) ||
      ...
      !discovery::DnsSdTxtRecord::IsValidTxtValue(kCapabilitiesId,
                                                  capabilities_str) ||
      !discovery::DnsSdTxtRecord::IsValidTxtValue(kStatusId, service.status) ||
      !discovery::DnsSdTxtRecord::IsValidTxtValue(kFriendlyNameId,
                                                  service.friendly_name) ||
      !discovery::DnsSdTxtRecord::IsValidTxtValue(kModelNameId,
                                                  service.model_name)) {
    return false;
  }

  return service.v4_address || service.v6_address;
}
