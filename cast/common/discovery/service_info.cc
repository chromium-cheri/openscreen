...
// Integer offset in ASCII, used when converting between characters and the
// integers they represent.
constexpr uint8_t kAsciiIntegerOffset = '0';
...
std::string EncodeIntegerString(uint64_t value) {
  ...
  data.reserve(20);  // Max size of a uint64 in base 10 is less than 10^21.
  while (value > 0) {
    data.push_back(value % 10 + kAsciiIntegerOffset);
    value = value / 10;
  }
...
ErrorOr<uint64_t> DecodeIntegerString(const std::string& value) {
  ...
  for (; it != value.end(); it++) {
    ...
    const uint8_t index_value = *it - kAsciiIntegerOffset;
    current = current * 10 + index_value;
    if (current <= last) {  // check for overflow.
...
ErrorOr<discovery::DnsSdInstanceRecord> Convert(const ServiceInfo& service) {
  ...
  const uint64_t capabilities = static_cast<uint64_t>(service.capabilities);
