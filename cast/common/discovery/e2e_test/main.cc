...
class DiscoveryE2ETest : public testing::Test {
 ...
 protected:
  ...
  void PublishRecords(RecordTypes... records) {
    ...
    for (const ServiceInfo& record : record_set) {
...
// 6) Soft reset the service, and ensure that no callback is received.
// 7) Stop publication of the service, and ensure that a callback is received.
// 8) Restart publication of the service and ensure a callback is received.
...
// 2) Start service discovery and new queries.
// 3) Publish one service and ensure it is received.
// 4) Update the published record and ensure that a new callback is called for
//    the updated service instance.
// 5) Stop service discovery.
// 6) Update a service, and ensure that no callback is received.
// 7) Restart the query and ensure that no outdated service is provided via
//    callback.
TEST_F(DiscoveryE2ETest, ValidateDiscoveryFlow) {}
