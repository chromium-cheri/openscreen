 ...
 public:
  TestCredentialsProvider() {
    ...
    InitStaticCredentialsFromFiles(
        &creds_, &parsed_cert, &fake_trust_store, data_path + "device_key.pem",
        data_path + "device_chain.pem", data_path + "device_tls.pem");
  }
 ...
 public:
  TestAuthChallengeMessage() {
    const auto result = ParseFromString(ReadEntireFileToString(
        GetTestDataPath() + "cast/receiver/channel/auth_challenge.pb"));
    OSP_CHECK(result);
  }
};
...
class FakeApplication : public ApplicationAgent::Application,
 ...
 public:
  explicit FakeApplication(const char* app_id, const char* display_name)
      : app_id_(app_id), display_name_(display_name) {
    OSP_CHECK(app_id_.size() == 8);
  }
  ...
  std::vector<std::string> GetAppIds() override { return {app_id_}; }
