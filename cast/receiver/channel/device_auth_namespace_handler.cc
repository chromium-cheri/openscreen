...
void DeviceAuthNamespaceHandler::OnMessage(VirtualConnectionRouter* router,
  ...
  SignatureAlgorithm sig_alg = challenge.signature_algorithm();
  ...
  absl::Span<uint8_t> tls_cert_der = creds_provider_->GetCurrentTlsCertAsDer();
  DeviceCredentials* device_creds =
      creds_provider_->GetCurrentDeviceCredentials();
  if (tls_cert_der.empty() || !device_creds || device_creds->certs.empty() ||
  ...
  for (auto it = device_creds->certs.begin() + 1;
       it != device_creds->certs.end(); ++it) {
  ...
  if (!EVP_DigestSignInit(ctx.get(), nullptr, digest, nullptr,
                          device_creds->private_key.get())) {
    router->SendMessage(virtual_conn,
                        GenerateErrorMessage(AuthError::INTERNAL_ERROR));
    return;
  }
  size_t signature_length = 0;
  if (!EVP_DigestSign(ctx.get(), 0, &signature_length,
                      reinterpret_cast<const uint8_t*>(to_be_signed.data()),
                      to_be_signed.size())) {
    router->SendMessage(virtual_conn,
                        GenerateErrorMessage(AuthError::INTERNAL_ERROR));
    return;
  ...
  auth_response->set_signature(std::move(signature));
