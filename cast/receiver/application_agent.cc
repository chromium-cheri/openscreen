...
Json::Value ApplicationAgent::HandleLaunch(const Json::Value& request,
  ...
  if (app_id.isString() && !app_id.asString().empty()) {
    error =
        SwitchToApplication(app_id.asString(), request["appParams"], socket);
  } else {
    error = Error(Error::Code::kParameterInvalid, "BAD_PARAMETER");
  }
  if (!error.ok()) {
    Json::Value response;
    response[kMessageKeyRequestId] = request[kMessageKeyRequestId];
...
Error ApplicationAgent::SwitchToApplication(std::string app_id,
  ...
  if (!app_id.empty()) {
    ...
    if (it != registered_applications_.end()) {
      ...
      fallback_app =
          (desired_app != idle_screen_app_) ? idle_screen_app_ : nullptr;
    } else {
      return Error(Error::Code::kItemNotFound, "NOT_FOUND");
    }
  }
  ...
  if (desired_app) {
    ...
    if (desired_app->Launch(app_id, app_params, &message_port_)) {
      ...
      error = Error(Error::Code::kUnknownError, "SYSTEM_ERROR");
      message_port_.SetSocket({});
    }
  }
...
void ApplicationAgent::PopulateReceiverStatus(Json::Value* out) {
  ...
  if (launched_app_) {
    ...
    details["universalAppId"] = launched_via_app_id_;
    details["displayName"] = launched_app_->GetDisplayName();
    ...
    Json::Value& namespaces =
        (details["namespaces"] = Json::Value(Json::arrayValue));
    for (int i = 0, count = app_namespaces.size(); i < count; ++i) {
      namespaces[i]["name"] = std::move(app_namespaces[i]);
    }
  }
