...
int RunStandaloneReceiver(int argc, char* argv[]) {
  ...
  OSP_LOG_ERROR
      << "It compiled! However cast_receiver currently only supports using a "
         "passed-in certificate and private key, and must be build with "
         "cast_enable_alternate_certificate=true set in the GN args to "
  ...
  if (private_key_path.empty() && !server_certificate_path.empty()) {
    OSP_LOG_ERROR << "Passing a root certificate but not its associated "
