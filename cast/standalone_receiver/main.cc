...
void LogUsage(const char* argv0) {
  ...
  constexpr char kUsageMessage[] = R"(
    usage: argv[0] <options>
      ...
      -l, --legacy: Start legacy session without TLS and dummy OFFER message.
...
int main(int argc, char* argv[]) {
  // TODO(jophba): refactor into separate method and make main a one-liner, so
  // we can avoid all of these using statements.
  ...
  int ch = -1;
  while ((ch = getopt_long(argc, argv, "i:lth", argument_options, nullptr)) !=
         -1) {
    switch (ch) {
      case 'i': {
