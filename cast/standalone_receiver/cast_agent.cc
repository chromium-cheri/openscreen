...
ErrorOr<TlsCredentials> CreateCredentials(
  ...
  *pkey = nullptr;
  ...
  auto root_cert_bytes_or_error =
      ExportX509CertificateToDer(*root_cert.value());
  if (root_cert_bytes_or_error) {
    *root_cert_bytes = std::move(root_cert_bytes_or_error.value());
  }

  ErrorOr<bssl::UniquePtr<X509>> intermediate_cert =
      CreateSelfSignedX509Certificate(
          "Cast Generated Intermediate Certificate", kCertificateDuration,
          *private_key.value(), GetWallTimeSinceUnixEpoch(), true,
          root_cert.value().get(), private_key.value().get());
  if (!intermediate_cert) {
    return intermediate_cert.error();
  }

  ErrorOr<bssl::UniquePtr<X509>> cert = CreateSelfSignedX509Certificate(
      "Cast Generated Target Certificate", kCertificateDuration,
      *private_key.value(), GetWallTimeSinceUnixEpoch(), true,
      intermediate_cert.value().get(), private_key.value().get());
  if (!cert) {
    return cert.error();
  }
