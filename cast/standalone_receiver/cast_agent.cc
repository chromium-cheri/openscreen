...
Error CastAgent::Start() {
  ...
  task_runner_->PostTask(
      [this] { this->wake_lock_ = ScopedWakeLock::Create(task_runner_); });
  ...
  controller_ = std::make_unique<StreamingPlaybackController>(task_runner_, this);
  current_session_ = std::make_unique<ReceiverSession>(
      controller_.get(), environment_.get(), &port_,
  ...
  port_.ReceiveMessage(offer_message.value());
