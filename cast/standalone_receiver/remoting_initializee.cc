...
#include "cast/standalone_receiver/remoting_initializee.h"
...
RemotingInitializee::RemotingInitializee(RpcMessenger* messenger)
...
void RemotingInitializee::IndicateReady() {
  OSP_DVLOG_INFO
...
void RemotingInitializee::SetInitializeCallback(
    RemotingInitializee::InitializeCallback initialize_cb) {
  initialize_cb_ = std::move(initialize_cb);
}

void RemotingInitializee::OnInitializeCallbackMessage(
  ...
  auto audio_codec = AudioCodec::kNotSpecified;
  auto video_codec = VideoCodec::kNotSpecified;
