...
class CastAgent final : public ReceiverSocketFactory::Client,
 ...
 public:
  ...
  // Necessary so that the fake sender in tests can validate the root
  // certificate, by storing it in the TrustStore.
  std::vector<uint8_t> GetRootTlsCertAsDerForTesting() {
    return root_cert_bytes_for_testing_;
  }
 ...
 private:
  ...
  std::vector<uint8_t> root_cert_bytes_for_testing_;
};
