...
void StreamingPlaybackController::OnNegotiated(const ReceiverSession* session,
...
void StreamingPlaybackController::OnReceiversDestroyed(const ReceiverSession* session) {
  audio_player_.reset();
  video_player_.reset();
}

void StreamingPlaybackController::OnError(const ReceiverSession* session, Error error) {
