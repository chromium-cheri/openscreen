...
//   auto agent =
//       MakeSerialDelete<CastAgent>(task_runner_, task_runner_,
//       loopback.value());
//   EXPECT_TRUE(agent->Start().ok());
//   EXPECT_TRUE(agent->Stop().ok());
// }

TEST_F(CastAgentIntegrationTest, BindsCastSocket) {
  absl::optional<InterfaceInfo> loopback = GetLoopbackInterfaceForTesting();
  ASSERT_TRUE(loopback.has_value());

  auto agent =
      MakeSerialDelete<CastAgent>(task_runner_, task_runner_, loopback.value());
  EXPECT_TRUE(agent->Start().ok());
  CastTrustStore::CreateInstanceForTest(agent->GetRootTlsCertAsDerForTesting());
  Connect(loopback.value().GetIpAddressV4());
  EXPECT_TRUE(agent->Stop().ok());
}
