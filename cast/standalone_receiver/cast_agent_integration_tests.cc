...
// Based heavily on MockSender from cast_socket_e2e_test.cc.
    ...
    task_runner_->PostTask(
        [this, &address, port = (static_cast<uint16_t>(kDefaultCastPort))]() {
          ...
          sender_factory_->Connect(
              ...
              SenderSocketFactory::DeviceMediaPolicy::kIncludesVideo,
              sender_router_.get());
        });
...
TEST_F(CastAgentIntegrationTest, BindsCastSocketToStandardPort) {
  auto platform_client = PlatformClientPosix::GetInstance();
