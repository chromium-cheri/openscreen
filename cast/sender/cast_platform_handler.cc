...
void CastPlatformHandler::RequestAppAvailability(
  ...
  int32_t socket_id = entry->second;
...
void CastPlatformHandler::AddOrUpdateReceiver(const ServiceInfo& device,
                                              int32_t socket_id) {
  socket_id_by_device_id_[device.unique_id] = socket_id;
}

void CastPlatformHandler::RemoveReceiver(const ServiceInfo& device) {
...
void CastPlatformHandler::OnMessage(VirtualConnectionRouter* router,
  ...
  if (request_id) {
    auto entry = std::find_if(
        ...
        [socket](const std::pair<std::string, int32_t>& entry) {
...
int32_t CastPlatformHandler::GetNextRequestId() {
