...
TEST_F(CastAppAvailabilityTrackerTest, RegisterSourceReturnsMultipleAppIds) {
  std::vector<std::string> app_ids(GetCastStreamingAppIds().begin(),
...
TEST_F(CastAppAvailabilityTrackerTest, MultipleAppIdsAlreadyTrackingOne) {
  auto single_app_id = GetCastStreamingAudioVideoAppIds()[0];
  std::vector<std::string> all_app_ids(GetCastStreamingAppIds().begin(),
                                       GetCastStreamingAppIds().end());
