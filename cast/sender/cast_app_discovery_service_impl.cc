...
void CastAppDiscoveryServiceImpl::AddOrUpdateDevice(const ServiceInfo& device) {
...
void CastAppDiscoveryServiceImpl::RemoveDevice(const ServiceInfo& device) {
  const std::string& device_id = device.unique_id;
...
void CastAppDiscoveryServiceImpl::RequestAppAvailability(
  ...
  if (ShouldRefreshAppAvailability(device_id, app_id, clock_())) {
    platform_handler_->RequestAppAvailability(
        device_id, app_id,
        // TODO(btolsch): This needs WeakPtr.
...
bool CastAppDiscoveryServiceImpl::ShouldRefreshAppAvailability(
  ...
  // TODO(btolsch): Consider an exponential backoff mechanism instead.  Devices
  // will typically respond with "unavailable" immediately after boot and then
  // become available 10-30 seconds later.
