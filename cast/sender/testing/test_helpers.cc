...
CastMessage CreateAppAvailabilityResponseChecked(
    int32_t request_id,
    const std::string& sender_id,
    const std::string& app_id,
    AppAvailabilityResult availability_result) {
  ErrorOr<CastMessage> message = CreateAppAvailabilityResponse(
      request_id, sender_id, app_id, availability_result);
  OSP_CHECK(message);
