...
void SenderSocketFactory::OnAccepted(
  ...
  OSP_DCHECK(false) << "This factory is connect-only.";
}
...
void SenderSocketFactory::OnError(CastSocket* socket, Error error) {
  auto it = std::lower_bound(pending_auth_.begin(), pending_auth_.end(),
                             socket->socket_id());
...
void SenderSocketFactory::OnMessage(CastSocket* socket, CastMessage message) {
  ...
  OSP_DCHECK_EQ((*it)->socket->socket_id(), socket->socket_id());
