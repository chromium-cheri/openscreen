...
using AuthResultCode = openscreen::Error::Code;
...
TEST_F(CastAuthUtilTest, VerifySuccess) {
  ...
  ASSERT_TRUE(certificate::ConvertTimeSeconds(
...
TEST_F(CastAuthUtilTest, VerifyBadCA) {
  ...
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kErrCertsParse, result.error().code());
}
...
TEST_F(CastAuthUtilTest, VerifyBadSignature) {
  ...
  AuthResult result = VerifyCredentials(auth_response, signed_data);
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kSignedBlobsMismatch, result.error().code());
}
...
TEST_F(CastAuthUtilTest, VerifyEmptySignature) {
  ...
  AuthResult result = VerifyCredentials(auth_response, signed_data);
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kSignatureEmpty, result.error().code());
}
...
TEST_F(CastAuthUtilTest, VerifyUnsupportedDigest) {
  ...
  AuthResult result = VerifyCredentialsForTest(
      ...
      nullptr, now, true);
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kDigestUnsupported, result.error().code());
}
...
TEST_F(CastAuthUtilTest, VerifySenderNonceMatch) {
  ...
  AuthResult result = context.VerifySenderNonce(context.nonce(), true);
  EXPECT_TRUE(result);
}
...
TEST_F(CastAuthUtilTest, VerifySenderNonceMismatch) {
  ...
  AuthResult result = context.VerifySenderNonce(received_nonce, true);
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kSenderNonceMismatch, result.error().code());
}
...
TEST_F(CastAuthUtilTest, VerifyTLSCertificateSuccess) {
  ...
  ASSERT_TRUE(certificate::GetCertValidity(tls_cert, &not_before, &not_after));
  uint64_t x;
  ASSERT_TRUE(ConvertTimeSeconds(not_before, &x));
  std::chrono::seconds s(x);

  AuthResult result = VerifyTLSCertificate(tls_cert, s);
  EXPECT_TRUE(result);
  X509_free(tls_cert);
}
...
TEST_F(CastAuthUtilTest, VerifyTLSCertificateTooEarly) {
  ...
  AuthResult result = VerifyTLSCertificate(tls_cert, s);
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kTlsCertValidStartDateInFuture,
            result.error().code());
  X509_free(tls_cert);
}
...
TEST_F(CastAuthUtilTest, VerifyTLSCertificateTooLate) {
  ...
  AuthResult result = VerifyTLSCertificate(tls_cert, s);
  EXPECT_FALSE(result);
  EXPECT_EQ(AuthResultCode::kTlsCertExpired, result.error().code());
  X509_free(tls_cert);
}
...
bool RunTest(const certificate::DeviceCertTest& test_case) {
  ...
  OSP_DCHECK(
      certificate::ConvertTimeSeconds(cert_verify_time, &verification_time));

  std::string crl_bundle = test_case.crl_bundle();
  AuthResult result(certificate::CastDeviceCertPolicy::kUnrestricted);
  switch (test_case.expected_result()) {
    case certificate::PATH_VERIFICATION_FAILED:
      ...
      EXPECT_EQ(result.error().code(),
                AuthResultCode::kCertNotSignedByTrustedCa);
      return result.error().code() == AuthResultCode::kCertNotSignedByTrustedCa;
    ...
    case certificate::REVOCATION_CHECK_FAILED_WITHOUT_CRL:
      result = TestVerifyRevocation(
          ...
          cast_trust_store.get(), crl_trust_store.get());
      EXPECT_EQ(result.error().code(), AuthResultCode::kErrCrlInvalid);
      return result.error().code() == AuthResultCode::kErrCrlInvalid;
    ...
    case certificate::REVOCATION_CHECK_FAILED:
      result = TestVerifyRevocation(
          certificate_chain, crl_bundle, verification_time, true,
          cast_trust_store.get(), crl_trust_store.get());
      EXPECT_EQ(result.error().code(), AuthResultCode::kErrCertsRevoked);
      return result.error().code() == AuthResultCode::kErrCertsRevoked;
    case certificate::SUCCESS:
      result = TestVerifyRevocation(
          certificate_chain, crl_bundle, verification_time, false,
          cast_trust_store.get(), crl_trust_store.get());
      EXPECT_EQ(result.error().code(), AuthResultCode::kSignedBlobsMismatch);
      return result.error().code() == AuthResultCode::kSignedBlobsMismatch;
