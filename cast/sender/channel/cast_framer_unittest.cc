...
class CastFramerTest : public testing::Test {
 public:
  CastFramerTest()
      : buffer_(MessageFramer::max_message_size()),
  ...
  static constexpr size_t header_size() { return MessageFramer::header_size(); }
  static constexpr size_t max_body_size() {
    return MessageFramer::max_body_size();
  }
  static constexpr size_t max_message_size() {
    return MessageFramer::max_message_size();
  }
...
TEST_F(CastFramerTest, TestMessageFramerCompleteMessage) {
  ...
  ErrorOr<std::unique_ptr<CastMessage>> result = framer_.Ingest(1);
  ASSERT_TRUE(result);
...
TEST_F(CastFramerTest, BigEndianMessageHeader) {
  ...
  EXPECT_EQ(4u, framer_.BytesRequested());
  ErrorOr<std::unique_ptr<CastMessage>> result = framer_.Ingest(4);
  ASSERT_TRUE(result);
  EXPECT_EQ(nullptr, result.value().get());
...
TEST_F(CastFramerTest, TestCompleteMessageAtOnce) {
  ...
  ErrorOr<std::unique_ptr<CastMessage>> result =
      framer_.Ingest(cast_message_str_.size());
  ASSERT_TRUE(result);
  std::unique_ptr<CastMessage> message = result.MoveValue();
  EXPECT_NE(nullptr, message.get());
  EXPECT_EQ(message->SerializeAsString(), cast_message_.SerializeAsString());
