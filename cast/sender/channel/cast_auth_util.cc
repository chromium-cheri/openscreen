...
openscreen::Error ParseAuthMessage(const CastMessage& challenge_reply,
  ...
  if (auth_message->has_error()) {
    ...
    return openscreen::Error(CastCertError::kMessageError, ss.str());
  }
  if (!auth_message->has_response()) {
    return openscreen::Error(CastCertError::kNoResponse, PARSE_ERROR_PREFIX
...
// Maps CastCertError from certificate verification to AuthResult.
// If crl_required is set to false, all revocation related errors are ignored.
openscreen::Error MapToAuthResult(CastCertError error, bool crl_required) {
  switch (error) {
    case CastCertError::kErrCertsMissing:
      return openscreen::Error(CastCertError::kPeerCertEmpty,
    ...
    case CastCertError::kErrCertsDateInvalid:
      return openscreen::Error(CastCertError::kCertNotSignedByTrustedCa,
                               "Failed date validity check.");
    case CastCertError::kErrCertsVerifyGeneric:
      return openscreen::Error(
          CastCertError::kCertNotSignedByTrustedCa,
  ...
  switch (response_digest_algorithm) {
    case SHA1:
      if (enforce_sha256_checking) {
        return openscreen::Error(CastCertError::kDigestUnsupported,
    ...
    case SHA256:
      *digest_algorithm = certificate::DigestAlgorithm::kSha256;
      break;
    default:
      return CastCertError::kDigestUnsupported;
  }
  return openscreen::Error::None();
}
...
openscreen::Error AuthContext::VerifySenderNonce(
  ...
  if (nonce_ != nonce_response) {
    if (enforce_nonce_checking) {
      return openscreen::Error(CastCertError::kSenderNonceMismatch,
...
openscreen::Error VerifyTLSCertificate(X509* peer_cert,
  ...
  if (!certificate::GetCertValidity(peer_cert, &not_before, &not_after)) {
  ...
  OSP_CHECK(certificate::ConvertTimeSeconds(verification_time.count(),
                                            &verification_time_exploded));
  OSP_CHECK(certificate::ConvertTimeSeconds(lifetime_limit.count(),
                                            &lifetime_limit_exploded));
  if (verification_time_exploded < not_before) {
    return openscreen::Error(
        CastCertError::kTlsCertValidStartDateInFuture,
        PARSE_ERROR_PREFIX "Certificate's valid start date is in the future.");
  }
  if (not_after < verification_time_exploded) {
    return openscreen::Error(CastCertError::kTlsCertExpired,
                             PARSE_ERROR_PREFIX "Certificate has expired.");
  }
  if (lifetime_limit_exploded < not_after) {
    return openscreen::Error(CastCertError::kTlsCertValidityPeriodTooLong,
                             PARSE_ERROR_PREFIX
...
AuthResult VerifyCredentialsImpl(const AuthResponse& response,
  ...
  openscreen::Error result = MapToAuthResult(
  ...
  if (response.signature().empty() && !signature_input.empty()) {
    return openscreen::Error(CastCertError::kSignatureEmpty,
                             "Signature is empty.");
  }
  certificate::DigestAlgorithm digest_algorithm;
  ...
  if (!verification_context->VerifySignatureOverData(signature, siginput,
                                                     digest_algorithm)) {
    return openscreen::Error(CastCertError::kSignedBlobsMismatch,
...
AuthResult VerifyCredentials(const AuthResponse& response,
  ...
  OSP_CHECK(certificate::ConvertTimeSeconds(
