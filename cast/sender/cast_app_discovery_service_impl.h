...
// Keeps track of availability queries, receives device updates, and issues app
// availability requests based on these signals.
class CastAppDiscoveryServiceImpl : public CastAppDiscoveryService {
 public:
  // |platform_handler| must outlive |this|.
  ...
  std::unique_ptr<Subscription> StartObservingAvailability(
  ...
  void AddOrUpdateDevice(const ServiceInfo& device);
  void RemoveDevice(const ServiceInfo& device);
