...
class CastAppDiscoveryServiceImplTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    device_.v4_endpoint = fake_cast_socket_pair_.remote_endpoint;
 ...
 protected:
  ...
  StartObservingAvailability(const CastMediaSource& source,
...
TEST_F(CastAppDiscoveryServiceImplTest, StartObservingAvailability) {
  ...
  auto subscription1 = StartObservingAvailability(source_a_1_, &devices1);
  ...
  platform_handler_.AddOrUpdateDevice(device_, socket_->socket_id());
  app_discovery_service_.AddOrUpdateDevice(device_);
  ...
  auto subscription2 = StartObservingAvailability(source_a_2_, &devices2);

  CastMessage availability_response = CreateAppAvailabilityResponseChecked(
  ...
  subscription1.reset();
  platform_handler_.RemoveDevice(device_);
  app_discovery_service_.RemoveDevice(device_);
  ASSERT_EQ(devices1.size(), 1u);
...
TEST_F(CastAppDiscoveryServiceImplTest, ReAddDeviceQueryUsesCachedValue) {
  ...
  EXPECT_CALL(peer_client(), OnMessage(_, _)).Times(0);
  subscription1 = StartObservingAvailability(source_a_1_, &devices1);
  ASSERT_EQ(devices1.size(), 1u);
  EXPECT_EQ(devices1[0].unique_id, "deviceId1");
}
...
TEST_F(CastAppDiscoveryServiceImplTest, Refresh) {
  ...
  auto subscription1 = StartObservingAvailability(source_a_1_, &devices1);
  std::vector<ServiceInfo> devices2;
  auto subscription2 = StartObservingAvailability(source_b_1_, &devices2);
  ...
  EXPECT_TRUE(peer_socket().SendMessage(availability_response).ok());
  availability_response = CreateAppAvailabilityResponseChecked(
...
TEST_F(CastAppDiscoveryServiceImplTest,
       StartObservingAvailabilityAfterDeviceAdded) {
  // No registered apps.
  EXPECT_CALL(peer_client(), OnMessage(_, _)).Times(0);
  platform_handler_.AddOrUpdateDevice(device_, socket_->socket_id());
  app_discovery_service_.AddOrUpdateDevice(device_);
  ...
  device2.v4_endpoint = fake_sockets2.remote_endpoint;
  ...
  platform_handler_.AddOrUpdateDevice(device2, socket2->socket_id());
  app_discovery_service_.AddOrUpdateDevice(device2);
}
