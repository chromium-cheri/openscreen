...
ReceiverChooser::ReceiverChooser(const InterfaceInfo& interface,
  ...
  watcher_ = std::make_unique<DnsWatcher>(
...
void ReceiverChooser::OnFatalError(Error error) {
  OSP_NOTREACHED() << "Fatal error: " << error;
}
...
void ReceiverChooser::PrintMenuAndHandleChoice() {
  ...
  std::cout << "\nEnter choice, or nothing to wait longer: " << std::flush;
  ...
  int menu_choice = -1;
  if (sscanf(input, "%d", &menu_choice) == 1 && menu_choice >= 0 &&
