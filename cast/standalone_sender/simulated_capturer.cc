...
SimulatedCapturer::SimulatedCapturer(Environment* environment,
  ...
  OverrideDecoderContextParameters(decoder_context_.get());
...
void SimulatedCapturer::OverrideDecoderContextParameters(
  ...
  if (receive_frame_result < 0) {
    switch (receive_frame_result) {
      case AVERROR(EAGAIN):
        ...
        return;
      case AVERROR_EOF:
        observer_->OnEndOfFileReached(this);
        return;
      default:
...
void SimulatedVideoCapturer::OverrideDecoderContextParameters(
    AVCodecContext* decoder_context) {
