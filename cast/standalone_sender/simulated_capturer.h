...
class SimulatedCapturer {
 public:
  ...
  class Observer {
   public:
    ...
    // halted.
    virtual void OnEndOfFileReached(SimulatedCapturer* capturer) = 0;
 ...
 protected:
  ...
  virtual void OverrideDecoderContextParameters(
 ...
 private:
  ...
  // and/or calls Observer::OnEndOfFileReached() if there are no more frames in
  // the file.
...
class SimulatedVideoCapturer final : public SimulatedCapturer {
 ...
 private:
  ...
  void OverrideDecoderContextParameters(AVCodecContext* decoder_context) final;
