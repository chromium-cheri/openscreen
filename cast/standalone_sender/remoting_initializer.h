...
#ifndef CAST_STANDALONE_SENDER_REMOTING_INITIALIZER_H_
#define CAST_STANDALONE_SENDER_REMOTING_INITIALIZER_H_
...
// RemotingInitializee instance (if configured) that initialization has been
...
class RemotingInitializer {
 public:
  RemotingInitializer(RpcMessenger* messenger,
                      AudioCodec audio_codec,
                      VideoCodec video_codec);

  // The remoting stream is considered "ready" when the receiver has responded
  // with an initialize message and we reply with the initialization callback
  // message.
  void SetReadyCallback(std::function<void()> ready_cb);
 ...
 private:
  ...
  // information.
  void OnInitializeMessage(std::unique_ptr<RpcMessage> message);
...
#endif  // CAST_STANDALONE_SENDER_REMOTING_INITIALIZER_H_
