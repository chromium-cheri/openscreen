...
#include "third_party/aomedia/src/aom/aomcx.h"
...
void StreamingAv1Encoder::ProcessWorkUnitsUntilTimeToQuit() {
  ...
  OSP_VLOG << "ProcessWorkUnitsUntilTimeToQuit";

  for (;;) {
    OSP_VLOG << "inside for loop";
    ...
    OSP_VLOG << "time: " << work_unit.stats.encode_wall_time;
...
void StreamingAv1Encoder::PrepareEncoder(int width,
  ...
  // auto ctl_result = aom_codec_control(
  //     &encoder_, AOME_SET_CPUUSED,
  //     GetCpuSpeed(config_.g_w, config_.g_h, settings.number_of_cores));
  auto ctl_result = aom_codec_control(&encoder_, AV1E_SET_ENABLE_CDEF, 1);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_ENABLE_TPL_MODEL, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result = aom_codec_control(&encoder_, AV1E_SET_DELTAQ_MODE, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result = aom_codec_control(&encoder_, AV1E_SET_ENABLE_ORDER_HINT, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result = aom_codec_control(&encoder_, AV1E_SET_AQ_MODE, 3);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // if (SvcEnabled()) {
  //   ctl_result= aom_codec_control(&encoder_, AV1E_SET_SVC_PARAMS,
  //   &*svc_params_);
  // }
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result =
      aom_codec_control(&encoder_, AOME_SET_MAX_INTRA_BITRATE_PCT, 300);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_COEFF_COST_UPD_FREQ, 2);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_MODE_COST_UPD_FREQ, 2);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_MV_COST_UPD_FREQ, 3);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result = aom_codec_control(&encoder_, AV1E_SET_TILE_COLUMNS,
                                 config_.g_threads >> 1);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_ROW_MT, 1);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_ENABLE_OBMC, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result = aom_codec_control(&encoder_, AV1E_SET_ENABLE_WARPED_MOTION, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  // ctl_result= aom_codec_control(&encoder_, AV1E_SET_ENABLE_GLOBAL_MOTION, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
  ctl_result = aom_codec_control(&encoder_, AV1E_SET_ENABLE_REF_FRAME_MVS, 0);
  OSP_CHECK_EQ(ctl_result, AOM_CODEC_OK);
}
...
void StreamingAv1Encoder::EncodeFrame(bool force_key_frame,
  ...
  const auto encode_result =
      aom_codec_encode(&encoder_, work_unit->image.get(), pts,
                       work_unit->duration.count(), flags);
...
void StreamingAv1Encoder::UpdateSpeedSettingForNextFrame(const Stats& stats) {
  OSP_DCHECK_EQ(std::this_thread::get_id(), encode_thread_.get_id());
...
void StreamingAv1Encoder::SendEncodedFrame(WorkUnitWithResults results) {
  ...
  OSP_VLOG << "SendEncodedFrame → " << frame.frame_id << ": "
