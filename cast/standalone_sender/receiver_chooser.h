...
class ReceiverChooser final : public discovery::ReportingClient {
 ...
 private:
  using DnsService = discovery::DnsSdService;
  using DnsWatcher = discovery::DnsSdServiceWatcher<ServiceInfo>;
  ...
  SerialDeletePtr<DnsService> service_;
  std::unique_ptr<DnsWatcher> watcher_;
