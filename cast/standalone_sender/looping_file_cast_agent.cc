...
void LoopingFileCastAgent::CreateAndStartSession() {
  ...
  if (connection_settings_->use_remoting) {
    initializer_ = std::make_unique<RemotingInitializer>(
        current_session_->rpc_messenger(), AudioCodec::kOpus, VideoCodec::kVp8);
    initializer_->SetReadyCallback([this]() { OnRemotingReady(); });
...
void LoopingFileCastAgent::OnRemotingReady() {
...
void LoopingFileCastAgent::StartRemotingSenders() {
  ...
  file_sender_ = std::make_unique<LoopingFileSender>(
      environment_.get(), connection_settings_->path_to_file.c_str(),
      current_session_.get(), std::move(current_negotiation_->senders),
