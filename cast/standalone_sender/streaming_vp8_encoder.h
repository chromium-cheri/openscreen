...
// the main thread while frames are being encoded. EncodeAndSend() is used to
// queue-up video frames for encoding and sending, which will be encoded and
// sent on a best-effort basis.
//
// The client is expected to call SetTargetBitrate() frequently based on its own
// bandwidth estimates and congestion control logic. In addition, a client may
// provide a callback for each frame's encode statistics, which can be used to
// further optimize the user experience. For example, the stats can be used as a
// signal to reduce the data volume (i.e., resolution and/or frame rate) coming
...
class StreamingVp8Encoder {
 public:
  ...
  int target_bitrate() const { return target_bitrate_; }
