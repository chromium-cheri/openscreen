...
bool ExtractStringArray(CborValue* it, std::vector<std::string>* strings_out) {
  CborValue array;
  size_t array_length = 0;
  CBOR_RETURN_WHAT_ON_ERROR(cbor_value_get_array_length(it, &array_length),
                            false);
  CBOR_RETURN_WHAT_ON_ERROR(cbor_value_enter_container(it, &array), false);
  strings_out->reserve(array_length);
  for (size_t i = 0; i < array_length; ++i) {
    size_t url_length = 0;
    CBOR_RETURN_WHAT_ON_ERROR(cbor_value_validate(&array, CborValidateUtf8),
                              false);
    if (cbor_value_is_length_known(&array)) {
      CBOR_RETURN_WHAT_ON_ERROR(
          cbor_value_get_string_length(&array, &url_length), false);
    } else {
      CBOR_RETURN_WHAT_ON_ERROR(
          cbor_value_calculate_string_length(&array, &url_length), false);
    }
    ...
    CBOR_RETURN_WHAT_ON_ERROR(
        cbor_value_copy_text_string(
            ...
            nullptr),
        false);
    CBOR_RETURN_WHAT_ON_ERROR(cbor_value_advance(&array), false);
  }
  CBOR_RETURN_WHAT_ON_ERROR(cbor_value_leave_container(it, &array), false);
  return true;
}
...
ssize_t DecodeUrlAvailabilityRequest(uint8_t* buffer,
  ...
  if (!ExtractStringArray(&map, &request->urls)) {
    return -1;
  }
