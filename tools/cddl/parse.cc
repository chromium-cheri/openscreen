...
// Determines if the given character matches [a-zA-Z@_$]
...
// Looks for an occurance indicator prefacing the group definition
...
AstNode* ParseGroupEntry1(Parser* p) {
  ...
  // Check for an occurance indicator ('?', '*', "+") before the entry def
...
// Parses an entry which is a reference to another sub-group
...
// Parse a group entry that's another group
...
AstNode* ParseGroupEntry(Parser* p) {
  ...
  // NOTE: This is the method hit durign the top-level group parsing, and the
