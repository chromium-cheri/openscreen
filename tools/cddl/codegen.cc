...
bool Encode%1$s(
  ...
  local_buffer.resize(CborEncodeBuffer::kDefaultInitialEncodeBufferSize);
  local_buffer[0] = static_cast<uint8_t>(Type::k%1$s);
  while (true) {
    ssize_t error_or_size = msgs::Encode%1$s(
        data, &local_buffer[1], local_buffer.size() - 1);
    if (error_or_size < 0)
      return false;
    else if (error_or_size > static_cast<ssize_t>(local_buffer.size() - 1))
      local_buffer.resize(error_or_size + 1);
    else
      return buffer->Append(local_buffer.data(), error_or_size + 1);
  }
}
)";
...
CborEncodeBuffer::CborEncodeBuffer(size_t initial_size, size_t max_size)
    : max_size_(max_size), data_(initial_size) {}
