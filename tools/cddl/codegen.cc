...
bool Encode%1$s(
  ...
  while (true) {
    ...
    if (IsError(error_or_size)) {
      ...
      if (!buffer->Extend(error_or_size - available_length))
        return false;
    } else {
      buffer->Release(available_length - error_or_size);
      return true;
    }
  }
}
...
bool CborEncodeBuffer::Extend(size_t extra_length) {
  if ((data_.size() + extra_length) > max_size_ || extra_length == 0)
    return false;
