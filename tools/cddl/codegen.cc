...
bool WriteTypeParserDefinition(int fd, const CppSymbolTable& table) {
  dprintf(fd, "\n//static\nType TypeParser::Parse(uint64_t type_id) {\n");
...
std::string GetEncodedTypeId(const CppType& type) {
  if (type.type_key == absl::nullopt) {
    return "";
  }
  ...
  uint8_t start_processing_bit;
  if (type_id < 0x1 << 6) {
    encoding_size = 0x0;
    start_processing_bit = 0;
  } else if (type_id < 0x1 << 14) {
    encoding_size = 0x01;
    start_processing_bit = 1;
  } else if (type_id < 0x1 << 30) {
    encoding_size = 0x02;
    start_processing_bit = 3;
  } else if (type_id < uint64_t{0x1} << 60) {
    encoding_size = 0x03;
    start_processing_bit = 7;
  } else {
    return "";
  }
  ...
  std::stringstream ss;
  uint8_t first_byte =
      encoding_size << 6 | GetByte(type_id, start_processing_bit);
  ss << "{0x" << std::hex << uint64_t{first_byte};
  for (int i = start_processing_bit - 1; i >= 0; i--) {
    ss << ", 0x" << std::hex << uint64_t{GetByte(type_id, i)};
  }
  ss << "}";
  return ss.str();
}
...
bool Encode%1$s(
  ...
  while (true) {
    ...
    std::string encoded_id = GetEncodedTypeId(*real_type);
...
class TypeParser {
 public:
  static Type Parse(uint64_t type_id);
};
