...
CddlType::~CddlType() {
  switch (which) {
    ...
    case CddlType::Which::kValue:
      value_value.std::string::~basic_string();
      break;
...
CddlType* HandleType(CddlSymbolTable* table, AstNode* type);
CddlGroup* HandleGroup(CddlSymbolTable* table, AstNode* group);

CddlType* HandleType2(CddlSymbolTable* table, AstNode* type2) {
  ...
  if (node->type == AstNode::Type::kNumber ||
    ...
    table->types.emplace_back(new CddlType);
    CddlType* id = table->types.back().get();
    ...
    if (*type2->text == '#' && type2->text[1] == '6' && type2->text[2] == '.') {
      table->types.emplace_back(new CddlType);
      CddlType* tagged_type = table->types.back().get();
      ...
      tagged_type->tagged_type.type = HandleType(table, node);
      return tagged_type;
    }
  } else if (node->type == AstNode::Type::kGroup) {
    if (*type2->text == '{') {
      table->types.emplace_back(new CddlType);
      CddlType* map = table->types.back().get();
      map->which = CddlType::Which::kMap;
      map->map = HandleGroup(table, node);
      return map;
    } else if (*type2->text == '[') {
      table->types.emplace_back(new CddlType);
      CddlType* array = table->types.back().get();
      array->which = CddlType::Which::kArray;
      array->array = HandleGroup(table, node);
      return array;
    } else if (*type2->text == '&') {
      table->types.emplace_back(new CddlType);
      CddlType* group_choice = table->types.back().get();
      group_choice->which = CddlType::Which::kGroupChoice;
      group_choice->group_choice = HandleGroup(table, node);
      return group_choice;
    }
...
CddlType* HandleType1(CddlSymbolTable* table, AstNode* type1) {
  return HandleType2(table, type1->children);
}
...
CddlType* HandleType(CddlSymbolTable* table, AstNode* type) {
  ...
  if (type1->sibling) {
    table->types.emplace_back(new CddlType);
    CddlType* type_choice = table->types.back().get();
    type_choice->which = CddlType::Which::kDirectChoice;
    new (&type_choice->direct_choice) std::vector<CddlType*>();
    while (type1) {
      type_choice->direct_choice.push_back(HandleType1(table, type1));
      type1 = type1->sibling;
    }
    return type_choice;
  } else {
    return HandleType1(table, type1);
  }
}

bool HandleGroupEntry(CddlSymbolTable* table,
                      AstNode* group_entry,
                      CddlGroup::Entry* entry);
...
CddlGroup* HandleGroup(CddlSymbolTable* table, AstNode* group) {
  ...
  while (node) {
    ...
    HandleGroupEntry(table, node, group_def->entries.back().get());
    node = node->sibling;
  }
  return group_def;
}
...
bool HandleGroupEntry(CddlSymbolTable* table,
  ...
  if (node->type == AstNode::Type::kMemberKey) {
    ...
    new (&entry->type) CddlGroup::Entry::EntryType;
  ...
  if (node->type == AstNode::Type::kType) {
    ...
    entry->type.value = HandleType(table, node);
  } else if (node->type == AstNode::Type::kGroupname) {
    return false;
  } else if (node->type == AstNode::Type::kGroup) {
    entry->which = CddlGroup::Entry::Which::kGroup;
    entry->group = HandleGroup(table, node);
  }
  return true;
}
...
std::pair<bool, CddlSymbolTable> BuildSymbolTable(AstNode* rules) {
  ...
  node = node->sibling;
  if (is_type) {
    CddlType* type = HandleType(&table, node);
    if (!type) {
      return result;
    }
    ...
    HandleGroupEntry(&table, node, group->entries.back().get());
  ...
  while (rule) {
    ...
    if (is_type) {
      ...
      table.type_map.emplace(name, type);
    } else {
      table.groups.emplace_back(new CddlGroup);
      CddlGroup* group = table.groups.back().get();
      group->entries.emplace_back(new CddlGroup::Entry);
      HandleGroupEntry(&table, node, group->entries.back().get());
      table.group_map.emplace(name, group);
    }
    rule = rule->sibling;
  }
  ...
  for (const auto& x : entries) {
    if (x->which == CddlGroup::Entry::Which::kType) {
      if (x->type.opt_key.empty()) {
        ...
        CppType* member_type = MakeCppType(
            table, cddl_table,
            cpp_type->name + std::string("_") + x->type.opt_key, x->type.value);
        if (!member_type) {
          return false;
        }
        if (x->opt_occurrence == "?") {
...
CppType* MakeCppType(CppSymbolTable* table,
  ...
  switch (type->which) {
    ...
    case CddlType::Which::kDirectChoice: {
      ...
      for (auto* cddl_choice : type->direct_choice) {
    ...
    case CddlType::Which::kTaggedType: {
      ...
      cpp_type->tagged_type.tag = atoi(type->tagged_type.tag_value.c_str());
...
std::pair<bool, CppSymbolTable> BuildCppTypes(CddlSymbolTable* cddl_table) {
  ...
  for (const auto& type_entry : cddl_table->type_map) {
    ...
    if (!MakeCppType(&table, cddl_table, type_entry.first, type_entry.second)) {
      return result;
    }
  }
  result.first = true;
