...
void CppType::InitVector() {
  ...
  vector_type.min_length = 0;
  vector_type.max_length = 0;
}
...
bool AnalyzeGroupEntry(CddlSymbolTable* table,
  ...
  if (node->type == AstNode::Type::kOccur) {
    if (node->text == "?") {
      entry->opt_occurrence_min = CddlGroup::Entry::kOccurenceMinUnbounded;
      entry->opt_occurrence_max = 1;
    } else if (node->text == "+") {
      entry->opt_occurrence_min = 1;
      entry->opt_occurrence_max = CddlGroup::Entry::kOccurenceMaxUnbounded;
    } else {
      ...
      int lower_bound = CddlGroup::Entry::kOccurenceMinUnbounded;
      ...
      int upper_bound = CddlGroup::Entry::kOccurenceMaxUnbounded;;
    ...
    entry->opt_occurrence_set = true;
    node = node->sibling;
  } else {
      entry->opt_occurrence_min = 1;
      entry->opt_occurrence_max = 1;
      entry->opt_occurrence_set = false;
  }
  ...
  for (const auto& x : entries) {
    if (x->which == CddlGroup::Entry::Which::kType) {
      if (x->type.opt_key.empty()) {
        ...
        if (x->opt_occurrence_min == CddlGroup::Entry::kOccurenceMinUnbounded &&
