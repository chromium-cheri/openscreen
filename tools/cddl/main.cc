...
int main(int argc, char** argv) {
  ...
  if (!DumpHeaderPrologue(header_fd, args.header_filename) ||
      !DumpDefs(header_fd, &cpp_result.second) ||
      !DumpHeaderEpilogue(header_fd, args.header_filename) ||
      !DumpSourcePrologue(cc_fd, args.header_filename) ||
      !DumpEncoders(cc_fd, &cpp_result.second) ||
      !DumpDecoders(cc_fd, &cpp_result.second) ||
      !DumpSourceEpilogue(cc_fd)) {
    return 1;
  }
